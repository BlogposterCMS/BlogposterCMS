import { mediaItemPath, type FolderListing } from './mediaLibraryData.js';
...
export function mediaEntries(listing: FolderListing, path: string): MediaEntry[] {
...
export function mediaType(entry: MediaEntry): string {
  if (entry.kind === 'folder') return 'Folder';
  const extension = entry.name.includes('.') ? entry.name.split('.').pop()!.toLowerCase() : '';
  if (/^(png|jpe?g|gif|webp|avif|svg|bmp|ico)$/.test(extension)) return 'Image';
  if (/^(mp4|webm|mov)$/.test(extension)) return 'Video';
  if (/^(mp3|wav|ogg|m4a)$/.test(extension)) return 'Audio';
  return extension ? `${extension.toUpperCase()} file` : 'File';
}

export function visibleMediaEntries(entries: MediaEntry[], query: string, key: MediaSortKey, descending: boolean): MediaEntry[] {
...
export function mediaSize(size: number | null): string {
  if (size === null) return '—';
...
export function mediaDate(value: string): string {
...
export function isMediaImage(entry: MediaEntry): boolean {
  return /\.(png|jpe?g|gif|webp|avif|bmp|ico)$/i.test(entry.name);
}

export function acceptsMedia(entry: MediaEntry, accept = ''): boolean {
  if (!accept || entry.kind === 'folder') return true;
  const name = entry.name.toLowerCase();
  return accept.split(',').some(value => {
    const rule = value.trim().toLowerCase();
    if (rule.startsWith('.')) return name.endsWith(rule);
    const type = mediaType(entry).toLowerCase();
    if (rule === 'image/*' || rule === 'video/*' || rule === 'audio/*') return rule.startsWith(type);
