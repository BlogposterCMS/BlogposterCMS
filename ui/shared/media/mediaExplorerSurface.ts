...
function commandButton(label: string, iconName: string, className = ''): HTMLButtonElement {
...
function fileIcon(name: string): string {
  ...
  if (/\.(png|jpe?g|gif|webp|svg)$/.test(lower)) return 'file-image';
  if (/\.(html?|css|js|mjs|json)$/.test(lower)) return 'file-code-2';
  return 'file';
}

function filterEntries(entries: MediaEntry[], query: string): MediaEntry[] {
...
function toEntries(listing: FolderListing, path: string): MediaEntry[] {
  return [
    ...listing.folders.map(name => ({ kind: 'folder' as const, name, path: mediaItemPath(path, name) })),
    ...listing.files.map(name => ({ kind: 'file' as const, name, path: mediaItemPath(path, name) }))
  ];
}
...
export function createMediaExplorerSurface(options: MediaExplorerSurfaceOptions = {}): MediaExplorerSurface {
  ...
  let currentView: MediaExplorerView = 'grid';
  let selectedPath = '';
  ...
  function setView(view: MediaExplorerView): void {
    currentView = view;
  ...
  async function choose(entry: MediaEntry): Promise<void> {
    try {
  ...
  async function share(entry: MediaEntry): Promise<void> {
    try {
  ...
  async function rename(entry: MediaEntry): Promise<void> {
  ...
  async function remove(entry: MediaEntry): Promise<void> {
    if (!await dialogApi().confirm(`Delete ${entry.name}?`)) return;
    try {
  ...
  function renderEntry(entry: MediaEntry): HTMLElement {
