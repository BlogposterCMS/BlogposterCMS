...
export function pageContentKind(page: ArticlePage): 'design' | 'html' | 'article' | 'empty' {
  if (ownPagePresentation(page)) return 'design';
  if (page.meta?.htmlFileName) return 'html';
  if (page.meta?.contentFormat === ARTICLE_FORMAT) return 'article';
...
export async function createPageDesign(base: ArticlePage): Promise<string> {
  ...
  if (pageContentKind(current) !== 'empty') throw new Error('PAGE_CONTENT_CHANGED: Reopen the content action.');
