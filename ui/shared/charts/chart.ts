/** Public widget API: data only, no raw ECharts options, HTML or remote assets. */
...
interface EngineChart {
  setOption(options: unknown, replace?: boolean): void;
  resize(): void;
  dispose(): void;
  on(event: string, handler: (value: { seriesName: string; dataIndex: number; value: number }) => void): void;
}
interface Engine { init(host: HTMLElement, theme: unknown, options: unknown): EngineChart }
...
function loadEngine(): Promise<Engine> {
  ...
  loading = new Promise((resolve, reject) => {
    ...
    script.onload = () => {
      const engine = (window as unknown as { echarts?: Engine }).echarts;
      if (engine?.init) resolve(engine);
      else { loading = undefined; reject(new Error('CHART_ENGINE_INVALID')); }
    };
...
export function chartOptions(data: ChartData): unknown {
  if (!Array.isArray(data.labels) || data.labels.length > 2000 || !Array.isArray(data.series)
    ...
    || data.series.some(series => typeof series.name !== 'string' || series.name.length > 200
      ...
      || series.values.some(value => !Number.isFinite(value)))) throw new Error('CHART_DATA_INVALID');
  return {
...
export async function mountChart(host: HTMLElement, data: ChartData): Promise<{ update(data: ChartData): void; dispose(): void }> {
  const options = chartOptions(data);
  const engine = await loadEngine();
  if (!host.isConnected) throw new Error('CHART_HOST_DETACHED');
  host.style.height ||= '260px';
  const chart = engine.init(host, null, { renderer: 'canvas' });
  let disposed = false;
  const update = (next: ChartData) => {
    if (disposed) throw new Error('CHART_DISPOSED');
  ...
  const resize = new ResizeObserver(() => chart.resize()); resize.observe(host);
  ...
  if (localRoot !== document) removal.observe(localRoot, { childList: true, subtree: true });
  function dispose() { if (disposed) return; disposed = true; resize.disconnect(); removal.disconnect(); chart.dispose(); }
