...
let projectNameSyncRetry: number | null = null;
...
function scheduleProjectNameSyncRetry(delay = 600): void {
  if (projectNameSyncRetry !== null) return;
...
function getProjectNameElement(): HTMLElement | null {
  const el = document.getElementById(PROJECT_NAME_ID);
  return el instanceof HTMLElement ? el : null;
}

function updateProjectName(name = PROJECT_NAME_FALLBACK): void {
...
async function syncProjectName(): Promise<void> {
  const projectName = getProjectNameElement();
  ...
  if (projectName.dataset.loading === 'true') return;
...
function bindLogout(): void {
  ...
  if (!(logoutIcon instanceof HTMLElement)) return;
