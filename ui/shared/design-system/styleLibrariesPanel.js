...
export function mountColorSchemePanel(host) {
    ...
    const render = () => {
        ...
        host.append(scopeIntro('Global color defaults. Individual elements can still override a color locally.'), schemeBar, fieldLabel('Scheme name', schemeName), schemeActions, createForm, fieldLabel('Default slot', slotSelect), fieldLabel('Name', slotName), fieldLabel('Light color', colorRow), fieldLabel('Dark color (optional)', darkRow), slotActions, errorNode);
        ...
        darkValue.addEventListener('input', () => {
            if (/^#[0-9a-f]{6}$/i.test(darkValue.value))
                darkControl.value = darkValue.value;
        });
