...
test.each(['ci.yml', 'release.yml'])('%s retains the Linux sandbox gate', filename => {
  ...
  expect(gate.run).toContain('git archive HEAD | sudo -u blogposter-sandbox-test tar -x -C /tmp/blogposter-sandbox-tests');
  expect(integration.run).toContain('cd /tmp/blogposter-sandbox-tests');
