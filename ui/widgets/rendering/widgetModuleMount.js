...
function renderRuntimeWidgetError(container, error) {
    container.replaceChildren(createRuntimeWidgetError(error));
}
function toRuntimeErrorMessage(err) {
    if (err instanceof Error && err.message)
        return err.message;
    return String(err);
}
/** Shared module lifecycle; callers retain loader and permission-context ownership. */
export async function mountWidgetModule(container, def, loadWidgetModule, createContext) {
    if (!def.codeUrl) {
