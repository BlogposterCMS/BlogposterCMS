import { normalizeUiDocument, readBinding, conditionMatches, resolveInput, uiError } from '/ui/shared/widget-ui/model.js';
/** Instance-local presentation state; module responses remain the authoritative data. */
export function createUiBindings(input, request, paint, preview = false) {
    const doc = normalizeUiDocument(input);
    const state = { ...doc.state };
    const data = {};
    const jobs = new Map();
    let disposed = false, resolvedNodes = 0;
    const scope = (event) => ({ state, data, event: event || {} });
    function resolveNode(node, local, suffix = '') {
        if (++resolvedNodes > 500)
            throw uiError('WIDGET_VIEW_NODE_LIMIT');
        if (node.repeat) {
            ...
            return items.slice(0, 100).flatMap((item, index) => {
                ...
                if (!/^[A-Za-z0-9_-]{1,80}$/.test(identity) || seen.has(identity))
                    throw uiError('WIDGET_REPEAT_KEY_INVALID');
                seen.add(identity);
                return resolveNode({ ...node, repeat: undefined }, { ...local, item, index }, `${suffix}-${identity}`);
            });
        }
        const result = { ...node, key: node.key ? node.key + suffix : undefined, props: { ...node.props } };
        for (const [field, binding] of Object.entries(node.bindings || {})) {
            const value = readBinding(local, binding.ref);
            if (['open', 'hidden'].includes(field))
                result[field] = !!value;
            else if (['text', 'value', 'label'].includes(field))
                result[field] = value == null ? '' : String(value);
            else
                result.props[field] = value;
        }
        if (node.when && !conditionMatches(node.when, local))
            result.hidden = true;
    ...
    function render() { if (!disposed) {
        resolvedNodes = 0;
        paint(resolveNode(doc.view, scope())[0]);
    } }
    async function load(name, event) {
        ...
        const job = { version: (previous?.version || 0) + 1, controller: new AbortController(), timer: undefined };
        jobs.set(name, job);
        ...
        const run = async () => {
            try {
                ...
                if (disposed || jobs.get(name) !== job)
                    return;
                data[name] = { status: 'ready', result };
                render();
            }
            catch (error) {
                if (disposed || jobs.get(name) !== job)
                    return;
                data[name] = { status: 'error', error: error?.status === 401 ? 'WIDGET_LOGIN_REQUIRED' : 'WIDGET_DATA_REQUEST_FAILED' };
                render();
            }
        };
        if (source.debounceMs)
            job.timer = setTimeout(() => { void run(); }, source.debounceMs);
        else
            await run();
    }
    async function dispatch(event) {
        if (disposed || event.isComposing)
            return;
        ...
        for (const action of actions) {
            if (action.type === 'set')
                state[action.target] = resolveInput(action.value, scope(event));
            else if (action.type === 'toggle')
                state[action.target] = !state[action.target];
            else
                void load(action.target, event);
        }
        render();
    }
    render();
