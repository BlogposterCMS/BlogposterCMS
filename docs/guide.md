...
Clicking **Publish** in the builder opens the right-side **Publishing** panel. It shows where the current design is already used, including public pages linked by `meta.designId` or `meta.layoutTemplate` and the current published bundle path when one exists. The same panel keeps the first-publication flow: search existing public pages by slug, or create a new page when the entered path does not exist. A preview below the slug field shows the full URL so you know where the page will live. A checkbox lets you set the page to draft, and a red notice reminds you that draft pages aren't publicly accessible.
