...
import type { RuntimeAdminGridLayoutItem } from './runtimeAdminGridMounting.js';
...
export type RuntimeAdminDashboardController = RendererGrid & {
  el: HTMLElement;
  widgets: HTMLElement[];
  ...
  updateSlot: (
    ...
    def?: RuntimeWidgetDefinition | null
  ) => void;
  cycleSlot: (el: HTMLElement, def?: RuntimeWidgetDefinition | null) => DashboardSlotName;
  moveWidget: (
    el: HTMLElement,
  ...
  on: (eventName: string, handler: GridHandler) => void;
...
type DashboardDragState = {
  ...
  widgetDef: RuntimeWidgetDefinition | null;
  ...
  lastClientX: number;
  lastClientY: number;
  snapPulseTimer: ReturnType<typeof setTimeout> | null;
};
...
function emitToHandlers(
  handlers: Map<string, GridHandler[]>,
  eventName: string,
  ...args: any[]
): void {
...
function removePageWidgetsBeforeAppend(
  gridEl: HTMLElement,
  nextWidget: HTMLElement
): void {
  const nextIsPage = nextWidget.dataset.dashboardSlot === 'page';
  gridEl.querySelectorAll<HTMLElement>('.dashboard-widget').forEach(widget => {
    if (widget === nextWidget) return;
...
function getDashboardFlowItems(gridEl: HTMLElement): HTMLElement[] {
  return Array.from(
    gridEl.querySelectorAll<HTMLElement>(`.dashboard-widget, .${DASHBOARD_PLACEHOLDER_CLASS}`)
  );
}

function syncWidgetOrder(gridEl: HTMLElement): void {
  let widgetIndex = 0;
...
function getRegisteredDashboardWidgets(gridEl: HTMLElement, widgets: HTMLElement[]): HTMLElement[] {
  const registered = new Set(widgets);
  return Array.from(gridEl.querySelectorAll<HTMLElement>('.dashboard-widget'))
    .filter(widget => registered.has(widget));
}

function closestDashboardWidget(
  target: EventTarget | null,
  gridEl: HTMLElement
): HTMLElement | null {
  if (!(target instanceof HTMLElement)) return null;
  const widget = target.closest<HTMLElement>('.dashboard-widget');
  return widget && gridEl.contains(widget) ? widget : null;
}

function closestDashboardWidgetFromEvent(
  event: Event,
  gridEl: HTMLElement
): HTMLElement | null {
...
function isIgnoredDashboardDragStart(event: Event): boolean {
...
function findDashboardWidgetByInstanceId(
  gridEl: HTMLElement,
  instanceId: string
): HTMLElement | null {
  return Array.from(gridEl.querySelectorAll<HTMLElement>('.dashboard-widget'))
...
function insertWidgetInFlow(
  ...
  if (!target || target === widget) {
    if (!target) gridEl.appendChild(widget);
    return;
  }
  gridEl.insertBefore(widget, placement === 'after' ? target.nextSibling : target);
}

function createDashboardDropPlaceholder(): HTMLElement {
...
function getDashboardPreviewLabel(
...
function removePreviewOnlyAttributes(root: HTMLElement): void {
  root.removeAttribute('id');
  root.querySelectorAll<HTMLElement>('[id]').forEach(el => el.removeAttribute('id'));
  root.querySelectorAll('script').forEach(script => script.remove());
  root
    .querySelectorAll<HTMLElement>('.widget-remove, .widget-resize, .widget-menu, .resize-handle')
...
function positionDashboardDragPreviewAt(
  preview: HTMLElement,
  clientX: number,
  clientY: number,
  anchorX: number,
...
function getSupportedSlotsFromElement(el: HTMLElement): DashboardSlotName[] {
...
function applyWidgetGridColumn(el: HTMLElement, column?: unknown): void {
...
function updatePlaceholderFromWidget(
  placeholder: HTMLElement,
  widget: HTMLElement,
  column?: unknown
): void {
...
function updatePlaceholderFromDefinition(
  placeholder: HTMLElement,
...
function getDashboardColumnFromEvent(
  gridEl: HTMLElement,
  event: DragEvent,
  slot: DashboardSlotName
): number | null {
...
function getDashboardColumnFromPoint(
  gridEl: HTMLElement,
  clientX: number,
  slot: DashboardSlotName
): number | null {
...
function findDashboardWidgetAtPoint(
  gridEl: HTMLElement,
  clientX: number,
  clientY: number,
  ignored?: HTMLElement | null
): HTMLElement | null {
...
function findNextDashboardWidget(
  placeholder: HTMLElement,
  ignored?: HTMLElement | null
): HTMLElement | null {
...
function resolvePanelDraggedWidget(): RuntimeWidgetDefinition | null {
  const id = window.__dashboardDraggingWidgetId;
  const widgets = Array.isArray(window.availableWidgets)
    ? window.availableWidgets
    : [];
  return widgets.find((widget: RuntimeWidgetDefinition) => widget.id === id) || null;
}
...
function clearSnapFeedback(state: DashboardDragState | null, gridEl: HTMLElement): void {
...
function updateSnapFeedback(
  ...
  if (changedColumn && slot !== 'full' && slot !== 'page') {
    ...
    if (state.snapPulseTimer) clearTimeout(state.snapPulseTimer);
...
export function createAdminDashboardController(
  ...
  const controller = {
    ...
    updateSlot(el: HTMLElement, slot: DashboardSlotName, def?: RuntimeWidgetDefinition | null) {
    ...
    cycleSlot(el: HTMLElement, def?: RuntimeWidgetDefinition | null) {
      const nextSlot = def
    ...
    emitChange(el?: HTMLElement | null) {
      emitToHandlers(handlers, 'change', el);
    },
    on(eventName: string, handler: GridHandler) {
  ...
  return controller as RuntimeAdminDashboardController;
}

export function bindAdminDropTarget(
  gridEl: HTMLElement,
  grid: RuntimeAdminDashboardController
): void {
  let dragState: DashboardDragState | null = null;
  ...
  const updateDragPreviewAtPoint = (
    state: DashboardDragState,
    clientX: number,
    clientY: number,
    ...
    positionDashboardDragPreviewAt(
      state.preview,
      state.lastClientX,
      state.lastClientY,
      state.previewAnchorX,
      state.previewAnchorY
    );

    const targetWidget = findDashboardWidgetAtPoint(
      gridEl,
      state.lastClientX,
      state.lastClientY,
      state.draggedWidget
    ) || closestDashboardWidget(fallbackTarget, gridEl);
    ...
    const slot = state.draggedWidget
      ...
      : resolveDashboardSlotForWidget(state.widgetDef as RuntimeWidgetDefinition);
  ...
  const releasePointerCapture = (state: DashboardDragState | null) => {
    if (!state?.draggedWidget || state.pointerId === null) return;
    try {
  ...
  const commitDraggedWidget = (event: PointerEvent) => {
    ...
    grid.moveWidget(
      draggedWidget,
      nextWidget,
      nextWidget ? 'before' : 'after',
      dragState.lastColumn ?? getDashboardColumnFromPoint(gridEl, event.clientX, slot)
    );
    cleanupDragState();
  };

  function handlePointerMove(event: PointerEvent): void {
    if (!dragState?.draggedWidget) return;
  ...
  gridEl.addEventListener('pointerdown', (event: PointerEvent) => {
    if (!document.body.classList.contains('dashboard-edit-mode')) return;
    if (event.button !== 0 || isIgnoredDashboardDragStart(event)) return;
    const widget = closestDashboardWidgetFromEvent(event, gridEl);
    if (!widget?.dataset.instanceId) return;
  ...
  gridEl.addEventListener('dragstart', (event: DragEvent) => {
    const widget = closestDashboardWidgetFromEvent(event, gridEl);
  ...
  gridEl.addEventListener('drop', (event: DragEvent) => {
    ...
    if (draggedInstanceId) {
      ...
      if (draggedWidget) {
        ...
        grid.moveWidget(
          ...
          nextWidget,
          nextWidget ? 'before' : 'after',
          dragState?.lastColumn ?? getDashboardColumnFromEvent(gridEl, event, slot)
        );
      }
      cleanupDragState();
      return;
    ...
    const widgets = Array.isArray(window.availableWidgets)
      ? window.availableWidgets
      : [];
    const def = widgets.find((widget: RuntimeWidgetDefinition) => widget.id === id);
...
export function serializeAdminDashboardLayout(
  gridEl: HTMLElement,
  resolveMeta: (instanceId: string, el: HTMLElement) => LayoutItem | null | undefined = () => ({})
): LayoutItem[] {
  return Array.from(gridEl.querySelectorAll<HTMLElement>('.dashboard-widget'))
    .map((el, index) => {
...
export function bindAdminLayoutPersistence({
  ...
  grid.on('change', () => {
    window.adminCurrentLayout = serializeAdminDashboardLayout(
      gridEl,
