...
export function articleLocaleDecorations(isFallback, languageButton) {
    return Extension.create({ name: 'sourceLanguageFeedback', addProseMirrorPlugins() {
            return [new Plugin({ props: { decorations(state) {
                            const decorations = [];
                            state.doc.forEach((node, position) => {
                                if (isFallback(node.toJSON()))
                                    decorations.push(Decoration.node(position, position + node.nodeSize, { class: 'article-editor__source-language' }));
