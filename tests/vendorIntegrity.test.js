...
const domPurifyManifest = require('../ui/shared/vendor/dompurify-manifest.json');
...
test('reviewed DOMPurify ESM distribution and licenses match pinned SHA-256 hashes', () => {
  for (const file of domPurifyManifest.files) {
    const bytes = fs.readFileSync(path.join(__dirname, '..', file.path));
    expect(() => verifyBytes(bytes, file.sha256, file.path)).not.toThrow();
  }
});
