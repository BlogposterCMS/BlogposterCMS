...
const ROLE_LABELS: Record<FontPackageRole, string> = {
  body: 'Body',
  h1: 'Heading 1',
  h2: 'Heading 2',
  h3: 'Heading 3',
...
function scopeIntro(text: string): HTMLParagraphElement {
...
function setInitialInputValue(input: HTMLInputElement, value: string): void {
...
function stabilizeMountedInputValue(input: HTMLInputElement, value: string): void {
  ...
  window.setTimeout(() => {
    ...
    if (input.isConnected) setInitialInputValue(input, value);
  }, 0);
}

function stabilizeMountedSelectValue(select: HTMLSelectElement, value: string): void {
  select.autocomplete = 'off';
  select.value = value;
  window.setTimeout(() => {
    if (select.isConnected) select.value = value;
  }, 0);
}

function showError(host: HTMLElement, error: unknown): void {
  const errorNode = host.querySelector<HTMLElement>('[data-font-package-error]');
  if (!errorNode) return;
  errorNode.textContent = error instanceof Error ? error.message : String(error || 'Unable to update font packages.');
  errorNode.classList.remove('hidden');
}

function clearError(host: HTMLElement): void {
  const errorNode = host.querySelector<HTMLElement>('[data-font-package-error]');
  if (!errorNode) return;
...
export function mountColorSchemePanel(host: HTMLElement): () => void {
  ...
  const render = (): void => {
    ...
    const activateButton = button(
      scheme.id === library.activeSchemeId ? 'Scheme active' : 'Use scheme',
      'font-package-action font-package-action--primary'
    );
    ...
    const darkValue = document.createElement('input');
    darkValue.type = 'text';
    setInitialInputValue(darkValue, selectedSlot.darkValue || '');
    darkValue.placeholder = 'Inherit light color';
    darkValue.setAttribute('aria-label', 'Dark default color value');
    const darkControl = document.createElement('input');
    darkControl.type = 'color';
    setInitialInputValue(darkControl, (selectedSlot.darkValue || selectedSlot.value).slice(0, 7));
    darkControl.setAttribute('aria-label', 'Choose dark default color');
    const darkRow = document.createElement('div');
    darkRow.className = 'color-scheme-value';
    darkRow.append(darkControl, darkValue);
    ...
    const showColorError = (error: unknown): void => {
      errorNode.textContent = error instanceof Error ? error.message : String(error || 'Unable to update color scheme.');
      errorNode.classList.remove('hidden');
    };

    host.append(
      scopeIntro('Global color defaults. Individual elements can still override a color locally.'),
      schemeBar,
      fieldLabel('Scheme name', schemeName),
      schemeActions,
      createForm,
      fieldLabel('Default slot', slotSelect),
      fieldLabel('Name', slotName),
      fieldLabel('Light color', colorRow),
      fieldLabel('Dark color (optional)', darkRow),
      slotActions,
      errorNode
    );
    ...
    slotSelect.addEventListener('change', () => {
      ...
      setInitialInputValue(darkValue, nextSlot.darkValue || '');
      setInitialInputValue(darkControl, (nextSlot.darkValue || nextSlot.value).slice(0, 7));
    ...
    darkControl.addEventListener('input', () => { darkValue.value = darkControl.value.toUpperCase(); });
    darkValue.addEventListener('input', () => {
      if (/^#[0-9a-f]{6}$/i.test(darkValue.value)) darkControl.value = darkValue.value;
    });
    ...
    deleteSchemeButton.addEventListener('click', async () => {
      if (!(await bpDialog.confirm(`Delete color scheme "${scheme.name}"?`))) return;
      try {
    ...
    createForm.addEventListener('submit', async event => {
      ...
      try {
        ...
        if (created) selectedSchemeId = created.id;
      } catch (error) {
        showColorError(error);
      }
    });
    saveSlotButton.addEventListener('click', async () => {
      try {
        await updateLibraryColor({
          ...
          darkValue: darkValue.value.trim()
        });
      } catch (error) {
        showColorError(error);
      }
    ...
    addSlotButton.addEventListener('click', async () => {
      try {
        ...
        if (created) selectedSlotId = created.id;
      } catch (error) {
        showColorError(error);
      }
    });
...
async function loadFontCatalog(): Promise<string[]> {
  const emit = window.meltdownEmit;
  if (typeof emit !== 'function') return [...SYSTEM_FONT_FAMILIES];
  try {
    const records = await emitRuntimeAdmin<FontRecord[]>(
      emit,
      window.ADMIN_TOKEN,
      'fonts',
      'list'
...
function setRolePreview(preview: HTMLElement, styles: FontRoleStyles): void {
...
function currentPackage(selectedPackageId: string): FontPackage | null {
...
function mountFontPackagesPanel(host: HTMLElement): () => void {
  let selectedPackageId = getFontPackagesSnapshot().activePackageId;
  let selectedRole: FontPackageRole = 'body';
  ...
  const render = (): void => {
    ...
    const renameButton = button('Save name', 'font-package-action');
    const activateButton = button(
      pkg.id === library.activePackageId ? 'Package active' : 'Use package',
      'font-package-action font-package-action--primary'
    );
    ...
    const controls = new Map<keyof FontRoleStyles, HTMLInputElement | HTMLSelectElement>();
    ...
    if (!linked) literalOption.selected = true;
    ...
    editor.append(
      fieldLabel('Font family', familySelect),
      fieldLabel('Color source', colorSelect),
      fieldLabel('Color', colorInput)
    );
    ...
    host.append(
      scopeIntro('Global typography defaults. Text set to Default follows the selected role.'),
      packageBar,
      fieldLabel('Package name', packageName),
      packageActions,
      newPackageForm,
      fieldLabel('Text role', roleSelect),
      editor,
      preview,
      roleActions,
      errorNode
    ...
    const readFormStyles = (): FontRoleStyles => Object.fromEntries(
      Array.from(controls.entries()).map(([key, control]) => [key, control.value])
    ) as unknown as FontRoleStyles;

    const updatePreview = (): void => {
      const previewStyles = readFormStyles();
      const selectedColor = colorSelect.selectedOptions[0]?.dataset.cssValue;
      if (selectedColor) previewStyles.color = selectedColor;
    ...
    roleSelect.addEventListener('change', () => {
      const nextRole = roleSelect.value as FontPackageRole;
    ...
    colorSelect.addEventListener('change', () => {
      ...
      if (selectedCssValue) colorInput.value = selectedCssValue;
      else if (parseLinkedColorValue(colorInput.value)) colorInput.value = '#111827';
      updatePreview();
    });
    ...
    deleteButton.addEventListener('click', async () => {
      if (!(await bpDialog.confirm(`Delete font package "${pkg.name}"?`))) return;
      clearError(host);
    ...
    newPackageForm.addEventListener('submit', async event => {
      ...
      try {
        ...
        if (created) selectedPackageId = created.id;
      } catch (error) {
        showError(host, error);
...
export function initStyleLibrariesPanel(sidebar: HTMLElement): () => void {
  const colorHost = sidebar.querySelector<HTMLElement>('[data-color-scheme-host]');
  const fontHost = sidebar.querySelector<HTMLElement>('[data-font-packages-host]');
  const cleanups: Array<() => void> = [];
  if (colorHost) cleanups.push(mountColorSchemePanel(colorHost));
  if (fontHost) cleanups.push(mountFontPackagesPanel(fontHost));
