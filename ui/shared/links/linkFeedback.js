...
import { createEditorLinkTargetChecker } from './linkTargetStatus.js';
/** Ephemeral authoring feedback. Decorations live outside saved content and never alter a link. */
export function mountLinkFeedback(root, options) {
    ...
    function show(link, result) {
        if (!link.anchor.isConnected)
            return;
        ...
        if (editorPopover) {
            let hint = editorPopover.querySelector('[data-bp-link-hint]');
            if (!hint) {
                hint = document.createElement('p');
                hint.dataset.bpLinkHint = 'true';
                hint.setAttribute('role', 'status');
                editorPopover.append(hint);
            }
        ...
        proxy.dataset.bpLinkFeedback = 'anchor';
        ...
        popover = openPopover(proxy, { content: result.message, role: 'tooltip', ariaLabel: 'Link target warning', autoFocus: false,
            onClose: () => { proxy.remove(); if (popoverAnchor?.proxy === proxy)
    ...
    async function refresh(announce = false) {
        ...
        for (let index = 0; index < links.length; index += 4) {
            ...
            if (ownRevision !== revision || stopped)
                return;
            next.push(...batch);
        }
        pending = false;
        ...
        for (const input of externalInputs) {
            const hint = input.closest('.bp-popover')?.querySelector('[data-bp-link-hint]');
            const warning = warnings.find(item => links.find(link => link.id === item.id)?.anchor === input);
            if (hint) {
                hint.textContent = warning?.message || '';
                hint.hidden = !warning;
            }
        }
    ...
    const detachObserver = new MutationObserver(() => { if (!root.isConnected)
        controller.stop(); });
    ...
    const controller = {
        ...
        watchInput: (input) => { externalInputs.add(input); input.addEventListener('input', schedule); },
        stop: () => {
            stopped = true;
            revision += 1;
            window.clearTimeout(timer);
...
export function htmlSourceLinks(input, anchor = input) {
    const template = document.createElement('template');
    template.innerHTML = input.value.slice(0, 2000000);
    return Array.from(template.content.querySelectorAll('a[href]')).slice(0, 41)
