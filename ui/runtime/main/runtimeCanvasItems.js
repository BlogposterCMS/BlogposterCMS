...
import { normalizeResponsivePlacementContract, projectResponsiveHorizontalPosition, resolveResponsivePlacementGeometry } from '/ui/shared/layout/responsivePlacement.js';
...
function applyRuntimeStyleSourceMetadata(wrapper, item) {
    const styleSource = item.styleSource || item.style_source || item.code?.meta?.styleSource || item.code?.meta?.style_source;
    if (!styleSource || typeof styleSource !== 'object' || Array.isArray(styleSource))
        return;
    if (styleSource.enabled !== undefined)
        wrapper.dataset.styleSourceEnabled = String(styleSource.enabled);
    if (styleSource.role)
        wrapper.dataset.styleSourceRole = String(styleSource.role);
    if (styleSource.sourceId)
        wrapper.dataset.styleSourceId = String(styleSource.sourceId);
    if (styleSource.syncLayout !== undefined)
        wrapper.dataset.styleSyncLayout = String(styleSource.syncLayout);
    if (styleSource.syncDesign !== undefined)
        wrapper.dataset.styleSyncDesign = String(styleSource.syncDesign);
}
...
export function resolveRuntimeCanvasRect(item, { scaleX, scaleY, percentDivisor = 100, defaultW = 8, defaultH = 4, def, heightProjectionMode = 'percent' }) {
    ...
    const canvasWidth = Math.max(1, percentDivisor === 1 ? scaleX * 100 : scaleX);
