...
export class SourceEditorError extends Error {
    code;
    constructor(code, message) { super(`${code}: ${message}`); this.name = 'SourceEditorError'; this.code = code; }
}
function lineSeparatorFor(value) {
    const at = value.search(/\r\n?|\n/);
    return at < 0 ? undefined : value.startsWith('\r\n', at) ? '\r\n' : '\n';
}
function supportFor(language) { return language === 'html' ? html() : css(); }
function theme() {
...
async function formatWithPrettier(source, language, endOfLine) {
...
export function createSourceEditor(options) {
    if (!options.parent || typeof options.parent.appendChild !== 'function')
        throw new SourceEditorError('SOURCE_EDITOR_INVALID_PARENT', 'A valid source editor parent is required.');
    if (!['html', 'css'].includes(options.language))
        throw new SourceEditorError('SOURCE_EDITOR_INVALID_LANGUAGE', 'The source editor language is unsupported.');
    if (typeof options.value !== 'string' || typeof options.onChange !== 'function')
        throw new SourceEditorError('SOURCE_EDITOR_INVALID_OPTIONS', 'Source editor value and change callback are required.');
    ...
    const usable = () => { if (destroyed)
        throw new SourceEditorError('SOURCE_EDITOR_DESTROYED', 'The source editor has been destroyed.'); };
    ...
    return {
        ...
        setValue: (nextValue) => {
            ...
            if (typeof nextValue !== 'string')
                throw new SourceEditorError('SOURCE_EDITOR_INVALID_VALUE', 'Source editor value must be text.');
            preferredSeparator = lineSeparatorFor(nextValue) ?? preferredSeparator;
            if (value() !== nextValue)
                view.dispatch({ changes: { from: 0, to: view.state.doc.length, insert: nextValue }, userEvent: 'input.setValue' });
        },
        ...
        format: async () => {
            usable();
            if (readOnly)
                throw new SourceEditorError('SOURCE_EDITOR_READ_ONLY', 'Read-only source cannot be formatted.');
            const sequence = ++formatSequence;
            const before = value();
            const version = documentVersion;
            if (before.length > SOURCE_EDITOR_MAX_FORMAT_LENGTH)
                throw new SourceEditorError('SOURCE_EDITOR_TOO_LARGE', 'Source is too large to format safely.');
            let formatted;
            try {
                formatted = await formatWithPrettier(before, options.language, preferredSeparator === '\r\n' ? 'crlf' : 'lf');
            }
            catch {
                throw formatError(options.language);
            }
            usable();
            if (readOnly)
                throw new SourceEditorError('SOURCE_EDITOR_READ_ONLY', 'Read-only source cannot be formatted.');
            if (sequence !== formatSequence || version !== documentVersion || value() !== before)
                throw new SourceEditorError('SOURCE_EDITOR_FORMAT_STALE', 'Source changed while formatting was in progress.');
            // Formatting must undo independently of the user's immediately preceding edits.
            if (formatted !== before)
                view.dispatch({ changes: { from: 0, to: view.state.doc.length, insert: formatted }, userEvent: 'input.format', annotations: isolateHistory.of('full') });
        },
        destroy: () => { if (destroyed)
            return; destroyed = true; formatSequence += 1; view.destroy(); },
    };
}
