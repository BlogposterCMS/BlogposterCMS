        ...
        for (const child of items.filter((candidate) => candidate.is_content)) {
            ...
            if (designId) {
                try {
                    ...
                    const rendered = await renderRuntimeDesignDocument(section, getRuntimeDesignDocument({ ...res, placements: layout }), allWidgets, lane, { emit, widgetEmit, designPath: [String(designId)] });
                    if (!rendered)
