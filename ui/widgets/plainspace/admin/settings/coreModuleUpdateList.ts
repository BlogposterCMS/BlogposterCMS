...
export function createCoreModuleUpdateList(mount: HTMLElement, install: (row: CoreModuleUpdateRow) => void, titleText = 'CMS module updates', selection?: {
  selected: (row: CoreModuleUpdateRow) => boolean;
  change: (row: CoreModuleUpdateRow, selected: boolean) => void;
}) {
  ...
  const entries = new Map<string, { container: HTMLElement; summary: HTMLElement; button: HTMLButtonElement; checkbox: HTMLInputElement; row: CoreModuleUpdateRow }>();
  return (rows: CoreModuleUpdateRow[], disabled: boolean, lastCheckedAt?: string) => {
    ...
    for (const row of rows) {
      ...
      if (!entry) {
        ...
        checkbox.addEventListener('change', () => {
          ...
          if (current && !checkbox.disabled) selection?.change(current.row, checkbox.checked);
        });
        button.addEventListener('click', () => {
          const current = entries.get(row.moduleName);
          if (current && !current.button.disabled) install(current.row);
        });
        details.append(summary, notes, button); container.append(checkbox, details); section.append(container);
      }
      ...
      const name = document.createElement('span'); name.textContent = row.label || row.moduleName;
