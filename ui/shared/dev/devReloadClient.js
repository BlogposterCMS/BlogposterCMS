...
export function refreshChangedStylesheets(paths, revision, targetDocument = document) {
    const changedPaths = new Set(paths);
    let refreshed = 0;
    targetDocument.querySelectorAll('link[rel="stylesheet"][href]').forEach(link => {
        const stylesheetUrl = new URL(link.href, targetDocument.baseURI);
        if (!changedPaths.has(stylesheetUrl.pathname))
            return;
...
export function handleDevReloadChange(change) {
...
export function initDevReloadClient() {
    const targetWindow = window;
    if (targetWindow.__BLOGPOSTER_DEV_RELOAD_ACTIVE__)
        return null;
    ...
    eventSource.addEventListener('hello', event => {
        const hello = parseEventData(event);
    ...
    eventSource.addEventListener('change', event => {
        const change = parseEventData(event);
        if (change)
            handleDevReloadChange(change);
    });
    return eventSource;
}
