...
test('handleSaveDesignPlaceholder updates existing design by ID', async () => {
  ...
  const sceneLayout = {
    ...
    scenes: [
      ...
      { id: 'empty-showcase', title: 'Empty Showcase' }
    ]
  };

  const first = await handleSaveDesignPlaceholder({
    dbClient: db,
    params: [{ design: { title: 'First', owner_id: 'u1', version: 0 }, widgets: [], layout: sceneLayout }]
  });
  ...
  const second = await handleSaveDesignPlaceholder({
    dbClient: db,
    params: [{ design: { id: first.id, title: 'Updated', owner_id: 'u1', version: first.version }, widgets: [], layout: sceneLayout }]
  });
  ...
  const latestLayout = JSON.parse(layouts[layouts.length - 1].layout_json);
  expect(latestLayout.scenes).toEqual(sceneLayout.scenes);

  const fetched = await handleGetDesignPlaceholder({
    dbClient: db,
    params: [{ id: first.id }]
  });
