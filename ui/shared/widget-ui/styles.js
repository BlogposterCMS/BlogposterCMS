...
export function applyUiStyles(element, styles = {}, preview = false) {
    normalizeKitComponents([{ id: 'styles', name: 'Widget styles', type: 'text', styles }]);
    element.removeAttribute('style');
    for (const [key, value] of Object.entries(styles)) {
        if (!COMPONENT_STYLE_PROPERTIES.has(key) && !layout.has(key) && !/^--[a-z][a-z0-9-]{0,79}$/.test(key))
            throw uiError('WIDGET_STYLE_PROPERTY_DENIED');
        ...
        if (key === 'zIndex' && (!/^\d{1,3}$/.test(value) || Number(value) > 100))
            throw uiError('WIDGET_STYLE_LAYER_LIMIT');
