...
function layoutNodeRole(el) {
    if (el.id === 'layoutRoot' || el.classList.contains('layout-root'))
        return 'layout-root';
    if (el.classList.contains('layout-section'))
        return 'section';
...
function layoutNodeFeedback(el, index) {
    ...
    const parentGridSurface = el.parentElement?.closest('.layout-grid-surface') || null;
    const unexpectedInteractionLocks = Array.from(el.children).filter(child => (child instanceof HTMLElement &&
        child.classList.contains('canvas-item') &&
        child.getAttribute('gs-no-move') === 'true' &&
        child.getAttribute('gs-locked') !== 'true' &&
        !child.classList.contains('inactive-layer'))).length;
...
function widgetPlacementFeedback() {
    const ownerSnapshot = window.blogposterDesignerCommands?.snapshot?.();
    ...
    return Array.from(document.querySelectorAll('.canvas-item')).map((el, index) => {
        ...
        const saved = savedWidgets.find((item) => String(item.id) === String(el.dataset.instanceId || el.id));
        return {
...
function snapGuideFeedback() {
    const grid = document.querySelector('.layout-section.layout-section--active.layout-grid-surface') || document.querySelector('.layout-grid-surface');
...
function designerFeedbackWarnings(visual, layoutNodes, widgets) {
    ...
    const freeAncestors = new Set();
    for (const host of contentHosts) {
        let parent = nodesById.get(host.parentId);
        const visited = new Set();
        while (parent && !visited.has(parent.id)) {
            visited.add(parent.id);
            if (parent.mode === 'free' && parent.role !== 'layout-root')
                freeAncestors.add(parent.id);
            parent = nodesById.get(parent.parentId);
        }
    }
    if (freeAncestors.size)
        warnings.push({
    ...
    const sectionGridCount = document.querySelectorAll('#layoutRoot > .layout-section[data-section-id].layout-grid-surface').length;
    const expectedGridNodes = Array.from(document.querySelectorAll('#layoutRoot > .layout-section[data-section-id], #layoutRoot > .layout-section[data-section-id] .layout-container'));
    const gridSurfaces = Array.from(document.querySelectorAll('#layoutRoot .layout-grid-surface[data-grid-surface-id]'));
    const gridSurfaceIds = new Set(gridSurfaces.map(surface => (surface.dataset.gridSurfaceId || surface.dataset.sectionId || surface.dataset.nodeId || '')));
...
function motionTimelineFeedbackState() {
    const timeline = document.querySelector('.scene-timeline');
    const progress = timeline?.querySelector('.scene-timeline-range');
    return {
...
function widgetLibraryFeedbackState() {
    const rail = document.querySelector('.scene-widget-scroll');
    const popover = document.querySelector('[data-sidebar-panel="insert"]');
    const categories = Array.from(document.querySelectorAll('[data-insert-group]'));
    ...
    const visiblePresets = popover && !popover.hidden
        ? Array.from(popover.querySelectorAll('[data-insert-preset]'))
            .filter(button => !button.closest('[data-insert-group-panel]')?.hidden)
...
function sectionNodes() {
    const sections = Array.from(document.querySelectorAll('#layoutRoot > .layout-section[data-section-id]'));
    ...
    return sections.map((section, index) => {
        ...
        const sceneElements = Array.from(workspace?.querySelectorAll(':scope > .canvas-item') || []);
        ...
        return {
            ...
            meta: {
                ...
                ownsCanvasGrid: Boolean(workspace?.__grid),
...
function availableControls() {
    ...
    document.querySelectorAll('[data-insert-preset]').forEach(button => {
        const group = button.closest('[data-insert-group-panel]');
        controls.push({
...
async function captureStageVisual(reason) {
    const stageEl = document.getElementById('layoutRoot');
    if (!stageEl) {
...
async function handleColorLibraryCommand(action, command) {
    ...
    if (action === 'colorLibrary.createScheme') {
        ...
        if (!name)
            return { handled: false, reason: 'missing-color-scheme-name' };
        return {
            handled: true,
    ...
    if (action === 'colorLibrary.updateScheme') {
        const name = String(commandParam(command, 'name') || '').trim();
        if (!schemeId || !name)
            return { handled: false, reason: 'missing-color-scheme-id-or-name' };
        return { handled: true, scheme: await renameColorScheme(schemeId, name) };
    }
    if (action === 'colorLibrary.activateScheme') {
        if (!schemeId)
            return { handled: false, reason: 'missing-color-scheme-id' };
    ...
    if (action === 'colorLibrary.create') {
        ...
        if (!name || !value)
            return { handled: false, reason: 'missing-color-name-or-value' };
        return {
            handled: true,
    ...
    if (action === 'colorLibrary.update') {
        ...
        if (!id)
            return { handled: false, reason: 'missing-color-id' };
...
async function handleFontPackagesCommand(action, command) {
    const id = String(commandParam(command, 'id') || command.target || '').trim();
    ...
    if (action === 'fontPackages.create') {
        ...
        const copyFromId = String(commandParam(command, 'copyFromId') || '').trim();
        if (!name)
            return { handled: false, reason: 'missing-font-package-name' };
        return {
            handled: true,
    ...
    if (!id)
        return { handled: false, reason: 'missing-font-package-id' };
    ...
    if (action === 'fontPackages.updateRole') {
        const role = String(commandParam(command, 'role') || '').trim();
    ...
    if (action === 'fontPackages.resetRole') {
        const role = String(commandParam(command, 'role') || '').trim();
        if (!role)
            return { handled: false, reason: 'missing-font-package-role' };
...
async function handleSitePresetsCommand(action, command) {
    ...
    const id = String(commandParam(command, 'id') || command.target || '').trim();
    if (!id)
        return { handled: false, reason: 'missing-site-preset-id' };
    if (action === 'sitePresets.export')
        return { handled: true, encoding: 'base64-utf8', jsonParts: sitePresetJsonParts(exportSitePresetJson(id)) };
...
async function dispatchDesignerAgentCommand(command) {
    ...
    if (action.startsWith('colorLibrary.'))
        return handleColorLibraryCommand(action, command);
    if (action.startsWith('fontPackages.'))
        return handleFontPackagesCommand(action, command);
    if (action.startsWith('sitePresets.'))
        return handleSitePresetsCommand(action, command);
