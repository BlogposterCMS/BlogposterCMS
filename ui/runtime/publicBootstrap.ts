...
export function readPublicBootstrap(): PublicBootstrap | null {
  ...
  if (value.version === 2) {
    ...
    const attachments = value.envelope.attachments!.map(attachment => {
      const descriptor = attachment.descriptor as { htmlFromInitialResponse?: boolean; inline?: Record<string, unknown> } | undefined;
      if (attachment.type !== 'html' || descriptor?.htmlFromInitialResponse !== true) return attachment;
      return {
        ...attachment,
