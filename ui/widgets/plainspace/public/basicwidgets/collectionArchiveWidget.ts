...
function collectionArchiveStyle(): HTMLStyleElement {
...
function parseMeta(page: PageRecord): LooseRecord {
  if (!page.meta) return {};
  if (typeof page.meta === 'object' && !Array.isArray(page.meta)) return page.meta;
  if (typeof page.meta !== 'string') return {};
  try {
...
function normalizePageItem(page: PageRecord): ArchiveItem | null {
  ...
  const image = normalizeMediaUrl(firstString(
    page.image,
    page.imageUrl,
    page.featuredImage,
    page.featured_image,
...
function toPages(value: unknown): PageRecord[] {
  if (Array.isArray(value)) return value.filter((item): item is PageRecord => Boolean(item) && typeof item === 'object');
  if (value && typeof value === 'object' && Array.isArray((value as { data?: unknown[] }).data)) {
    return ((value as { data?: unknown[] }).data || []).filter((item): item is PageRecord => Boolean(item) && typeof item === 'object');
  }
  return [];
}

function styleSourceIdFor(ctx: PublicWidgetContext): string {
  const raw = String(ctx.id || ctx.widgetId || 'default').replace(/[^a-z0-9_-]+/gi, '-').replace(/^-+|-+$/g, '');
  return `collection-card-template-${raw || 'default'}`;
}

function resolveEmit(ctx: PublicWidgetContext): PublicWidgetContext['emit'] {
  return typeof ctx.emit === 'function'
    ? ctx.emit
    : (typeof window !== 'undefined' && typeof window.meltdownEmit === 'function' ? window.meltdownEmit : undefined);
}

async function loadArchiveItems(settings: LooseRecord, ctx: PublicWidgetContext): Promise<ArchiveItem[]> {
  const manualItems = readArray(settings, ['items', 'pages']);
  if (manualItems.length) {
    return toPages(manualItems).map(normalizePageItem).filter((item): item is ArchiveItem => Boolean(item));
  }
  ...
  return toPages(response).map(normalizePageItem).filter((item): item is ArchiveItem => Boolean(item));
}

function renderArchiveCard(item: ArchiveItem, index: number, buttonLabel: string, styleSourceId: string): HTMLElement {
  ...
  if (index === 0) card.id = styleSourceId;
  if (index > 0) card.dataset.styleSourceId = styleSourceId;

  const media = document.createElement(item.href ? 'a' : 'span');
  media.className = 'bp-collection-archive__media';
  if (item.href) (media as HTMLAnchorElement).href = item.href;
  if (item.image) {
