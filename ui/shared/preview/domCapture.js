import { firstSectionCapture } from './sectionCapture.js';
let _toPng;
...
export async function capturePreview(gridEl, options = {}) {
  if (!gridEl) return '';
  // Section thumbnails use computed styles plus explicitly fetched font CSS;
  // one inaccessible stylesheet must not disable the entire image capture.
  if (!options.firstSection && !canReadStyleSheetsForPreview()) {
    return options.structuralFallback ? structuralPreviewDataUrl(gridEl, options) : '';
  }
  try {
    const toPng = await loadToPng();
    const fontEmbedCss = await getFontEmbedCss();
    const capture = options.firstSection ? firstSectionCapture(gridEl) : { target: gridEl, options: resolvePreviewOptions(gridEl, options) };
    return await toPng(capture.target, {
      cacheBust: true,
