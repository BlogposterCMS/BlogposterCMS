...
type StorageConfig = {
  ...
  publicBaseUrl?: string; forcePathStyle?: boolean; credentialsConfigured?: boolean; canConfigure?: boolean;
};
type Options = { request: typeof fetch; csrfToken?: string | null; listDownloads?: () => Promise<any[]> };

/** Uses the existing form controls and media catalog; password values are write-only. */
export function createMediaStoragePanel({ request, csrfToken, listDownloads }: Options): HTMLElement {
  ...
  for (const [value, label] of [['local', 'Local server'], ['alibaba-oss', 'Alibaba OSS'], ['s3', 'AWS S3 / S3-compatible storage']]) {
    ...
    option.value = value!; option.textContent = label!; provider.append(option);
  }
  ...
  pathStyle.type = 'checkbox'; pathStyle.name = 'forcePathStyle';
  ...
  function button(label: string): HTMLButtonElement {
    const item = document.createElement('button'); item.type = 'button'; item.className = 'btn btn-secondary'; item.textContent = label; return item;
  }
  const save = button('Save storage'); save.type = 'submit';
  const test = button('Test saved connection');
  controls.append(createFormActions(save, test));
  form.append(controls);
  const uploads = document.createElement('fieldset'); uploads.disabled = true;
  for (const group of [controls, uploads]) {
    group.style.minWidth = '0'; group.style.border = '0'; group.style.padding = '0'; group.style.margin = '0';
  }
  const file = document.createElement('input'); file.type = 'file'; file.name = 'downloadFile';
  ...
  function fill(config: StorageConfig) {
    provider.value = config.provider;
    for (const name of Object.keys(fields)) fields[name]!.value = name.startsWith('accessKey') ? '' : String(config[name as keyof StorageConfig] || '');
  ...
  async function api(suffix = '', options: RequestInit = {}): Promise<any> {
    ...
    if (!response.ok || result.error) throw new Error(result.error || 'MEDIA_STORAGE_REQUEST_FAILED');
    return result;
  }
  function showError(error: unknown) { status.textContent = error instanceof Error ? error.message : 'MEDIA_STORAGE_REQUEST_FAILED'; }
  function appendLink(target: HTMLElement, url: string, label: string) {
    // Persisted metadata is untrusted even though Media Manager also normalizes URLs.
    const parsed = new URL(url, window.location.origin);
    if (!['http:', 'https:'].includes(parsed.protocol)) return;
    const link = document.createElement('a'); link.href = parsed.href; link.textContent = label; link.target = '_blank'; link.rel = 'noopener noreferrer'; target.append(link);
  }
  async function loadDownloads() {
    if (!listDownloads) return;
    refresh.disabled = true;
    try {
      const rows = await listDownloads(); recent.replaceChildren();
      for (const row of rows) {
        if (!row.url) continue;
        const item = document.createElement('li');
        appendLink(item, row.url, row.fileName || row.file_name || row.url); recent.append(item);
      }
  ...
  async function load() {
    setBusy(true);
    try {
      const config = await api(); canConfigure = config.canConfigure === true; ready = true; fill(config);
  ...
  publish.addEventListener('click', async () => {
    if (!file.files?.[0]) { status.textContent = 'Choose a download file.'; return; }
    setBusy(true); status.textContent = 'Uploading and publishing…'; linkArea.replaceChildren();
    try {
      ...
      const checksum = document.createElement('p'); checksum.textContent = `SHA-256: ${result.checksum}`; linkArea.append(checksum);
      status.textContent = 'Published. Open the download link to verify public access.'; file.value = '';
      await loadDownloads();
    } catch (error) { showError(error); }
    finally { setBusy(false); }
  });
  ...
  panel.addEventListener('toggle', () => { if (panel.open && !ready) { void load(); void loadDownloads(); } });
  updateProvider();
  return panel;
}
