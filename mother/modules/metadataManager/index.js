...
module.exports = {
  ...
  async healthCheck({ motherEmitter, jwt }) {
    const rows = await metadataDbSelect(motherEmitter, jwt, 'LIST_META_FIELDS', { limit: 1, offset: 0 });
    if (!Array.isArray(rows)) throw new Error('CORE_MODULE_METADATA_NOT_READY');
  },
