...
function buildWorkspaceActions(nav: HTMLElement, pages: AdminPage[], adminBase: string, workspaceSlug: string): void {
  const fragment = document.createDocumentFragment();
...
async function renderWorkspaceNav(): Promise<void> {
  ...
  if (actionNav) {
    ...
    const store = createExtensionStoreButton(); store.id = 'extension-store';
    store.onclick = () => openExtensionUpload(async (data, name, progress, kind) => {
      ...
      if (kind === 'widget') return (await import('../../shared/module-access/widgetPackageControls.js')).installWidgetArchive(data, name, progress);
