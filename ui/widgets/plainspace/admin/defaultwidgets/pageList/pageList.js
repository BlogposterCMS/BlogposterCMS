...
export function getAllowedParentPages(pages, page) {
    ...
    return pages.filter(candidate => {
        ...
        if (candidate.status === 'deleted' && candidateId !== normalizePageId(page.parent_id))
            return false;
        return true;
    });
}
...
export function filterPages(pages, filter) {
    switch (filter) {
        ...
        case 'Collections':
            return deriveCollections(pages).map(collection => collection.page);
        default:
...
export function renderPageList(el, pages, options = {}) {
    ...
    let query = '';
    let creating = false;
    let creationNext = null;
    let createdForEditor = null;
    const slotChecks = new Map();
    const slotWarnings = new Set();
    const slotDesigns = new Map();
    function refreshContentChecks() {
        slotChecks.clear();
        slotWarnings.clear();
        slotDesigns.clear();
        for (const row of tree.querySelectorAll('[data-page-id]')) {
            const host = row.querySelector('.page-manager__identity-line, .page-manager__form > label');
            if (host) {
                host.querySelector('.page-manager__slot-warning')?.remove();
                checkContentSlot(row.dataset.pageId || '', host);
            }
        }
    }
    function checkContentSlot(id, host) {
        if (!id || mainDesignLoading)
            return;
        let check = slotChecks.get(id);
        if (!check) {
            const loadDesign = (designId) => {
                if (!slotDesigns.has(designId))
                    slotDesigns.set(designId, emitRuntimeAdmin(window.meltdownEmit, window.ADMIN_TOKEN, 'designer', 'get', { id: designId, includeDrafts: true }));
        ...
        void check.then(missing => {
            if (slotChecks.get(id) === check && missing)
                slotWarnings.add(id);
            if (slotChecks.get(id) !== check || !missing || !host.isConnected || host.querySelector('.page-manager__slot-warning'))
                return;
            const warning = document.createElement('span');
            warning.className = 'page-manager__slot-warning';
            warning.tabIndex = 0;
            ...
            warning.innerHTML = icon('triangle-alert');
            host.append(warning);
    ...
    const root = requireElement(el, '.page-manager');
    registerWorkspaceChanges(root, { isDirty: () => dirty, isBusy: () => busy });
    const tree = requireElement(root, '.page-manager__tree');
    const details = requireElement(root, '.page-manager__details');
    ...
    function message(text, error = false) {
        ...
        const dialogStatus = creationBody?.querySelector('[data-creation-status]');
        if (dialogStatus)
    ...
    function setBusy(value) {
        busy = value;
        if (creationBody)
            details.inert = value || refreshPending;
        root.setAttribute('aria-busy', String(value));
        root.querySelectorAll('.page-manager__header, .page-manager__layout').forEach(node => {
    ...
    async function run(code, action, allowRefresh = false, propagate = false) {
        if (busy || (refreshPending && !allowRefresh)) {
            if (propagate)
                throw new Error('PAGE_MANAGER_RECOVERY_REQUIRED: Refresh the page list before continuing.');
            return;
        }
        setBusy(true);
        try {
            ...
            if (!creating && creationBody) {
                creationBody.closest('.bp-dialog')?.querySelector('.bp-dialog__actions [data-action="cancel"]')?.click();
            }
    ...
    async function afterWrite(success) {
        dirty = false;
        try {
            await refresh();
            message(success);
    ...
    async function add(parent) {
        ...
        createButton.addEventListener('click', () => { creationNext = creatingCollection ? null : 'article'; details.querySelector('form')?.requestSubmit(); });
        const footer = document.createElement('div');
        footer.className = 'page-manager__create-actions';
        if (!creatingCollection) {
            const design = document.createElement('button');
            design.type = 'button';
            design.className = 'button secondary';
            design.textContent = 'Create design';
            design.addEventListener('click', () => { creationNext = 'design'; details.querySelector('form')?.requestSubmit(); });
        ...
        void bpDialog.open({ title: creatingCollection ? 'Add collection' : 'Add page', body,
            ...
            const id = createdForEditor;
            const next = creationNext;
            createdForEditor = null;
            creationNext = null;
            if (!id || !next)
                return;
            if (next === 'article')
                await openArticle(id, refresh);
            else {
    ...
    function renderTree() {
        ...
        <div class="page-manager__select" role="button" tabindex="0" aria-pressed="${!creating && selectedId === id}">
          ...
          <span class="page-manager__identity"><span class="page-manager__identity-line"><strong class="page-name">${escapeHtml(row.page.title || 'Untitled')}</strong><button type="button" class="page-manager__status" aria-label="Change page status" aria-haspopup="menu" data-status="${escapeHtml(row.page.status || 'draft')}">${row.page.is_start ? 'Home · ' : ''}${escapeHtml(row.page.status || 'draft')}${icon('chevron-down')}</button></span><span>/${escapeHtml(row.page.slug || '')}</span></span>
            ...
            element.querySelector('button.page-manager__toggle')?.addEventListener('click', () => void run('PAGE_MANAGER_EXPAND_FAILED', () => toggleBranch(row.rowId)));
            checkContentSlot(id || '', element.querySelector('.page-manager__identity-line'));
            element.querySelector('.page-manager__status')?.addEventListener('click', async () => {
                if (busy)
                    return;
                await select(id);
                if (selectedId === id && !creating)
                    details.querySelector('.page-manager__status')?.click();
            });
            const activateRow = () => void run('PAGE_MANAGER_SELECT_FAILED', async () => {
                await select(id);
                if (selectedId === id && !creating && hasVisibleChildren && !showMatches)
                    await toggleBranch(row.rowId);
            });
            ...
            element.querySelector('.page-manager__select')?.addEventListener('keydown', event => {
                const key = event;
                if (key.target !== key.currentTarget || !['Enter', ' '].includes(key.key))
                    return;
                key.preventDefault();
                activateRow();
            });
            element.addEventListener('click', event => {
                ...
                if (!(event.target instanceof Element) || event.target.closest('button, a, input, select, label, form'))
                    return;
                activateRow();
            });
            if (id && (creating || selectedId !== id)) {
                ...
                const invoke = async (action) => {
                    ...
                    await select(id);
                    if (!creating && selectedId === id)
                        details.querySelector(`[data-action="${action}"]`)?.click();
                };
    ...
    async function toggleBranch(rowId) {
        const descendants = () => {
            const all = [...tree.querySelectorAll('[data-page-row-id]')];
        ...
        if (!window.matchMedia?.('(prefers-reduced-motion: reduce)').matches) {
            await Promise.all(descendants().map(async (row) => {
                ...
                try {
                    await row.animate(opening ? [collapsed, full] : [full, collapsed], { duration: 180, easing: 'ease-out' }).finished;
                }
    ...
    function mountDetails() {
        const row = [...tree.querySelectorAll('[data-page-id]')]
        ...
        if (inline)
            ...
            root.querySelector('.page-manager__layout').append(details);
    }
    function renderDetails() {
      ...
      ${creating ? '<button type="button" class="button text sm" data-action="cancel">Cancel</button>' : `<div class="page-manager__secondary"><button type="button" class="button text sm" data-action="child">${icon('plus')} Add subpage</button><button type="button" class="button text sm" data-action="share" ${page.status !== 'published' ? 'disabled' : ''}>${icon('link')} Copy link</button><button type="button" class="button text sm" data-action="home" ${page.is_start || page.status !== 'published' ? 'disabled' : ''}>${icon('house')} ${page.is_start ? 'Current home page' : 'Set as home page'}</button><button type="button" class="button text sm page-manager__delete" data-action="delete">${icon('trash-2')} Delete page</button></div>`}`;
        ...
        if (!creating) {
            ...
            requireElement(inlineForm, '[name="title"]').setAttribute('aria-label', 'Title');
            requireElement(inlineForm, '[name="slug"]').setAttribute('aria-label', 'Page address');
            ...
            state.addEventListener('click', () => {
                ...
                for (const value of ['draft', 'published']) {
                    ...
                    item.addEventListener('click', () => {
                        ...
                        const field = requireElement(inlineForm, '[name="status"]');
                        field.value = value;
            ...
            for (const button of details.querySelectorAll('[data-action="view"], .page-manager__secondary button')) {
            ...
            const save = requireElement(inlineForm, '[type="submit"]');
            ...
            inlineForm.addEventListener('keydown', event => {
                if (!(event.target instanceof HTMLInputElement) || event.isComposing)
                    return;
                if (event.key === 'Escape') {
                    event.preventDefault();
                    inlineForm.reset();
                    dirty = false;
                    requireElement(inlineForm, '.page-manager__save-state').textContent = 'Saved';
                    requireElement(inlineForm, '[type="submit"]').disabled = true;
                    event.target.blur();
                }
        ...
        form.addEventListener('input', event => {
            ...
            if (creating && target.name === 'title' && !slugEdited) {
                ...
                requireElement(form, '[name="slug"]').value = prefix + sanitizeSlug(target.value.replace(/\s+/g, '-'));
            }
            markDirty();
        });
        ...
        function markDirty() {
            dirty = true;
            requireElement(form, '.page-manager__save-state').textContent = 'Unsaved changes';
            requireElement(form, '[type="submit"]').disabled = false;
        }
        ...
        saveDraft = async () => {
            ...
            if (!title || !slug)
                throw new Error('PAGE_MANAGER_REQUIRED: Enter a title and page address.');
            if (slug !== rawSlug.replace(/^\/+/, ''))
                throw new Error('PAGE_MANAGER_SLUG_INVALID: Use lowercase letters, numbers, hyphens and slashes in the page address.');
            const parent_id = normalizePageId(data.get('parent_id'));
            const parentError = getParentValidationError(pages, page, parent_id);
            if (parentError)
                throw new Error(`PAGE_MANAGER_PARENT_INVALID: ${parentError}`);
        ...
        bind('back', async () => {
            focusAfterAction = root.querySelector('[type="search"]');
        });
        ...
        bind('delete', async () => {
            if (!(await bpDialog.confirm(`Delete “${page.title || 'Untitled'}”?${dirty ? ' Unsaved details will also be discarded.' : ''}`, { title: 'Delete page', confirmLabel: 'Delete page' })))
                return;
    ...
    const importButton = requireElement(root, '[data-action="import-example"]');
    const menuButton = requireElement(root, '[data-action="page-menu"]');
        ...
        actions: [
            ...
            { action: 'pages.openArticle', label: 'Open article editor', params: [{ name: 'id', type: 'string', required: true }], run: async (p) => {
                    ...
                    if (!pages.some(page => normalizePageId(page.id) === id))
                        throw new Error('PAGE_MANAGER_PAGE_NOT_FOUND');
                    const page = await readArticlePage(id);
                    if (!['empty', 'article'].includes(pageContentKind(page)))
                        throw new Error('ARTICLE_FORMAT_CONFLICT');
            ...
            { action: 'pages.createDraft', label: 'Start a page draft', params: [{ name: 'parentId', type: 'string', required: false }], run: async (p) => {
                    ...
                    if (parent && !pages.some(page => normalizePageId(page.id) === parent && page.status !== 'deleted'))
                        throw new Error('PAGE_MANAGER_PARENT_INVALID');
                    await add(parent);
                } },
