...
function stagePackage(buffer, options = {}) {
  ...
  try {
    ...
    return { staged, dir, widget, info: { ...widget, version: info.version, developer: String(info.developer || ''), description: String(info.description || '') }, requestedAccess, reviewedHash, warnings: report.warnings || [] };
  } catch (err) { fs.rmSync(staged, { recursive: true, force: true }); throw err; }
}

function inspectionOf(prepared, config) {
  const policy = projectWidgetPolicy(config, prepared.widget.widgetId);
...
async function inspectWidgetPackage(emitter, jwt, buffer, options = {}) {
  ...
  try { return inspectionOf(prepared, await readPolicies(emitter, jwt)); }
...
async function installWidgetPackage(emitter, jwt, buffer, params, options = {}) {
  ...
  busy = true;
  let prepared, target, created = false, oldPolicy, policyWritten = false;
  try {
    ...
    if (fs.existsSync(target)) throw packageError('WIDGET_PACKAGE_EXISTS', 'This widget is already installed.');
    ...
    policyWritten = true;
    fs.renameSync(prepared.dir, target);
    created = true;
    ...
    return { installed: true, widgetId: id };
  } catch (err) {
    if (created) {
      fs.rmSync(target, { recursive: true, force: true });
      require('../../security/extensionIntegrity').removeExtensionReceipt(path.dirname(options.widgetsRoot || ROOT), 'widgets', prepared.widget.widgetId);
    }
    if (policyWritten) {
      try {
        const config = await readPolicies(emitter, jwt);
