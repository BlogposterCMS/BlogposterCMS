// Shared presentation primitives keep previews and widgets on one implementation.
...
function recordAt(source: LooseRecord, key: string): LooseRecord {
...
export function readNumber(source: LooseRecord, keys: string[], fallback = 0): number {
  for (const key of keys) {
    const value = source[key];
...
export function readBoolean(source: LooseRecord, keys: string[], fallback = false): boolean {
  for (const key of keys) {
    const value = source[key];
...
export function readArray(source: LooseRecord, keys: string[]): unknown[] {
  for (const key of keys) {
    const value = source[key];
...
function serializeSameOrigin(url: URL): string {
  return `${url.pathname}${url.search}${url.hash}`;
}

function normalizeUrl(value: unknown, protocols: string[]): string {
  const raw = normalizeString(value);
  if (!raw || raw.startsWith('//') || /[\u0000-\u001f\s]/u.test(raw)) return '';
...
export function normalizeAspectRatio(value: unknown): string {
  ...
  if (raw === 'video') return '16 / 9';
  if (raw === 'portrait') return '4 / 5';
  return /^\d{1,3}\s*\/\s*\d{1,3}$/u.test(raw) ? raw.replace(/\s+/gu, ' ') : '';
}

export function renderWidgetMessage(
...
export function sharedStyle(): HTMLStyleElement {
...
export function normalizeMediaItems(items: unknown[]): MediaItem[] {
  return items
...
export function normalizeNavigationItems(items: unknown[]): NavigationItem[] {
  return items
    .map((item): NavigationItem | null => {
      ...
      const childSource = Array.isArray(item.children)
        ...
        : Array.isArray(item.items)
          ...
          : [];
      return {
        id: item.id as string | number | null | undefined,
        parentId: (item.parentId ?? item.parent_id) as string | number | null | undefined,
...
export function currentPathSegments(): string[] {
