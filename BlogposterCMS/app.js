...
app.get('/admin/app/:appName/:pageId?', csrfProtection, async (req, res) => {
  ...
  const entry = rawEntry.includes('/') || rawEntry.endsWith('.js')
    ...
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="csrf-token" content="${req.csrfToken()}"><title>${titleSafe}</title><script src="/assets/js/meltdownEmitter.js"></script><script src="/assets/js/icons.js"></script></head><body><div id="sidebar"></div><div id="content"></div><script type="module" src="/${entry}"></script></body></html>`;
  return res.send(html);
});
