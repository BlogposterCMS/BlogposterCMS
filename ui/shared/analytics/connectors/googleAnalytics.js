...
export const googleAnalytics = {
    ...
    start(config, nonce) {
        if (started || !config.googleEnabled || !/^G-[A-Z0-9]{4,20}$/.test(config.measurementId))
            return;
        started = true;
    ...
    stop(config) {
        if (!/^G-[A-Z0-9]{4,20}$/.test(config.measurementId))
            return;
