...
import { createLoader, createProgress, setButtonLoading } from '/ui/shared/feedback/loading.js';
import { createFormActions, createFormChoice, createFormField, createFormSwitch } from '/ui/shared/forms/formField.js';
import { createTabSystem } from '/ui/shared/navigation/tabs.js';
import { bpPopover } from '/ui/shared/overlays/popover.js';
...
function createButton(label, variant = 'ghost', onClick) {
...
function createSection(id, title, description, size = 'default') {
    const section = createElement('section', `ui-kit__section${size === 'wide' ? ' ui-kit__section--wide' : ''}`);
    section.id = `ui-kit-${id}`;
    section.dataset.uiKitSection = id;
    const header = createElement('header', 'ui-kit__section-header');
    header.append(createElement('h2', '', title), createElement('p', '', description));
    const body = createElement('div', 'ui-kit__stack');
    section.append(header, body);
    return { section, body };
}
function createSpecimen(caption) {
...
function createFoundationsSection() {
    const { section, body } = createSection('foundations', 'Foundations & icons', 'The same Studio tokens and bundled Lucide SVG assets used by the admin shell.', 'wide');
...
function createButtonsSection() {
    const { section, body } = createSection('buttons', 'Buttons', 'Variants, sizes, icon actions, disabled and asynchronous states.');
    const specimen = createSpecimen('.button · .icon-button · .is-loading');
    const variants = createElement('div', 'ui-kit__row');
    variants.append(createButton('Primary', 'primary'), createButton('Secondary', 'secondary'), createButton('Ghost', 'ghost'), createButton('Outline', 'outline'), createButton('Text action', 'text'), createButton('Delete', 'danger'));
    ...
    states.append(createButton('Small', 'ghost sm'), createButton('Large', 'ghost lg'), disabled, asyncButton, createIconButton('pencil-line', 'Edit'), createIconButton('trash-2', 'Delete'));
    specimen.content.append(variants, states);
    body.appendChild(specimen.root);
    return section;
}
function createFieldsSection() {
    const { section, body } = createSection('fields', 'Form controls', 'Labels, help, validation, text areas, choices and switches share one field contract.', 'wide');
    ...
    grid.append(createFormField('Text input', name, { hint: 'Use a concise, descriptive value.' }), createFormField('Email', email), createFormField('Password', password), createFormField('Search', search), createFormField('Number', number), createFormField('Read-only value', readonly), createFormField('Invalid URL', invalid, { error: 'BP_FORM_URL_INVALID: Enter a complete URL.' }), createFormField('Textarea', textarea, {
    ...
    choices.append(createFormChoice('Checkbox', checkbox), createFormChoice('Comfortable', radioA), createFormChoice('Compact', radioB), createFormSwitch('Live preview', switchInput));
    form.append(grid, choices, createFormActions(createButton('Save form', 'primary', () => bpToast.success('Form example submitted.')), createButton('Cancel', 'ghost')));
    specimen.content.appendChild(form);
    body.appendChild(specimen.root);
    return section;
}
function createPickerSection() {
    const { section, body } = createSection('pickers', 'Dropdowns & color picker', 'Single-selects are upgraded to the shared keyboard-accessible control; no browser-native dropdown is shown.', 'wide');
...
function createPopoverMenu(onSelect) {
...
function createOverlaysSection() {
    const { section, body } = createSection('overlays', 'Dialogs, prompts & popover', 'Every example uses the shared overlay APIs with focus restoration, Escape and safe text rendering.');
    const specimen = createSpecimen('bpDialog · bpPopover');
    const actions = createElement('div', 'ui-kit__row');
    actions.append(createButton('Alert', 'ghost', () => {
        ...
        void bpDialog.prompt('Choose a page title.', 'New article', {
            ...
            if (value)
                bpToast.success(`Title set to “${value}”.`);
        });
        ...
        customBody.append(createIcon('triangle-alert'), createElement('p', '', 'This action only demonstrates the destructive modal variant.'));
        void bpDialog.open({
...
function createAlert(tone, title, message) {
    const iconNames = {
        info: 'info',
...
function createFeedbackSection() {
    const { section, body } = createSection('feedback', 'Toasts, alerts & status', 'Transient feedback and persistent inline messages use the same semantic tones.');
    const toastSpecimen = createSpecimen('bpToast.info/success/warning/error');
    const toastActions = createElement('div', 'ui-kit__row');
    toastActions.append(createButton('Info toast', 'ghost', () => bpToast.info('A neutral update is available.', { title: 'Information' })), createButton('Success toast', 'ghost', () => bpToast.success('Page saved successfully.', { title: 'Saved' })), createButton('Warning toast', 'ghost', () => bpToast.warning('Two fields still need review.', { title: 'Review needed' })), createButton('Error toast', 'ghost', () => bpToast.error('The request could not be completed.', { title: 'Save failed' })), createButton('Toast with action', 'secondary', () => bpToast.info('The draft was moved.', {
        title: 'Draft moved',
        duration: 7000,
        action: {
            label: 'Undo',
    ...
    alerts.append(createAlert('info', 'Information', 'Use for helpful context that does not block the task.'), createAlert('success', 'Complete', 'The operation finished and no further action is required.'), createAlert('warning', 'Check this', 'The operation can continue after a quick review.'), createAlert('error', 'Could not save', 'BP_UI_KIT_DEMO_ERROR: Try the action again.'));
    body.append(toastSpecimen.root, alerts);
    return section;
}
function createLoadingSection() {
    const { section, body } = createSection('loading', 'Loading & progress', 'Inline, spinner, skeleton, overlay, progress and button-loading states are reusable across admin modules.', 'wide');
    ...
    small.append(createLoader({ variant: 'inline', label: 'Loading settings…' }), createLoader({ variant: 'spinner', label: 'Loading' }));
...
function createDataSection() {
    const { section, body } = createSection('data', 'Tabs, badges, cards & table', 'Reusable navigation and data-display patterns complete the everyday admin surface.', 'wide');
    ...
    empty.append(createIcon('inbox'), createElement('strong', '', 'Nothing here yet'), createElement('p', '', 'Create the first item or adjust the current filters.'), createButton('Create item', 'primary'));
    body.append(badgeRow, tabSpecimen.root, tableWrap, footer, empty);
    return section;
}
function createJumpNavigation(sections) {
...
export function renderUiKitGallery(target) {
    if (!target) {
    ...
    copy.append(createElement('h1', 'page-title', 'Blogposter UI Kit'), createElement('p', 'settings-hint', 'Interactive reference for the canonical admin components and their states.'));
