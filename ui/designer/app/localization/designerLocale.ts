...
import { localeDesign, localeWidget, normalizeContentLocale, projectLocalizedDesign, readDesignLocalizations, resolveDesignLocale, saveDesignLocale, type LocaleDesign, type DesignLocalizations } from '../../../shared/localization/designLocaleModel.js';

type RecordValue = Record<string, any>;
let session: { base: LocaleDesign; localizations: DesignLocalizations; language: string; initial?: LocaleDesign; publication: RecordValue } | undefined;
let pendingLanguage = '';
let resetRequested = false;

/** This is an editor projection of the versioned Designer record, never a second persistence owner. */
export function hydrateDesignerLocale(response: RecordValue) {
  if (!response?.design) { session = undefined; return response; }
...
export function captureDesignerLocaleBaseline(layout: unknown, widgets: unknown[]) {
  if (session) session.initial = { ...localeDesign({ layout, widgets }), styles: resolveDesignLocale(session.base, session.localizations, session.language).styles };
}

export function prepareDesignerLocaleSave(payload: RecordValue) {
  if (!session) return { payload, commit() {} };
  ...
  return {
    ...
    commit() { if (session) { session.base = saved.base; session.localizations = saved.localizations; session.initial = edited; if (typeof payload.design.isDraft === 'boolean') session.publication.isDraft = payload.design.isDraft; } }
  };
}

export async function resetDesignerLocale() {
  const state = designerLocaleState();
  if (!state.inherited || !state.hasOverride) throw new Error('DESIGN_LOCALE_OVERRIDE_MISSING');
  ...
  try {
    ...
    if (!result || result.handled === false) throw new Error('DESIGN_LOCALE_RESET_FAILED');
    pendingLanguage = state.language;
    return { language: state.language, inherited: true };
  } finally { resetRequested = false; }
}
...
export function designerLocaleState() {
  ...
  const current = window.blogposterDesignerCommands?.snapshot?.() as RecordValue | undefined;
  const widgets: RecordValue[] = current?.document?.widgets?.map(localeWidget) || selected?.widgets || [];
  const content = (widget?: RecordValue) => {
  ...
  return { language, sourceLanguage, availableLanguages: availableContentLanguages(), inherited: language !== sourceLanguage, locales: [sourceLanguage, ...Object.keys(session?.localizations.variants || {})],
    ...
    widgets: widgets.map((widget: RecordValue) => ({ id: widget.id, translationStatus: language === sourceLanguage ||
      content(widget) !== content(session?.base.widgets.find(item => item.id === widget.id)) ? 'authored' : 'source-fallback' })) };
}

export async function requestDesignerLanguage(language: unknown) {
  const value = normalizeContentLocale(language, '');
  if (!value) throw new Error('DESIGN_LOCALE_CODE_INVALID: Enter a language code such as en or zh-CN.');
  const current = window.blogposterDesignerCommands?.snapshot?.() as RecordValue | undefined;
  if (current?.save?.dirty || current?.save?.busy || current?.publishing?.busy) throw new Error('DESIGN_LOCALE_SAVE_REQUIRED: Save changes before switching language.');
  await assertContentLanguage(value, designerLocaleState().locales);
  if (value !== designerLocaleState().language) {
    if (window.parent !== window) await window.meltdownEmit?.('appView.setLanguage', { language: value });
...
export function finishDesignerLanguageSwitch() {
  ...
  const language = pendingLanguage; pendingLanguage = '';
  let target: Window = window;
  try { if (window.parent !== window && window.parent.location.origin === location.origin) target = window.parent; } catch { /* Keep the owned frame when embedded across origins. */ }
  const url = new URL(target.location.href); url.searchParams.set('lang', language);
  if (url.searchParams.has('contentPageId')) url.searchParams.set('contentLang', language);
