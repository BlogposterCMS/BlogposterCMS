import { dialogApi, accessEventLabel, reviewModuleAccess, installModuleArchive } from '../../../shared/module-access/modulePackageControls.js';
export { installModuleArchive } from '../../../shared/module-access/modulePackageControls.js';
...
function moduleNameFromRecord(moduleRecord: ModuleRecord): string {
  const info = moduleInfoFromRecord(moduleRecord);
  return info.moduleName || moduleRecord.module_name || '';
}

function accessStatusLabel(access: ModuleAccessRequest, info: ModuleInfo, pendingAccess: ModuleAccessRuntimeRequest[]): string {
...
function makeEmpty(text: string): HTMLParagraphElement {
...
function appendAccessRows(
  list: HTMLElement,
  items: ModuleAccessRequest[],
  info: ModuleInfo,
  pendingAccess: ModuleAccessRuntimeRequest[]
): void {
  if (!items.length) {
...
export async function render(el: HTMLElement | null, options: { tabsHost?: HTMLElement; actionsHost?: HTMLElement } = {}): Promise<void> {
  ...
  try {
    ...
    let currentTab: 'installed' | 'system' = 'installed';
    ...
    function syncDetailPanel(): void {
      ...
      detailMount.appendChild(renderModuleDetail(
        currentTab === 'installed' ? selectedInstalled : selectedSystem,
        pendingAccess,
        currentTab === 'system'
      ));
    ...
    if (!installed.length) {
      ...
      installed.forEach(moduleRecord => {
        ...
        toggleBtn.addEventListener('click', async event => {
          event.stopPropagation();
          try {
            let approvedAccess: ModuleAccessRequest[] | undefined;
            if (!moduleRecord.is_active) {
              const infoForReview = moduleRecord.module_info || {};
              const reviewed = await reviewModuleAccess(
                `Activate ${name}`,
                'Review requested module access before activation.',
                infoForReview,
                'Activate'
              );
              if (reviewed === null) return;
...
function openUploadPopup(): void {
  openExtensionUpload(installModuleArchive);
}
