...
export function renderPageDesignPreview(host: HTMLElement, source: PagePresentation | null, designs: DesignRecord[], adminBase: string, pageId?: string | number, compact = false, onDelete?: () => void, onSaved?: () => void | Promise<void>): void {
  ...
  if (pageId != null) bindPageContentAction(edit, pageId, adminBase, onSaved);
  if (compact) {
  ...
  more.addEventListener('click', () => {
    ...
    if (onDelete) {
      ...
      remove.type = 'button'; remove.className = 'bp-popover__item';
      remove.setAttribute('role', 'menuitem'); remove.textContent = 'Delete page';
      remove.addEventListener('click', () => { popover.close(); onDelete(); });
      menu.append(remove);
    }
  });
  actions.append(edit, settings, more);
  if (compact) { host.append(actions); return; }
  if (!id) {
