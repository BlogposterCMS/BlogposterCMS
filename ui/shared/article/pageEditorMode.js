...
import { articleTitle, readArticlePage, saveContentPageDraft } from './articleData.js';
...
export function pageDesignEditorUrl(pageId, designId, language, adminBase = '/admin') {
...
export async function ensureContentDesign(base) {
    ...
    if (own?.designId)
        return own.designId;
    if (own)
        throw new Error('PAGE_LEGACY_LAYOUT_UNSUPPORTED: Change this legacy layout in page settings first.');
    ...
    if (ownPagePresentation(latest))
        throw new Error('PAGE_CONTENT_CHANGED: Design saved in the library but not attached; reopen the page.');
    await saveContentPageDraft({ ...latest, meta: pageLayoutMeta(latest, 'composed', String(saved.id), articleTitle(latest)) });
    return String(saved.id);
}
export async function useDocumentEditor(pageId, expectedDesignId, language) {
    const page = await readArticlePage(pageId, language);
    if (ownPagePresentation(page)?.designId !== expectedDesignId)
        throw new Error('PAGE_CONTENT_CHANGED: This page no longer uses the selected design.');
...
export async function reuseContentDesign(page, designId) {
    const current = await readArticlePage(page.id, page.contentLanguage);
    if (ownPagePresentation(current))
        throw new Error('PAGE_CONTENT_CHANGED: Reopen the page before changing its design.');
