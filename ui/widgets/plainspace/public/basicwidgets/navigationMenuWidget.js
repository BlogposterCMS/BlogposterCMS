...
function renderList(items, maxDepth, expanded, depth = 1) {
    ...
    items.forEach(item => {
        ...
        if (mega.enabled) {
            ...
            if (mega.layoutId)
                row.dataset.megaLayoutId = String(mega.layoutId);
        }
        ...
        if (link.target === '_blank')
            link.rel = [...new Set(`${link.rel} noopener noreferrer`.split(/\s+/u).filter(Boolean))].join(' ');
        if (isCurrentNavigationLink(item.href))
        ...
        if (item.children.length && depth < maxDepth) {
            ...
            if (mega.layoutId)
                childList.dataset.layoutId = String(mega.layoutId);
            if (!expanded) {
                ...
                row.addEventListener('keydown', event => {
                    if (event.key === 'Escape' && !childList.hidden) {
                        open(false);
                        toggle.focus();
                ...
                row.addEventListener('focusout', event => {
                    if (!expanded && event.relatedTarget instanceof Node && !row.contains(event.relatedTarget))
                        open(false);
                });
    ...
    if (!items.length) {
        try {
            ...
            if (renderRequests.get(el) !== request)
                return;
            renderWidgetMessage(el, 'BP_WIDGET_NAVIGATION_LOAD_FAILED', 'Navigation unavailable', err instanceof Error ? err.message : 'Navigation request failed.');
            return;
        }
    }
    if (renderRequests.get(el) !== request)
