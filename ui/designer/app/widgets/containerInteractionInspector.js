import { normalizeContainerInteraction } from '../../../shared/layout/containerInteractionModel.js';
const conditions = {
    always: undefined,
...
export function createContainerInteractionInspector(inspector, options) {
    ...
    function apply(patch) {
        ...
        try {
            ...
            warning.textContent = `Check the interaction settings. ${error.message}`;
            group.append(warning);
        }
    }
    function select(label, key, value, choices, change) {
        const wrapper = document.createElement('label');
        wrapper.className = 'scene-select-field';
        const caption = document.createElement('span');
        caption.textContent = label;
        const input = document.createElement('select');
        input.dataset.interactionField = key;
        input.id = `designer-container-${key}`;
        choices.forEach(([id, title]) => input.add(new Option(title, id)));
        if (value && !choices.some(([id]) => id === value))
            input.add(new Option(`Missing target: ${value}`, value));
        input.value = value;
    ...
    function sync() {
        ...
        if (!settings)
            return;
        const targets = [['', 'Select an element…'], ...options.targets().filter(target => target.id !== current.id).map(target => [target.id, target.label])];
        select('Show as', 'presentation', settings.presentation, [['normal', 'Container in page'], ['popover', 'Popover by trigger'], ['dialog', 'Modal dialog'], ['drawer', 'Side drawer']], value => apply({ presentation: value }));
        select('When this element', 'triggerId', settings.triggerId || '', targets, value => apply({ triggerId: value || undefined }));
        select('Receives', 'triggerEvent', settings.triggerEvent || 'click', [['click', 'Click'], ['input', 'Text input'], ['focus', 'Focus']], value => apply({ triggerEvent: value }));
        select('Data source', 'source', settings.source || '', [['', 'No data request'], ['sampleArticles', 'Example articles (no request)'], ['publishedArticles', 'Published articles']], value => apply({ source: (value || undefined) }));
        if (settings.source) {
        ...
        select('Visible when', 'when', selectedCondition, [['always', 'Always / while opened'], ['query', 'Search text is not empty'], ['loading', 'Data is loading'], ['ready', 'Data loaded successfully'], ['results', 'Results are not empty'], ['error', 'Data request failed'], ...(selectedCondition === 'custom' ? [['custom', 'Custom saved condition']] : [])], value => {
            if (value !== 'custom')
                apply({ when: conditions[value] });
        });
