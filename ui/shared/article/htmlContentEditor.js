...
async function openSourceDialog(pageId, onSaved, options) {
    ...
    source.append(label, html, css);
    body.append(cssToggle, source);
    ...
    function update() {
        ...
        for (const [control, value] of [[html, initial.html], [css, initial.css], [header.title, initial.title]])
            control.classList.toggle('article-editor__source-language', fallback && control.value === value);
        language.button.disabled = busy;
        save.disabled = busy || !dirty;
        discard.disabled = header.studio.disabled = header.title.disabled = busy;
        source.inert = busy;
    }
    ...
    async function persist() {
        ...
        update();
        try {
            page = await saveHtmlContent(page, html.value, css.value, header.title.value);
            header.title.value = articleTitle(page);
            saved = true;
            initial.html = html.value;
            initial.css = css.value;
