...
import { validateView } from '/ui/shared/widget-ui/model.js';
...
export function createWidgetUi(host, options = {}) {
    ...
    function send(record, event, original, control) {
        ...
        const input = (control || record.element);
        ...
        options.dispatch?.({ action, event, value: value.slice(0, 16384), checked: input.checked === true,
            key: original?.key, isComposing: record.composing || original?.isComposing, inputRevision: record.inputRevision });
    }
    function bind(record) {
        const element = record.element;
        const on = (name, handler) => element.addEventListener(name, handler, { signal: record.events.signal });
        on('compositionstart', () => { record.composing = true; });
        on('compositionend', event => { record.composing = false; send(record, 'input', event); });
        for (const name of ['click', 'input', 'change', 'focus', 'blur'])
            on(name, event => {
                if (record.composing)
                    return;
                if (event.target !== element && name !== 'click' && !['composer', 'kit'].includes(record.node.tag))
                    return;
                const action = record.node.events?.[name] || record.node.action;
                if (!action)
                    return;
                event.stopPropagation();
                const control = record.node.tag === 'kit' ? event.composedPath().find(item => item instanceof HTMLInputElement || item instanceof HTMLTextAreaElement || item instanceof HTMLSelectElement) : undefined;
                send(record, name, event, control);
            });
        on('keydown', event => {
            const key = event;
            if (record.composing || key.isComposing || key.keyCode === 229)
                return;
            if (key.key === 'Enter' && !key.shiftKey && record.node.events?.submit) {
                key.preventDefault();
                send(record, 'submit', key);
            }
            else if (record.node.events?.keydown && ['ArrowDown', 'ArrowUp', 'Escape'].includes(key.key)) {
                key.preventDefault();
                send(record, 'keydown', key);
            }
        });
        on('close', () => send(record, 'close'));
    }
    function patch(node, parent, path) {
        const identity = node.key || path;
        currentKeys.add(identity);
        let record = records.get(identity);
        if (record && record.node.tag !== node.tag) {
            record.closing = true;
            record.events.abort();
            record.measurement?.disconnect();
        ...
        if (!record) {
            ...
            record.element.dataset.uiKey = identity;
            bind(record);
        }
        record.node = node;
        const element = record.element;
        ...
        if (!['richtext', 'composer', 'kit'].includes(node.tag)) {
            if (node.text !== undefined && !node.children?.length && element.textContent !== node.text)
                element.textContent = node.text;
            const desired = (node.children || []).map((child, index) => patch(child, element, `${identity}-${index}`));
            if (desired.length) {
                desired.forEach((child, index) => { if (element.children[index] !== child)
                    element.insertBefore(child, element.children[index] || null); });
                for (const child of [...element.children])
                    if (!desired.includes(child))
                        child.remove();
            }
            else if (node.text === undefined && element.childNodes.length)
        ...
        if (node.tag === 'kit') {
            ...
            if (component.type === 'image')
                component.props = { ...component.props, src: localUiMedia(component.props?.src) || '' };
            const signature = JSON.stringify(component);
            if (record.signature !== signature) {
                renderKitComponent(element, component, { preview: options.preview });
                record.signature = signature;
            }
        }
        if (['popover', 'tooltip'].includes(node.tag)) {
            record.slot ||= document.createElement('span');
            record.slot.hidden = true;
            if (!record.overlay && element.parentNode !== record.slot)
                record.slot.append(element);
            if (record.slot.parentNode !== parent)
                parent.append(record.slot);
            return record.slot;
        }
    ...
    function overlay(record) {
        ...
        for (const [property, attribute] of [['controlsKey', 'aria-controls'], ['activeDescendantKey', 'aria-activedescendant']]) {
            const target = typeof node.props?.[property] === 'string' ? records.get(node.props[property])?.element : undefined;
            if (target)
        ...
        if (Number.isSafeInteger(focusRequest) && Number(focusRequest) > 0 && focusRequest !== record.focusRequest && !node.hidden && !options.preview) {
            if (!element.matches('button,input,select,textarea,a[href],[contenteditable]'))
                element.tabIndex = -1;
            element.focus();
            record.focusRequest = Number(focusRequest);
        }
        if (['popover', 'tooltip'].includes(node.tag)) {
            ...
            if (!record.overlay) {
                record.overlay = bpPopover.open(anchor, { content: element, portalRoot: root, role: node.tag === 'tooltip' ? 'tooltip' : 'dialog',
                    ...
                    autoFocus: node.props?.autoFocus === true, onClose: () => { record.overlay = undefined; element.hidden = true; if (!record.closing && !disposed)
                        send(record, 'close'); } });
                ...
                record.overlay.updatePosition();
        }
        if (['dialog', 'drawer'].includes(node.tag)) {
            const dialog = element;
            if (node.open && !node.hidden && !dialog.open) {
                if (options.preview || typeof dialog.showModal !== 'function')
    ...
    function render(tree) {
        ...
        if (view.firstElementChild !== element)
