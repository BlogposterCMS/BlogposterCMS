...
async function runDialog(pageId, onSaved, options, navigation) {
    ...
    toolbar.className = 'article-editor__toolbar';
    toolbar.setAttribute('role', 'toolbar');
    toolbar.setAttribute('aria-label', 'Text formatting');
    const canvas = document.createElement('div');
    canvas.className = 'article-editor__document';
    ...
    for (const [value, label] of [['p', 'Paragraph'], ['1', 'Heading 1'], ['2', 'Heading 2'], ['3', 'Heading 3']])
        block.add(new Option(label, value));
    block.addEventListener('change', () => block.value === 'p' ? editor.chain().focus().setParagraph().run() : editor.chain().focus().toggleHeading({ level: Number(block.value) }).run());
    ...
    const link = button('Edit link', 'link', () => {
        ...
        apply.addEventListener('click', () => {
            ...
            if (input.value)
                editor.chain().focus().extendMarkRange('link').setLink({ href: input.value }).run();
            else
    ...
    async function insertMedia(kind) {
        ...
        update();
        try {
            const picked = await window.meltdownEmit?.('openMediaExplorer', { jwt: window.ADMIN_TOKEN, publicUrlOnly: true, accept: `${kind}/*` });
            if (!picked || picked.cancelled || !picked.shareURL)
                return;
            if (!safeArticleUrl(picked.shareURL))
                throw new Error('ARTICLE_MEDIA_URL_INVALID');
            if (kind === 'image')
                editor.chain().focus().setImage({ src: picked.shareURL, alt: '' }).run();
            else
                editor.chain().focus().insertContent({ type: 'video', attrs: { src: picked.shareURL } }).run();
        }
        catch (error) {
            message(error instanceof Error ? error.message : 'ARTICLE_MEDIA_FAILED', true);
        }
        finally {
            busy = false;
            update();
    ...
    const description = button('Media description', 'settings', () => {
        ...
        fields.className = 'article-editor__link';
        const input = document.createElement('input');
        input.setAttribute('aria-label', 'Media description');
    ...
    const table = button('Table actions', 'table', () => {
        ...
        const commands = editor.isActive('table') ? [
    ...
    await new Promise(resolve => { const attach = () => { if (body.isConnected)
        resolve();
    else
    ...
    function validateBlock(node) {
        ...
        try {
            const parsed = editor.schema.nodeFromJSON(node);
            parsed.check();
            if (!parsed.isBlock)
                throw new Error();
        }
    ...
    const agent = registerWorkspaceAgent({ root: body, id: `article-${pageId}`, title: 'Article editor', read: () => ({ dirty, busy, selection: pageId, title: header.title.value, contentLanguage: activeLanguage, sourceLanguage, availableLanguages: availableContentLanguages(), toolbar: toolbarState?.read(), translationExists: Boolean(page.trans_lang), seoTitleOverride: page.seo_title || '', document: editor.getJSON(), blocks: blocks(), linkFeedback: linkFeedback?.read(), error: status.getAttribute('role') === 'alert' ? status.textContent : null }),
        onCommandSettled: (_command, acknowledged) => { if (acknowledged && (studioRequested || nextLanguage))
            ...
            { action: 'article.replaceBlock', label: 'Replace article block', acceptsDraft: true, params: [{ name: 'id', type: 'string', required: true }, { name: 'node', type: 'object', required: true }], run: params => {
                    ...
                    if (!editor.commands.insertContentAt(blockRange(params.id), replacement))
                        throw new Error('ARTICLE_BLOCK_INVALID');
                } },
            { action: 'article.appendBlock', label: 'Append article block', acceptsDraft: true, params: [{ name: 'node', type: 'object', required: true }], run: params => { validateBlock(params.node); if (!editor.commands.insertContentAt(editor.state.doc.content.size, params.node))
                    throw new Error('ARTICLE_BLOCK_INVALID'); } },
