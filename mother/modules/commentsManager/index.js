...
module.exports = {
  ...
  async healthCheck({ motherEmitter, jwt }) {
    const rows = await commentsDbSelect(motherEmitter, jwt, 'LIST_COMMENTS_FOR_ENTRY', { limit: 1, offset: 0 });
    if (!Array.isArray(rows)) throw new Error('CORE_MODULE_COMMENTS_NOT_READY');
  },
