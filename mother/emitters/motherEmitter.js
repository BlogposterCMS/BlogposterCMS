...
function markInternalSystemPayload(payload = {}) {
  return {
    ...payload,
    [INTERNAL_SYSTEM_EVENT]: true
  };
}

function isInternalSystemPayload(payload) {
  return Boolean(payload && payload[INTERNAL_SYSTEM_EVENT] === true);
}

function removeListenersForModule(motherEmitter, moduleName) {
  const modName = String(moduleName || '').trim();
  if (!modName || !motherEmitter || typeof motherEmitter.eventNames !== 'function') {
...
function getTokenModuleName(decodedJWT) {
  return decodedJWT && typeof decodedJWT.moduleName === 'string'
    ? decodedJWT.moduleName
    : null;
}
...
function validateRegisteredModuleType(payload, motherEmitter) {
  const registeredType = getRegisteredModuleType(motherEmitter, payload.moduleName);
  if (
    registeredType &&
    payload.moduleType &&
    payload.moduleType !== registeredType
  ) {
    throw new Error(`Registered module type mismatch: "${payload.moduleName}" is "${registeredType}", payload requested "${payload.moduleType}".`);
  }
}

function validateTokenSubject(payload) {
  const tokenModuleName = getTokenModuleName(payload.decodedJWT);
  if (tokenModuleName && tokenModuleName !== payload.moduleName) {
    if ((payload.decodedJWT?.trustLevel || '').toLowerCase() === 'high') {
      return;
    }
    throw new Error(`JWT moduleName "${tokenModuleName}" does not match payload moduleName "${payload.moduleName}".`);
  }
}
...
class MotherEmitter extends EventEmitter {
  ...
  emit(eventName, ...args) {
    ...
    const { moduleName, skipJWT, authModuleSecret, jwt: providedJwt } = firstArg;

    if (INTERNAL_SYSTEM_EVENTS.has(eventName)) {
      if (!isInternalSystemPayload(firstArg)) {
        console.warn('[MotherEmitter] Blocked external system event="%s".', eventName);
        const cb = args.find(a => typeof a === 'function');
        if (cb) cb(new Error(`Event "${eventName}" is internal and cannot be emitted directly.`));
        return false;
      }
    ...
    try {
      validateRegisteredModuleType(firstArg, this);
    } catch (typeErr) {
      meltdownForModule(typeErr.message, moduleName, this);
      return false;
    }
    ...
    try {
      validateTokenSubject(firstArg);
    } catch (subjectErr) {
      meltdownForModule(subjectErr.message, moduleName, this);
      return false;
    }
...
module.exports = {
  ...
  _internals: {
    ...
    getTokenModuleName,
    isInternalSystemPayload,
    markInternalSystemPayload,
    removeListenersForModule,
    validateRegisteredModuleType,
