...
export function createContentLanguageControl(current: () => string, source: () => string, open: (language: string) => Promise<void>, actions: Array<{ label: string; run: () => void | Promise<void>; available?: () => boolean }> = []) {
  ...
  button.addEventListener('click', () => {
    const body = document.createElement('div'); body.className = 'article-editor__link content-language-picker';
    const input = document.createElement('select'); input.disabled = true;
    input.setAttribute('aria-label', 'Language / locale');
    const label = createFormField('Language / locale', input);
    const hint = document.createElement('p'); hint.textContent = `Main language: ${source()}. Unsaved changes must be saved or discarded before switching.`;
    const status = document.createElement('p'); status.setAttribute('role', 'status');
    const apply = document.createElement('button'); apply.type = 'button'; apply.className = 'button secondary sm'; apply.textContent = 'Open language'; apply.disabled = true;
    body.append(label, hint, status, apply);
    for (const action of actions) {
      const extra = document.createElement('button'); extra.type = 'button'; extra.className = 'button ghost sm'; extra.textContent = action.label;
    ...
    void loadContentLanguages().then(config => {
      ...
      for (const code of [...new Set([source(), current(), ...config.languages])]) input.add(new Option(`${contentLanguageLabel(code)}${code === source() ? ' (main)' : !config.languages.includes(code) ? ' (existing content)' : ''}`, code));
      input.value = current(); input.disabled = false; status.textContent = '';
      enhanceSelects(body); popover?.updatePosition();
    ...
    const activate = async () => {
      apply.disabled = true;
      try { await open(normalizePageLanguage(input.value)); popover?.close(); value.textContent = current(); }
      catch (error) { status.textContent = error instanceof Error ? error.message : 'CONTENT_LANGUAGE_OPEN_FAILED'; }
      finally { apply.disabled = input.disabled || input.value === current(); }
    };
