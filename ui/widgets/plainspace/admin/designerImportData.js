...
function invalid(message) { throw new Error(`DESIGN_IMPORT_INVALID: ${message}`); }
/** A portable copy contains native presentation data, never executable widget packages. */
export function parseNativeDesignImport(text) {
    if (text.length > 1024 * 1024)
        invalid('Maximum file size is 1 MiB.');
    let value;
    try {
        value = JSON.parse(text);
    }
    catch {
        invalid('Choose valid native design JSON.');
    }
    let count = 0;
    function inspect(node, depth = 0) {
        if (++count > 32000 || depth > 32)
            invalid('Design exceeds nesting or value limits.');
        if (!node || typeof node !== 'object')
            return;
        for (const [key, child] of Object.entries(node)) {
            if (['__proto__', 'prototype', 'constructor', 'allowCustomJs'].includes(key)
                || (key === 'js' && child))
                invalid('Executable code and trust flags cannot be imported.');
    ...
    if (!value || typeof value !== 'object' || !value.design || typeof value.design.title !== 'string'
        ...
        || !Array.isArray(value.widgets) || value.widgets.length > 500)
        invalid('Expected design, layout and up to 500 widgets.');
    const ids = new Set();
    for (const widget of value.widgets) {
        if (!widget || typeof widget !== 'object' || typeof widget.id !== 'string' || !widget.id.trim()
            || ids.has(widget.id) || typeof widget.widgetId !== 'string' || !/^[A-Za-z0-9_-]{1,80}$/.test(widget.widgetId))
            invalid('Widget identities must be valid and unique.');
        ids.add(widget.id);
    }
...
export async function importNativeDesign(text) {
    const data = parseNativeDesignImport(text);
    if (!window.meltdownEmit)
        throw new Error('DESIGN_IMPORT_BRIDGE_MISSING');
    const saved = await emitRuntimeAdmin(window.meltdownEmit, window.ADMIN_TOKEN, 'designer', 'save', data);
    if (!saved?.id)
        throw new Error('DESIGN_IMPORT_SAVE_RESULT_INVALID');
