...
- **Content sanitization** – Public pages retain `<style>` tags while stripping scripts and unsafe CSS patterns (like `expression` or `javascript:` URLs) so designs render without enabling script injection.
