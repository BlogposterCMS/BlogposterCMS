...
async function handleBuiltInPlaceholderMongo(db, operation, params) {
  ...
  if (isCommentsPlaceholder(operation)) {
    return handleCommentsMongo(db, operation, params);
  }
  if (isNavigationPlaceholder(operation)) {
    return handleNavigationMongo(db, operation, params);
  }
  if (isSeoPlaceholder(operation)) {
    return handleSeoMongo(db, operation, params);
  }
  if (isSearchPlaceholder(operation)) {
    return handleSearchMongo(db, operation, params);
  }
  if (isRedirectPlaceholder(operation)) {
    return handleRedirectMongo(db, operation, params);
  }
  if (isMediaPlaceholder(operation)) {
    return handleMediaMongo(db, operation, params);
  }
  if (isMetadataPlaceholder(operation)) {
    return handleMetadataMongo(db, operation, params);
  }
  if (isWorkflowPlaceholder(operation)) {
    return handleWorkflowMongo(db, operation, params);
  }
  if (isTranslationPlaceholder(operation)) {
    return handleTranslationMongo(db, operation, params);
  }
  ...
  switch (operation) {
    ...
    else if (prefix) query.key = { $regex: `^${escapeRegex(prefix)}` };
    const docs = await db.collection('cms_settings').find(query).sort({ key: 1 }).toArray();
    return docs;
    }
   ...
   case 'DELETE_PAGE': {
    ...
    case 'LIST_APP_REGISTRY': {
      return await db.collection('app_registry')
        .find({})
        .sort({ app_name: 1 })
        .toArray();
    }
