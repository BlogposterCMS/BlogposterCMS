...
export function createWidgetServiceConfigurationControl(): HTMLElement {
  const group = document.createElement('span');
  const button = document.createElement('button'); button.type = 'button';
  button.className = 'button secondary sm'; button.textContent = 'Import service configuration';
  ...
  input.addEventListener('change', async () => {
    ...
    try {
      if (file.size > 128 * 1024) throw new Error('WIDGET_SERVICE_CONFIG_TOO_LARGE: Maximum size is 128 KiB.');
      const text = await file.text();
      const config = parseWidgetServiceConfiguration(text);
      const body = document.createElement('div');
      for (const [id, policy] of Object.entries(config.widgets) as Array<[string, any]>) {
        const heading = document.createElement('h3'); heading.textContent = id; body.append(heading);
        const list = document.createElement('ul');
        for (const [name, op] of Object.entries(policy.operations || {}) as Array<[string, any]>) {
      ...
      if (review.action !== 'save') { status.textContent = 'Configuration cancelled.'; return; }
