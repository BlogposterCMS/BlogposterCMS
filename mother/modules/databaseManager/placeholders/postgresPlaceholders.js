...
if (isCommentsPlaceholder(operation)) {
    return handleCommentsPostgres(client, operation, params);
}
if (isNavigationPlaceholder(operation)) {
    return handleNavigationPostgres(client, operation, params);
}
if (isSeoPlaceholder(operation)) {
    return handleSeoPostgres(client, operation, params);
}
if (isSearchPlaceholder(operation)) {
    return handleSearchPostgres(client, operation, params);
}
if (isRedirectPlaceholder(operation)) {
    return handleRedirectPostgres(client, operation, params);
}
if (isMediaPlaceholder(operation)) {
    return handleMediaPostgres(client, operation, params);
}
if (isMetadataPlaceholder(operation)) {
    return handleMetadataPostgres(client, operation, params);
}
if (isWorkflowPlaceholder(operation)) {
    return handleWorkflowPostgres(client, operation, params);
}
if (isTranslationPlaceholder(operation)) {
    return handleTranslationPostgres(client, operation, params);
}
switch (operation) {
    // ─────────────────────────────────────────────────────────────────────────
    // USER MANAGEMENT
    ...
    case 'INIT_SETTINGS_SCHEMA': {
      ...
      case 'DELETE_SETTING': {
        ...
        await client.query('DELETE FROM settingsManager.cms_settings WHERE key = $1;', [settingKey]);
      ...
      await client.query(`
        UPDATE pagesManager.pages
          SET title     = CASE WHEN $2 THEN $3 ELSE title END,
              ...
              weight    = CASE WHEN $6 THEN $7 ELSE weight END,
              slug      = CASE WHEN $8 THEN $9 ELSE slug END,
              status    = CASE WHEN $10 THEN $11 ELSE status END,
              seo_image = CASE WHEN $12 THEN $13 ELSE seo_image END,
              parent_id = CASE WHEN $14 THEN $15 ELSE parent_id END,
              is_content= CASE WHEN $16 THEN $17 ELSE is_content END,
              lane      = CASE WHEN $18 THEN $19 ELSE lane END,
              language  = CASE WHEN $20 THEN $21 ELSE language END,
              updated_at= NOW()
        WHERE id = $1;
      `, [
        /* $1 */ pageId,
        ...
        /*$10*/ hasField('status'),
        /*$11*/ status ?? null,
        /*$12*/ hasField('seo_image'),
        /*$13*/ seo_image ?? null,
        /*$14*/ hasField('parent_id'),
        /*$15*/ parent_id ?? null,
        /*$16*/ hasField('is_content'),
        /*$17*/ contentVal,
        /*$18*/ hasField('lane'),
        /*$19*/ lane ?? null,
        /*$20*/ hasField('language'),
        /*$21*/ language ?? null
      ]);
    ...
    case 'LIST_APP_REGISTRY': {
      const { rows } = await client.query(
        `SELECT * FROM "apploader"."app_registry" ORDER BY app_name ASC;`
      );
      return rows;
    }
    ...
    case 'UPSERT_PLAINSPACE_LAYOUT_TEMPLATE': {
      const d = params[0] || {};
      await client.query(`
        INSERT INTO plainspace.layout_templates (name, lane, viewport, layout_json, preview_path, is_global, updated_at)
