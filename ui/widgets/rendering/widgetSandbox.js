 ...
 */
export function widgetSandboxNonce(existing) {
    if (existing) {
...
export async function mountSandboxWidget(container, id, codeUrl, context) {
    mounted.get(container)?.();
    const parsed = new URL(codeUrl, document.baseURI);
    if (parsed.origin !== new URL(document.baseURI).origin || parsed.pathname !== `/widgets/${id}/widget.js`)
        throw new Error('WIDGET_SANDBOX_PATH_DENIED');
    const nonce = widgetSandboxNonce(window.NONCE || document.querySelector('script[nonce]')?.nonce);
    ...
    if (!response.ok || response.headers.get('X-Blogposter-Widget-Contract') !== '2')
        throw new Error('WIDGET_SANDBOX_MIGRATION_REQUIRED');
    const source = await response.text();
    if (source.length > 262144)
        throw new Error('WIDGET_SANDBOX_SOURCE_LIMIT');
    ...
    channel.port1.onmessage = async (event) => {
        ...
        try {
            ...
            if (++messages > 100 || JSON.stringify(message).length > 131072)
                return fail('WIDGET_SANDBOX_MESSAGE_LIMIT');
            if (message.type === 'connected' && !started) {
                ...
                if (!Number.isSafeInteger(message.id) || inflight >= 4)
                    return fail('WIDGET_SERVICE_MESSAGE_INVALID');
                inflight++;
                try {
                    ...
                    if (message.method === 'request' && NAME.test(message.name))
                        ...
                        result = services.draft.set(message.input);
                    else if (message.method === 'preferences.get' && NAME.test(message.name))
                        result = services.preferences.get(message.name);
                    else if (message.method === 'preferences.set' && NAME.test(message.name))
                        result = services.preferences.set(message.name, message.input);
                    else
