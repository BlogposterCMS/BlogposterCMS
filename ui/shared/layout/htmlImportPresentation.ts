type ImportFrame = { minWidth: number; maxWidth: number; height: number };
...
export function applyHtmlImportPresentation(wrapper: HTMLElement, info: Record<string, any>): void {
  const page = info?.page;
  if (!page || typeof page.rootId !== 'string' || !/^html-import-[a-f0-9]+(?:-[a-z0-9-]+)?$/.test(page.rootId)) return;
  const root = wrapper.closest<HTMLElement>(`[data-node-id="${page.rootId}"]`);
  if (!root || observedRoots.has(root) || !Array.isArray(page.frames)) return;
  const frames: ImportFrame[] = page.frames.filter((frame: ImportFrame) =>
    [frame?.minWidth, frame?.maxWidth, frame?.height].every(Number.isFinite)
  ...
  const containerRoot = page.containerRootId === 'page-root'
    ? root.closest<HTMLElement>('[data-node-id="page-root"]') || root : root;
  ...
  const apply = () => {
    ...
    if (root.dataset.layoutMinHeight !== String(page.initialMinHeight)) return;
    const width = root.clientWidth;
    const frame = frames.find(candidate => width >= candidate.minWidth && width <= candidate.maxWidth);
    if (frame) root.style.minHeight = `${frame.height}px`;
  };
  apply();
