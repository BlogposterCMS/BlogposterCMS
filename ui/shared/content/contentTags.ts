...
export function normalizeContentTags(value: unknown): string[] {
  ...
  if (!Array.isArray(items) || items.length > 24) return invalidTags();
  const tags: string[] = [];
  for (const item of items) {
    if (typeof item !== 'string' || /[\u0000-\u001f\u007f]/u.test(item)) return invalidTags();
    const tag = item.normalize('NFKC').trim().toLowerCase().replace(/\s+/gu, ' ');
    if (!tag) continue;
    if (tag.length > 64 || !/^[\p{L}\p{N}][\p{L}\p{N} ._:-]*$/u.test(tag)) return invalidTags();
    if (!tags.includes(tag)) tags.push(tag);
  }
  return tags;
}

function invalidTags(): never {
...
export function normalizeTaggedMeta<T>(meta: T): T {
  ...
  if (typeof meta === 'string') {
    let parsed: unknown;
    try { parsed = JSON.parse(meta); } catch { return meta; }
    return JSON.stringify(normalizeTaggedMeta(parsed)) as T;
  }
  if (!meta || typeof meta !== 'object' || Array.isArray(meta) || !Object.hasOwn(meta, 'tags')) return meta;
  return { ...meta, tags: normalizeContentTags((meta as Record<string, unknown>).tags) };
}
