...
function csrfToken(): string {
  return window.CSRF_TOKEN || document.querySelector<HTMLMetaElement>('meta[name="csrf-token"]')?.content || '';
}
...
export async function render(el: HTMLElement | null): Promise<void> {
  ...
  try {
    ...
    saveRegistration.type = 'button'; saveRegistration.className = 'button primary';
    saveRegistration.textContent = 'Save registration settings';
    const discard = document.createElement('button');
    discard.type = 'button'; discard.className = 'button ghost'; discard.textContent = 'Discard changes';
    discard.hidden = true;
    ...
    registerWorkspaceAgent({ root: card, id: 'settings-registration', title: 'Registration settings',
      ...
      actions: [
        { action: 'settings.updateDraft', label: 'Update registration', acceptsDraft: true,
          ...
          run: p => { if (p.group !== 'registration') throw new Error('SETTINGS_AGENT_GROUP_UNAVAILABLE'); patchAgentForm(card, p.fields, ['allowRegistration']); } },
        { action: 'settings.save', label: 'Save registration settings', acceptsDraft: true, confirm: true,
          ...
          run: p => { if (p.group !== 'registration') throw new Error('SETTINGS_AGENT_GROUP_UNAVAILABLE'); return saveRegistrationDraft(true); } }
      ] });
    ...
    async function refreshAgentCodes(): Promise<void> {
    ...
    const registrationGroup = document.createElement('div'); registrationGroup.className = 'settings-group';
    const agentGroup = document.createElement('div'); agentGroup.className = 'settings-group';
