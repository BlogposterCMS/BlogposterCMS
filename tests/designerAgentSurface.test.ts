...
test('designer uses the central agent manager instead of a private agent API', () => {
  ...
  expect(agentSurfaceSource).toContain('function widgetLibraryFeedbackState');
  expect(agentSurfaceSource).toContain('widgetLibrary: widgetLibraryFeedbackState()');
  expect(agentSurfaceSource).toContain('widgetPresetPopovers: true');
  expect(agentSurfaceSource).toContain('sectionInsertFeedback: true');
  expect(agentSurfaceSource).toContain('sectionDeletion: true');
  expect(agentSurfaceSource).toContain('activeSectionBoundaries: true');
  expect(agentSurfaceSource).toContain('activeSectionBoundaryTransition: true');
  expect(agentSurfaceSource).toContain('sectionToolbarInsetPlacement: true');
  expect(agentSurfaceSource).toContain('sectionResizeZoomInvariant: true');
  expect(agentSurfaceSource).toContain('placementModeIndependentWidgetSelection: true');
  expect(agentSurfaceSource).toContain("boundaryPresentation: 'filigree-fade'");
  expect(agentSurfaceSource).toContain("toolbarPlacement: 'top-right-inset'");
  expect(agentSurfaceSource).toContain("resizeInteraction: 'zoom-invariant-authored-pixels'");
  expect(agentSurfaceSource).toContain('sectionBackgroundDeselectsWidget: true');
  expect(agentSurfaceSource).toContain('widgetActionBarHidesOnCanvasScroll: true');
  expect(agentSurfaceSource).toContain('sectionInsertState');
  expect(agentSurfaceSource).toContain('flowingSectionCanvas: true');
  expect(agentSurfaceSource).toContain('dynamicCanvasHeight');
  expect(agentSurfaceSource).toContain("presentation: 'scrollable-widget-rail-with-preset-popover'");
  expect(agentSurfaceSource).toContain("role: 'insert-preset-command'");
  ...
  expect(agentSurfaceSource).toContain('DESIGNER_AGENT_FEEDBACK_LAYOUT_GRID_TREE_MISMATCH');
  expect(agentSurfaceSource).toContain('DESIGNER_AGENT_FEEDBACK_FREE_PLACEMENT_LOCKED');
  expect(agentSurfaceSource).toContain('freePlacementInteractive');
  expect(agentSurfaceSource).toContain('gridSurfaceId:');
  expect(agentSurfaceSource).toContain('parentGridSurfaceId:');
  expect(agentSurfaceSource).toContain('parentGridItem:');
  ...
  expect(agentSurfaceSource).toContain("label: 'Next section'");
  expect(agentSurfaceSource).toContain("label: 'Previous section'");
  expect(agentSurfaceSource).toContain("label: 'Add section'");
  expect(agentSurfaceSource).toContain("label: 'Delete section'");
  expect(agentSurfaceSource).toContain("deleteBehavior: 'remove-section-and-recursive-content'");
  expect(agentSurfaceSource).toContain("boundaryVisibility: section.classList.contains('layout-section--active')");
  expect(agentSurfaceSource).toContain("label: 'Scenes'");
  expect(agentSurfaceSource).toContain('`Section ${index + 1}`');
  expect(agentSurfaceSource).toContain('DESIGNER_AGENT_FEEDBACK_SECTION_STRUCTURE_MISMATCH');
  expect(agentSurfaceSource).toContain("'#layoutRoot > .layout-section[data-section-id]'");
  ...
  expect(agentSurfaceSource).toContain("action: 'container.duplicate'");
  ...
  expect(feedbackGuideSource).toContain('DESIGNER_AGENT_FEEDBACK_FREE_PLACEMENT_LOCKED');
  expect(feedbackGuideSource).toContain('container.duplicate');
});
