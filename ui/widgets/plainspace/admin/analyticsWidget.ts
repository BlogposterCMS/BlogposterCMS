import { fetchAnalytics, type AnalyticsSummary } from './analyticsData.js';
...
function table(title: string, columns: string[], rows: string[][], emptyMessage = 'No recorded activity in this period.'): HTMLElement {
  const section = node('section', '', 'analytics-panel'); section.append(node('h3', title));
  if (!rows.length) { section.append(node('p', emptyMessage)); return section; }
  ...
  if (columns.length > 5) table.style.minWidth = '1200px';
  const head = node('thead'); const header = node('tr');
  columns.forEach(label => { const th = node('th', label); th.scope = 'col'; header.append(th); }); head.append(header);
...
export function renderSummary(data: AnalyticsSummary): HTMLElement {
  ...
  for (const [name, count, previous] of [
    ...
    if (previous !== null) row.append(node('small', `Previous period: ${previous}`));
    section.append(row);
  }
  return section;
}
export type AnalyticsView = 'overview' | 'website' | 'devices' | 'system';
const VIEWS = {
...
} satisfies Record<AnalyticsView, { title: string; description: string; dimensions: string[] }>;

// All catalog widgets share rendering and the existing authorized summary API.
export async function renderAnalytics(el: HTMLElement | null, view: AnalyticsView): Promise<void> {
  if (!el) return;
  const config = VIEWS[view];
  const root = node('div', '', 'analytics-workspace');
  root.append(node('h2', config.title), node('p', config.description));
  const label = node('label', 'Period '); const period = node('select');
  [1, 7, 30].forEach(days => { const option = node('option', `${days} day${days === 1 ? '' : 's'}`); option.value = String(days); period.append(option); });
  period.value = '7'; label.append(period);
  const refresh = node('button', 'Refresh', 'button secondary sm'); refresh.type = 'button';
  const content = node('div', '', 'analytics-content'); content.setAttribute('aria-live', 'polite');
  const toolbar = node('div', '', 'analytics-toolbar'); toolbar.append(label, refresh);
  root.append(toolbar, content); el.replaceChildren(root);
  let generation = 0;
  async function load() {
    const current = ++generation;
    content.replaceChildren(node('p', 'Loading analytics…')); refresh.disabled = true;
    try {
      const data = await fetchAnalytics(Number(period.value));
      if (current !== generation) return;
      content.replaceChildren();
      if (view === 'overview') content.append(renderSummary(data));
      ...
      if (data.health.lastError || data.health.dropped || data.health.truncated || !data.health.enabled) {
        ...
        warning.setAttribute('role', 'alert'); content.append(warning);
      }
      if (view === 'overview') {
        const chart = node('div'); content.append(chart);
        ...
        for (const section of ['website', 'devices', 'system'] as const) {
          const link = node('a', VIEWS[section].title, 'button secondary sm'); link.href = `/admin/analytics/${section}`; links.append(link);
        }
      ...
      for (const [key, title] of Object.entries({ page: 'Pages (ID)', source: 'Referrer domains', device: 'Device classes', browser: 'Browsers', os: 'Operating systems', event: 'System events', module: 'Target modules', actor: 'Verified actors', outcome: 'Event outcomes' })) {
        if (!(config.dimensions as string[]).includes(key)) continue;
      ...
      if (view === 'website') {
        ...
        settings.href = '/admin/settings/general?tab=privacy'; content.append(settings);
        const filter = node('input'); filter.type = 'search'; filter.placeholder = 'Page, account, visitor, session or referrer';
        const filterLabel = node('label', 'Filter recent visits '); filterLabel.append(filter);
        ...
        const draw = () => {
          ...
          visits.replaceChildren(table('Recent page deliveries (latest 500)',
            ['Time', 'Page', 'Path', 'Account ID', 'Browser visitor ID', 'Session', 'Referrer domain', 'Location (approximate)', 'Device / browser / OS'],
            rows.map(row => [new Date(row.at).toLocaleString(), `${row.title || row.page} (${row.page})`, row.path || 'Not recorded',
        ...
        filter.addEventListener('input', draw); draw(); content.append(filterLabel, visits);
      ...
      if (view === 'system') content.append(table('Recent system activity', ['Time', 'Event', 'Actor', 'Outcome'], data.recent.filter(row => row.kind === 'system').map(row => [new Date(row.at).toLocaleString(), row.event, row.actor, row.outcome])));
      ...
      if (current !== generation) return;
      const message = node('p', `ANALYTICS_LOAD_FAILED: ${error instanceof Error ? error.message : String(error)}`); message.setAttribute('role', 'alert'); content.replaceChildren(message);
    } finally { if (current === generation) refresh.disabled = false; }
  }
  refresh.addEventListener('click', () => void load()); period.addEventListener('change', () => void load()); await load();
}

export async function render(el: HTMLElement | null): Promise<void> { await renderAnalytics(el, 'overview'); }
