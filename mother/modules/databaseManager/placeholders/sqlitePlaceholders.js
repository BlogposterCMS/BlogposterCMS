...
} = require('./contentEnginePlaceholders');
const {
  handleCommentsSqlite,
  isCommentsPlaceholder
} = require('./commentsPlaceholders');
const {
  handleNavigationSqlite,
  isNavigationPlaceholder
} = require('./navigationPlaceholders');
const {
  handleSeoSqlite,
  isSeoPlaceholder
} = require('./seoPlaceholders');
const {
  handleSearchSqlite,
  isSearchPlaceholder
} = require('./searchPlaceholders');
const {
  handleRedirectSqlite,
  isRedirectPlaceholder
} = require('./redirectPlaceholders');
const {
  handleMediaSqlite,
  isMediaPlaceholder
} = require('./mediaPlaceholders');
const {
  handleMetadataSqlite,
  isMetadataPlaceholder
} = require('./metadataPlaceholders');
const {
  handleWorkflowSqlite,
  isWorkflowPlaceholder
} = require('./workflowPlaceholders');
const {
...
async function handleBuiltInPlaceholderSqlite(db, operation, params) {
  ...
  if (isContentEnginePlaceholder(operation)) {
    return handleContentEngineSqlite(db, operation, params);
  }
  if (isCommentsPlaceholder(operation)) {
    return handleCommentsSqlite(db, operation, params);
  }
  if (isNavigationPlaceholder(operation)) {
    return handleNavigationSqlite(db, operation, params);
  }
  if (isSeoPlaceholder(operation)) {
    return handleSeoSqlite(db, operation, params);
  }
  if (isSearchPlaceholder(operation)) {
    return handleSearchSqlite(db, operation, params);
  }
  if (isRedirectPlaceholder(operation)) {
    return handleRedirectSqlite(db, operation, params);
  }
  if (isMediaPlaceholder(operation)) {
    return handleMediaSqlite(db, operation, params);
  }
  if (isMetadataPlaceholder(operation)) {
    return handleMetadataSqlite(db, operation, params);
  }
  if (isWorkflowPlaceholder(operation)) {
    return handleWorkflowSqlite(db, operation, params);
  }
  if (isTranslationPlaceholder(operation)) {
    return handleTranslationSqlite(db, operation, params);
  }
  ...
  switch (operation) {
    ...
    case 'LIST_SETTINGS': {
      ...
      if (keys.length) {
        ...
        return await db.all(`
          SELECT key, value FROM settingsManager_cms_settings
      ...
      return await db.all('SELECT key, value FROM settingsManager_cms_settings ORDER BY key ASC;');
    }
    ...
    case 'DELETE_SETTING': {
      ...
      `);
      await db.run('DELETE FROM settingsManager_cms_settings WHERE key = ?;', [settingKey]);
    ...
    case 'UPDATE_PAGE': {
      ...
      const hasField = name => Object.prototype.hasOwnProperty.call(p, name);
      const metaVal = (meta && typeof meta === 'object') ? JSON.stringify(meta) : meta;
      const hasWeight = hasField('weight');
      const weightVal = hasWeight ? Number(weight) || 0 : null;
      const contentVal = typeof is_content === 'undefined' ? null : is_content;

      await db.run(`
        UPDATE pagesManager_pages
           SET title      = CASE WHEN ? THEN ? ELSE title END,
               meta       = CASE WHEN ? THEN ? ELSE meta END,
               weight     = CASE WHEN ? THEN ? ELSE weight END,
               slug       = CASE WHEN ? THEN ? ELSE slug END,
               status     = CASE WHEN ? THEN ? ELSE status END,
               seo_image  = CASE WHEN ? THEN ? ELSE seo_image END,
               parent_id  = CASE WHEN ? THEN ? ELSE parent_id END,
               is_content = CASE WHEN ? THEN ? ELSE is_content END,
               lane       = CASE WHEN ? THEN ? ELSE lane END,
               language   = CASE WHEN ? THEN ? ELSE language END,
               updated_at = CURRENT_TIMESTAMP
         WHERE id = ?;
      `, [
        hasField('title') ? 1 : 0, title ?? null,
        hasField('meta') ? 1 : 0, metaVal ?? null,
        hasWeight ? 1 : 0, weightVal,
        hasField('slug') ? 1 : 0, slug ?? null,
        hasField('status') ? 1 : 0, status ?? null,
        hasField('seo_image') ? 1 : 0, seo_image ?? null,
        hasField('parent_id') ? 1 : 0, parent_id ?? null,
        hasField('is_content') ? 1 : 0, contentVal,
        hasField('lane') ? 1 : 0, lane ?? null,
        hasField('language') ? 1 : 0, language ?? null,
        pageId
      ]);
...
/* ────────────────────────────────────────────────────────────────
   DEPENDENCY LOADER
   ─────────────────────────────────────────────────────────────── */
case 'LIST_APP_REGISTRY': {
  const rows = await db.all(
    `SELECT * FROM appLoader_app_registry ORDER BY app_name ASC;`
  );
  return rows;
}
