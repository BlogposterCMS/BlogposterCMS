...
export function createCoreModuleUpdateList(mount: HTMLElement, install: (row: CoreModuleUpdateRow) => void, titleText = 'CMS module updates') {
  ...
  return (rows: CoreModuleUpdateRow[], disabled: boolean, lastCheckedAt?: string) => {
    ...
    for (const row of rows) {
      ...
      if (!entry) {
        ...
        container.append(summary, notes, button); section.append(container);
      }
