...
function buildFallbackHeader() {
    ...
    brand.className = 'builder-header-brand';
    header.appendChild(brand);
    const backBtn = document.createElement('button');
    backBtn.type = 'button';
    backBtn.className = 'builder-back-btn';
    backBtn.setAttribute('aria-label', 'Back');
    backBtn.innerHTML = '<img src="/assets/icons/panel-left.svg" alt="" class="icon" />';
    brand.appendChild(backBtn);
    const nameShell = document.createElement('label');
    nameShell.className = 'layout-name-shell';
    nameShell.innerHTML = '<span class="layout-name-label">Studio</span>';
    const nameInput = document.createElement('input');
    nameInput.id = 'layoutNameInput';
    nameInput.type = 'text';
    nameInput.className = 'layout-name-input';
    nameInput.placeholder = 'Scene builder';
    ...
    const createActionButton = ({ id, className, icon, label, alt }) => {
        const button = document.createElement('button');
        button.type = 'button';
    ...
    headerActions.appendChild(createActionButton({
        className: 'header-icon-btn menu-undo',
        icon: '/assets/icons/undo-2.svg',
        alt: 'Undo'
    }));
    headerActions.appendChild(createActionButton({
        className: 'header-icon-btn menu-redo',
        icon: '/assets/icons/redo-2.svg',
        alt: 'Redo'
    }));
    headerActions.appendChild(createActionButton({
        id: 'previewLayoutBtn',
        className: 'header-icon-btn builder-preview-btn',
    ...
    headerActions.appendChild(createActionButton({
        id: 'saveLayoutBtn',
        className: 'header-icon-btn builder-save-btn',
...
async function loadHeaderPartial(existing) {
    ...
    try {
        ...
        if (!sanitized)
            throw new Error('Empty header markup');
        const tpl = document.createElement('template');
        tpl.innerHTML = sanitized;
        const headerEl = tpl.content.firstElementChild;
        if (!(headerEl instanceof HTMLElement))
            throw new Error('Missing builder header root element');
...
export function createBuilderHeader({ initialLayoutName, layoutNameParam, pageData, gridEl, viewportSizeEl, grid, saveDesign, getCurrentLayoutForLayer, getActiveLayer, ensureCodeMap, capturePreview, updateAllWidgetContents, getAdminUserId, pageId, layoutRoot, state, startAutosave, showPreviewHeader, hidePreviewHeader, undo, redo }) {
    ...
    function observeHeader(el) {
        ...
        const updateVar = (height) => setHeaderHeightVariable(height);
    ...
    async function renderHeader({ reload = false } = {}) {
        try {
            ...
            saveMenuBtn.addEventListener('click', e => {
                e.stopPropagation();
                if (saveDropdown.style.display === 'block') {
                    hideSaveDropdown();
                    return;
                }
