...
import { asString, clearPageEditorCache, errorMessage, savePageEditorPage, loadPageEditorPage, loadPageEditorTranslation, normalizePageLanguage } from './pageEditorData.js';
export async function render(el) {
    ...
    let source;
    try {
        source = await loadPageEditorPage(emit, jwt, window.location.pathname, window.ADMIN_BASE || 'admin', window.pageDataPromise, window.pageDataLoader, new URLSearchParams(window.location.search).get('contentLang') || undefined);
    }
    catch (error) {
    ...
    root.querySelector('.page-editor-back').href = `${adminBase}/content/pages`;
    const heading = root.querySelector('h2');
    const details = root.querySelector('.page-editor-widget');
    const content = root.querySelector('.page-editor-content');
    const body = root.querySelector('.page-editor-body');
    const feedback = root.querySelector('.page-editor-feedback');
    const save = root.querySelector('[data-save]');
    const discard = root.querySelector('[data-discard]');
    const languageHost = document.createElement('section');
    languageHost.setAttribute('aria-label', 'Page translation');
    ...
    footer.append(feedback, root.querySelector('.page-editor-actions'));
    root.append(footer);
    const general = document.createElement('section');
    general.className = 'page-editor-general';
    general.append(details.querySelector('h3'));
    details.append(general);
    let fieldTarget = general;
    ...
    function field(name, label, type = 'text') {
        ...
        if ((name === 'featuredImage' || name === 'seoImage') && control instanceof HTMLInputElement) {
            ...
            urlSummary.textContent = 'Add via URL';
            ...
            preview.className = 'page-editor-image-preview';
            ...
            const removeImage = imageField.root.querySelector('.button.ghost');
            ...
            fieldTarget.append(imageField.root);
            imageFields.push(imageField);
        }
        else
            fieldTarget.append(createFormField(label, control));
    ...
    seo.className = 'page-editor-seo';
    seo.setAttribute('aria-label', 'SEO and link previews');
    seo.append(seoHeading);
    details.append(seo);
    const seoText = document.createElement('div');
    seoText.className = 'page-editor-seo-text';
    seo.append(seoText);
    fieldTarget = seoText;
    field('seoTitle', 'SEO title');
    field('seoDesc', 'SEO description', 'textarea');
    const seoImages = document.createElement('div');
    seoImages.className = 'page-editor-seo-images';
    seo.append(seoImages);
    fieldTarget = seoImages;
    ...
    tabBar.className = 'page-editor-tabs';
    const tabToolbar = document.createElement('div');
    tabToolbar.className = 'page-editor-tab-toolbar';
    const contentActions = document.createElement('div');
    contentActions.className = 'page-editor-tab-actions';
    tabToolbar.append(tabBar, contentActions);
    body.before(tabToolbar);
    const tabs = createTabSystem(body, tabBar, { variant: 'underline', onSelect: index => {
            enhanceSelects(body);
            contentActions.hidden = index !== 1;
        } });
    const detailsTab = tabs.addTab('Details');
    const contentTab = tabs.addTab('Layout & content');
    const seoTab = tabs.addTab('SEO & previews');
    detailsTab.append(general);
    contentTab.append(content);
    seoTab.append(seo);
    details.remove();
    root.addEventListener('invalid', event => {
        if (event.target instanceof Element && general.contains(event.target))
            tabs.select(0);
    }, true);
    if (['#page-design', '#page-html', '#page-html-upload'].includes(window.location.hash))
        tabs.select(1);
    ...
    function values() {
        return Object.fromEntries(Array.from(fields, ([name, input]) => [name, input.value]));
    }
    ...
    async function openLanguage(language) {
        language = normalizePageLanguage(language);
        if (busy || contentBusy || dirty)
            throw new Error('PAGE_EDITOR_LANGUAGE_PENDING: Save or discard your changes before switching language.');
        busy = true;
        updateState();
        try {
            ...
            page = translated;
            Object.assign(draft, translated, { meta: { ...translated.meta } });
            resetFields();
            await mountContent();
    ...
    async function savePage(propagate = false) {
        if (!root.isConnected || busy || contentBusy || !dirty)
            return;
        if (!root.reportValidity()) {
            if (propagate)
                throw new Error('PAGE_EDITOR_FIELDS_INVALID: Correct the required page fields.');
            return;
        }
        busy = true;
        updateState();
    ...
    discard.addEventListener('click', async () => {
        if (busy || contentBusy || !dirty || !(await bpDialog.confirm('Discard your unsaved page changes?', { confirmLabel: 'Discard changes' })))
            return;
    ...
    window.saveCurrentPage = savePage;
    await mountContent();
    if (page.contentLanguage && !page.trans_lang)
        setFeedback(`No ${page.contentLanguage} translation yet. Enter its title and content, then save the page.`);
    registerWorkspaceAgent({ root, id: 'page-editor', title: 'Page editor',
        ...
        actions: [
            ...
            { action: 'page.setLayout', label: 'Stage page layout', acceptsDraft: true,
                ...
                run: async (p) => {
                    if (!contentController || !['main', 'composed', 'inherit', 'design', 'none'].includes(String(p.mode)))
                        throw new Error('PAGE_LAYOUT_MODE_INVALID');
                    await contentController.setLayout(p.mode, typeof p.designId === 'string' ? p.designId : undefined);
                } },
            { action: 'page.setContent', label: 'Stage page body HTML', acceptsDraft: true,
                params: [{ name: 'html', type: 'string', required: true }],
                run: async (p) => { if (!contentController || typeof p.html !== 'string')
                    throw new Error('PAGE_CONTENT_HTML_REQUIRED'); await contentController.setHtml(p.html); } },
            { action: 'page.setContentCss', label: 'Stage page content CSS', acceptsDraft: true,
                params: [{ name: 'css', type: 'string', required: true }],
                run: async (p) => { if (!contentController || typeof p.css !== 'string')
                    throw new Error('PAGE_CONTENT_CSS_REQUIRED'); await contentController.setCss(p.css); } },
            ...
            { action: 'page.attachContent', label: 'Stage content attachment', acceptsDraft: true, confirm: true,
                ...
                run: async (p) => {
                    if (!contentController || (p.kind !== 'design' && p.kind !== 'html'))
                        throw new Error('PAGE_CONTENT_KIND_INVALID');
                    contentBusy = true;
                    updateState();
                    try {
                        await contentController.attach(p.kind, agentString(p, 'id'));
                    }
                    finally {
                        contentBusy = false;
                        updateState();
            ...
            { action: 'page.detachContent', label: 'Stage content removal', acceptsDraft: true, confirm: true,
                run: async () => { if (!contentController)
                    throw new Error('PAGE_CONTENT_UNAVAILABLE'); await contentController.detach(); } },
