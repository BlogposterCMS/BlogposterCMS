...
export async function render(el) {
    ...
    function createDesignCard(template, isSelected = false) {
        ...
        if (isSelected) {
            ...
            remove.addEventListener('click', async (e) => {
                e.stopPropagation();
                if (!confirm('Remove attached design?'))
                    return;
    ...
    function createHtmlCard(name, html, isSelected = false) {
        ...
        if (isSelected) {
            ...
            remove.addEventListener('click', async (e) => {
                e.stopPropagation();
                if (!confirm('Remove uploaded HTML?'))
                    return;
    ...
    async function renderGallery() {
        gallery.innerHTML = '';
        const templates = (await loadDesigns()).filter(template => template.id !== page.meta?.designId);
        const htmlFiles = (await loadHtmlFiles()).filter(file => file !== page.meta?.htmlFileName);
