...
export async function render(el: HTMLElement | null): Promise<void> {
  ...
  let templateNames: TemplateName[] = [];
  ...
  let templates: TemplateView[] = buildTemplateViews(templateNames, pages);

  let currentFilter: 'all' | 'used' | 'unused' = 'all';
  ...
  (['all', 'used', 'unused'] as const).forEach((filter, idx) => {
