...
function normalizeChildSizes(parent, forceAll = false) {
  if (!parent) return;
  const kids = Array.from(parent.children).filter(
    ch => ch.classList && ch.classList.contains('layout-container')
  );
  if (!kids.length) return;
  const pct = 100 / kids.length;
  kids.forEach(ch => {
    if (forceAll || !ch.style.flexBasis) {
...
export function splitContainer(container, orientation) {
  ...
  if (hasWorkspace && container.id !== 'layoutRoot') {
    ...
    if (parent) {
      ...
      normalizeChildSizes(parent, true);
      cleanupChooser();
