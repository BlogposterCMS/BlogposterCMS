...
export async function initBuilder(sidebarEl, contentEl, pageId = null, startLayer = 1, layoutNameParam = null) {
  ...
  async function insertNativeElement(type, position = null) {
    ...
    if (nativeType === 'text') return insertQuickText(position);
    if (nativeType === 'media') return insertQuickMedia(position);
    if (nativeType === 'shape') return insertQuickShape(position);
    if (nativeType === 'button') return insertQuickButton(position);
    return null;
  }
