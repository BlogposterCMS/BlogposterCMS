...
module.exports = {
  ...
  async healthCheck({ motherEmitter, jwt }) {
    const rows = await navigationDbSelect(motherEmitter, jwt, 'LIST_NAVIGATION_LOCATIONS', { limit: 1, offset: 0 });
    if (!Array.isArray(rows)) throw new Error('CORE_MODULE_NAVIGATION_NOT_READY');
  },
  async initialize({ motherEmitter, isCore, jwt, nonce, isModuleUpdate = false }) {
    ...
    if (!isModuleUpdate) await seedDefaultNavigationLocations(motherEmitter, jwt);
