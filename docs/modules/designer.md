...
- The builder embeds `#workspaceMain` inside a persistent `#layoutRoot`. Splitting the root appends a sibling builder grid that combines the `layout-container`, `builder-grid` and `canvas-grid` classes on a single element and receives a unique id. Only `#workspaceMain` loads design widgets; additional root containers start empty.
