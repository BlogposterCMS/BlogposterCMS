...
export function createImageField(label: string, input: HTMLInputElement, options: {
  hint: string;
  ...
  choose.className = 'button secondary sm'; choose.textContent = 'Choose from file manager';
  choose.setAttribute('aria-label', `Choose from file manager: ${label}`);
  const remove = document.createElement('button'); remove.type = 'button';
  remove.className = 'button ghost sm'; remove.textContent = 'Remove image';
  remove.setAttribute('aria-label', `Remove ${label.toLowerCase()}`);
  const preview = document.createElement('img'); preview.alt = `${label} preview`;
  preview.style.cssText = 'max-width:100%;width:240px;max-height:160px;object-fit:contain;';
  const status = document.createElement('p'); status.className = 'form-field__hint';
  ...
  preview.addEventListener('error', () => {
    preview.hidden = true; status.textContent = 'IMAGE_PREVIEW_UNAVAILABLE: The image could not be loaded.';
  });
  ...
  choose.addEventListener('click', async () => {
    ...
    try {
      if (!options.emit) throw new Error('Media explorer unavailable');
      const picked = await options.emit('openMediaExplorer', { jwt: options.jwt, publicUrlOnly: true }) as { cancelled?: boolean; shareURL?: string } | null;
  ...
  actions.append(choose, remove); root.insertBefore(actions, input);
  input.placeholder = 'Or enter a public image URL';
  root.append(preview, status); refresh();
  return { root, refresh };
}
