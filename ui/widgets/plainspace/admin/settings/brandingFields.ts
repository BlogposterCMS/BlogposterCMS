...
export function createBrandingFields(values: { logoUrl: string; logoDarkUrl: string; faviconUrl: string },
  pick: () => Promise<string | null | undefined>, reportError: (message: string) => void) {
  const root = document.createElement('div'); root.className = 'branding-fields';
  const fields: Record<string, HTMLInputElement> = {};
  const redraw: Array<() => void> = [];
  for (const [key, label] of [['logoUrl', 'Logo (light / default)'], ['logoDarkUrl', 'Logo (dark)'], ['faviconUrl', 'Favicon']] as const) {
    const section = document.createElement('section'); section.className = `branding-image branding-image--${key}`;
    const title = document.createElement('h4'); title.textContent = label;
    const preview = document.createElement('div'); preview.className = 'branding-image__preview';
    const image = document.createElement('img'); image.alt = `${label} preview`;
    const placeholder = document.createElement('span');
    preview.append(image, placeholder);
    const input = document.createElement('input'); input.type = 'text'; input.value = values[key] || '';
    input.setAttribute('aria-label', `${label} URL`); fields[key] = input;
    const urlArea = document.createElement('details'); urlArea.className = 'branding-image__url';
    const urlToggle = document.createElement('summary'); urlToggle.textContent = 'Add via URL';
    urlArea.append(urlToggle, input);
    const actions = document.createElement('div'); actions.className = 'branding-image__actions';
    ...
    const update = () => {
      ...
      image.hidden = !safe;
      if (safe) image.src = source; else image.removeAttribute('src');
    ...
    choose.addEventListener('click', async () => {
      ...
      try { const url = await pick(); if (url) { input.value = url; input.dispatchEvent(new Event('input', { bubbles: true })); } }
      catch (error) { reportError(`SETTINGS_BRANDING_MEDIA_FAILED: ${error instanceof Error ? error.message : 'Unable to open media picker'}`); }
    ...
    section.append(title, preview, note, actions, urlArea); root.append(section);
  }
