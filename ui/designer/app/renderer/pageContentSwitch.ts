...
let currentDesignId: (() => string) | undefined;
const context = () => {
...
export async function openDocumentMode() {
  ...
  if (!pageId || !designId) throw new Error('PAGE_DOCUMENT_CONTEXT_REQUIRED: Open Design Studio from a page first.');
  const snapshot = window.blogposterDesignerCommands?.snapshot?.() as { save?: { dirty?: boolean; busy?: boolean }; publishing?: { busy?: boolean } } | undefined;
  if (snapshot?.save?.dirty || snapshot?.save?.busy || snapshot?.publishing?.busy) throw new Error('PAGE_DESIGN_SAVE_REQUIRED: Save the design before switching editors.');
  ...
  if (!htmlEditor) await loadArticleEditor();
  const own = ownPagePresentation(page);
  if (own) page = await useDocumentEditor(pageId, designId, language);
  const options = { language, openDesign: (value: typeof page) => reuseContentDesign(value, designId) };
...
export function mountPageContentSwitch(header: HTMLElement, getDesignId: () => string) {
  currentDesignId = getDesignId;
  if (!context().pageId) return;
  const button = document.createElement('button'); button.type = 'button'; button.className = 'button header-icon-btn';
  button.title = 'Use document editor'; button.setAttribute('aria-label', button.title);
  button.innerHTML = '<img src="/assets/icons/file-text.svg" width="18" height="18" alt="">';
  button.addEventListener('click', () => {
    void (async () => {
      if (await bpDialog.confirm(DOCUMENT_MODE_WARNING, { title: 'Switch to document editor?', confirmLabel: 'Use document editor' })) await openDocumentMode();
