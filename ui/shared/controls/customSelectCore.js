...
function createOptionButton(select, state, option, index) {
    ...
    button.addEventListener('click', () => {
        ...
        if (select.multiple)
            state.optionList.querySelector(`[data-option-index="${index}"]`)?.focus();
        else {
            state.close();
            state.display.focus();
        }
...
function handleListKeydown(event, select, state) {
    ...
    if (event.key === 'Enter' || event.key === ' ') {
        ...
        emitNativeChange(select);
        if (select.multiple)
            state.optionList.querySelector(`[data-option-index="${optionIndex}"]`)?.focus();
        else {
            state.close();
            state.display.focus();
        }
...
function enhanceSelect(select) {
    ...
    const positionOptions = () => {
        if (!floating || !wrapper.classList.contains('open'))
            return;
    ...
    const state = {
        ...
        open: () => {
            ...
            if (floating && !optionList.matches(':popover-open'))
                optionList.showPopover();
            positionOptions();
            if (floating) {
...
function bindDocumentBehavior() {
    ...
    document.addEventListener('click', event => {
        ...
        const clickedInsideSelect = event.composedPath().some(node => (node instanceof Element && node.closest('.custom-select') !== null));
...
function bindRootObserver(root) {
    ...
    const target = observationRoot instanceof Document
        ...
        : observationRoot;
    if (!(target instanceof Node) || rootObservers.has(target))
        return;
...
export function destroyCustomSelects(root) {
    root.querySelectorAll('select').forEach(select => enhancedSelects.get(select)?.destroy());
