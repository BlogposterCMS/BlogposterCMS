...
(async () => {
  try {
    ...
    let lastAppliedColumnUnit = Math.max(
      1,
      grid?.options?.columnWidth || 0
    );
    let resizeRaf = 0;
    ...
    for (const entry of combinedAdmin) {
      ...
      const meta = { ...entry };
      const instanceId =
        meta.id || meta.instance_id || meta.instanceId || `w${Math.random().toString(36).slice(2, 8)}`;
      meta.id = instanceId;
      const x =
      ...
      if (meta.xPercent != null) wrapper.dataset.xPercent = String(meta.xPercent);
      if (meta.yPercent != null) wrapper.dataset.yPercent = String(meta.yPercent);
      if (meta.wPercent != null) wrapper.dataset.wPercent = String(meta.wPercent);
      if (meta.hPercent != null) wrapper.dataset.hPercent = String(meta.hPercent);
      if (meta.layer != null) {
