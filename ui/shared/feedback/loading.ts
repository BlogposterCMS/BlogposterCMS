export type BpLoaderVariant = 'inline' | 'spinner' | 'skeleton' | 'overlay';

export interface BpLoaderOptions {
  variant?: BpLoaderVariant;
  label?: string;
  lines?: number;
}

export interface BpProgressHandle {
  element: HTMLElement;
  update: (value: number) => void;
}

export function createLoader(options: BpLoaderOptions = {}): HTMLElement {
...
export function setButtonLoading(button: HTMLButtonElement, loading: boolean, label = 'Loading'): void {
  if (loading) {
    if (button.dataset.bpLoading === 'true') return;
...
export function createProgress(labelText: string, initialValue = 0): BpProgressHandle {
