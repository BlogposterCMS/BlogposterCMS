...
export function localUiMedia(value) {
...
export function elementTag(node) {
    if (['dialog', 'drawer'].includes(node.tag))
        return 'dialog';
    if (node.tag === 'accordion')
        return 'details';
    if (['composer', 'richtext', 'popover', 'tooltip', 'toast', 'tabs', 'skeleton', 'kit'].includes(node.tag))
        return 'div';
    return node.tag;
}
/** Native controls retain their DOM identity during composition, selection and undo. */
export function updateControl(element, node, composing, preview) {
    const props = node.props || {};
    element.hidden = node.hidden === true;
    if (node.label !== undefined)
        element.setAttribute('aria-label', node.label.slice(0, 200));
    else
        element.removeAttribute('aria-label');
    if (node.name)
        element.setAttribute('name', node.name.slice(0, 80));
    if (props.tabIndex === -1 || props.tabIndex === 0)
        element.tabIndex = props.tabIndex;
    if (node.tag === 'button')
        element.type = 'button';
    if (node.tag === 'input') {
        const type = node.type || 'text';
        if (!['text', 'search', 'email', 'number', 'checkbox', 'radio', 'date', 'time', 'range', 'color', 'tel', 'url'].includes(type))
            throw uiError('WIDGET_VIEW_INPUT_INVALID');
        element.type = type;
    }
    if (['input', 'textarea', 'select', 'option'].includes(node.tag)) {
        const input = element;
        if (node.value !== undefined && input.value !== node.value && !composing)
            input.value = node.value;
        input.disabled = props.disabled === true;
        if ('checked' in props)
            input.checked = props.checked === true;
        if (typeof props.placeholder === 'string')
            input.setAttribute('placeholder', props.placeholder.slice(0, 200));
    }
    if (element instanceof HTMLButtonElement)
        element.disabled = props.disabled === true;
    if (node.tag === 'composer') {
        ...
        if (node.value !== undefined && node.value !== text && !composing)
    ...
    if (node.tag === 'img') {
        ...
        element.setAttribute('alt', String(props.alt || ''));
        ...
        if (renderedArticles.get(element) !== html) {
            ...
            template.content.querySelectorAll('*').forEach(item => {
                for (const attribute of [...item.attributes]) {
                    if (!['href', 'src', 'alt', 'title', 'colspan', 'rowspan'].includes(attribute.name))
                        item.removeAttribute(attribute.name);
                }
                if (item.hasAttribute('href')) {
                    const href = normalizeLinkUrl(item.getAttribute('href'));
                ...
                if (item.hasAttribute('src')) {
                    const src = localUiMedia(item.getAttribute('src'));
                    if (src)
            ...
            renderedArticles.set(element, html);
        }
    }
    if (node.tag === 'progress') {
        element.max = 100;
        if (typeof props.progress === 'number')
            element.value = Math.max(0, Math.min(100, props.progress));
    }
    if (node.tag === 'skeleton') {
        element.dataset.uiSkeleton = 'true';
    ...
    if (node.tag === 'drawer')
        element.dataset.uiDrawer = 'true';
    for (const [key, attribute] of [['expanded', 'aria-expanded'], ['selected', 'aria-selected'], ['busy', 'aria-busy']]) {
        if (typeof props[key] === 'boolean')
            element.setAttribute(attribute, String(props[key]));
    }
    if (typeof props.role === 'string' && ['listbox', 'option', 'status', 'alert', 'tablist', 'tab', 'tabpanel', 'menu', 'menuitem'].includes(props.role))
