...
export type SceneRangeHandle = 'start' | 'end';

export const MIN_SCENE_RANGE_GAP = 1;

export function clampScenePercent(value: unknown, fallback = 0): number {
...
export function normalizeSceneRange(startValue: unknown, endValue: unknown): SceneRange {
  ...
  if (start === end) {
    ...
    if (end === start) start = Math.max(0, end - MIN_SCENE_RANGE_GAP);
  }
  return { start, end };
}

export function rangeFromPointer(
  clientX: number,
  rect: Pick<DOMRect, 'left' | 'width'>,
  handle: SceneRangeHandle,
  currentRange: SceneRange
): SceneRange {
