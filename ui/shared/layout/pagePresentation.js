...
export function mainDesignId(value) {
    const id = scalar(value);
    if (id && !/^[A-Za-z0-9_.:-]+$/.test(id))
        throw new Error('PAGE_MAIN_DESIGN_INVALID: Choose a saved design.');
    return id;
}
export function presentationMeta(page) {
    let value = page.meta;
...
export function inheritsPageLayout(page) {
...
export function ownPagePresentation(page, depth = 0) {
    const meta = presentationMeta(page);
    ...
    if (designId)
        return { ...source, designId, layoutRef: validRef ? explicit : `layout:${designId}@v1` };
...
export async function resolvePagePresentation(page, loadParent, options = {}) {
    const mode = pageLayoutMode(page);
    if (mode !== 'inherit')
        return directPresentation(page, mode, options.mainDesignId);
    let current = page;
    const seen = new Set();
    ...
    for (let depth = 0; current && depth <= maxDepth; depth += 1) {
        ...
        if (id && seen.has(id))
            throw new Error('PAGE_LAYOUT_CYCLE: The page hierarchy contains a cycle.');
        if (id)
            seen.add(id);
        if (depth && ((current.lane || 'public') !== (page.lane || 'public')
            || current.status === 'deleted' || current.deleted_at || current.deletedAt
            || (options.publicOnly && current.status !== 'published')))
            return null;
        const own = ownPagePresentation(current, depth);
        if (own)
            return own;
        if (!inheritsPageLayout(current))
            return null;
        const parentId = scalar(current.parent_id ?? current.parentId);
        if (!parentId)
            return directPresentation(page, 'main', options.mainDesignId);
        if (depth === maxDepth)
            throw new Error('PAGE_LAYOUT_DEPTH: The page hierarchy exceeds the supported depth.');
        current = await loadParent(parentId);
    }
    return null;
}
/** Synchronous counterpart for the already-loaded Page Manager list. */
export function pagePresentationFromList(page, pages, siteDesignId) {
    const mode = pageLayoutMode(page);
    if (mode !== 'inherit')
        return directPresentation(page, mode, siteDesignId);
    ...
    for (let depth = 0; current && depth <= MAX_PAGE_LAYOUT_DEPTH; depth += 1) {
        ...
        const parentId = scalar(current.parent_id ?? current.parentId);
        if (!parentId)
            return directPresentation(page, 'main', siteDesignId);
        current = byId.get(parentId);
    }
    return null;
}
/** Validate the explicit composition contract at the Pages write boundary. */
export function validatePageDesignMode(page) {
    const meta = presentationMeta(page);
    if (meta.pageDesignMode === undefined)
        return;
    const mode = String(meta.pageDesignMode);
    if (!['main', 'composed', 'design', 'inherit', 'none'].includes(mode))
        throw new Error('PAGE_LAYOUT_MODE_INVALID: Choose an available page design mode.');
...
function directPresentation(page, mode, siteDesignId) {
    ...
    if (!id || (page.lane && page.lane !== 'public'))
        return mode === 'composed' ? own : null;
    return {
...
export function pageLayoutMode(page) {
    const meta = presentationMeta(page);
    if (['main', 'composed', 'design', 'inherit', 'none'].includes(String(meta.pageDesignMode)))
        return meta.pageDesignMode;
    if (ownPagePresentation(page))
        return 'design';
    if (!inheritsPageLayout(page))
        return 'none';
...
export function pageLayoutMeta(page, mode, designId, title) {
    if (!['main', 'composed', 'inherit', 'design', 'none'].includes(mode))
        throw new Error('PAGE_LAYOUT_MODE_INVALID: Choose an available layout mode.');
    ...
    for (const key of ['designId', 'design_id', 'designTitle', 'designThumbnail', 'design_layout', 'designLayout',
        'layoutTemplate', 'layout_template', 'inheritPresentation', 'inheritDesign'])
        delete meta[key];
