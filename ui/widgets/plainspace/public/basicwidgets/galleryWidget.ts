...
type GalleryMode = 'grid' | 'masonry' | 'carousel';
type GalleryHeightMode = 'ratio' | 'natural' | 'smallest' | 'largest';
type GalleryAnimation = 'slide' | 'fade' | 'instant';

const OBJECT_FITS = ['cover', 'contain', 'fill', 'none', 'scale-down'];
const GALLERY_MODES: GalleryMode[] = ['grid', 'masonry', 'carousel'];
const HEIGHT_MODES: GalleryHeightMode[] = ['ratio', 'natural', 'smallest', 'largest'];
const ANIMATIONS: GalleryAnimation[] = ['slide', 'fade', 'instant'];

function galleryStyle(): HTMLStyleElement {
...
function normalizeMode(value: unknown): GalleryMode {
  const normalized = String(value || '').trim().toLowerCase();
  if (['slider', 'slideshow'].includes(normalized)) return 'carousel';
  return normalizeChoice(normalized, GALLERY_MODES, 'grid');
}

function normalizeHeightMode(value: unknown): GalleryHeightMode {
  const normalized = String(value || '').trim().toLowerCase();
  if (['shortest', 'min', 'minimum'].includes(normalized)) return 'smallest';
  if (['tallest', 'max', 'maximum'].includes(normalized)) return 'largest';
  return normalizeChoice(normalized, HEIGHT_MODES, 'ratio');
}

function normalizeAnimation(value: unknown): GalleryAnimation {
  const normalized = String(value || '').trim().toLowerCase();
  if (normalized === 'none') return 'instant';
  return normalizeChoice(normalized, ANIMATIONS, 'slide');
}

function normalizeFit(value: unknown, fallback = 'cover'): string {
  const normalized = String(value || '').trim().toLowerCase();
...
function normalizeObjectPosition(item: MediaItem, fallbackX: number, fallbackY: number, fallback = ''): string {
  const direct = readString(item as unknown as Record<string, unknown>, ['objectPosition', 'position']);
...
function setRowLimit(root: HTMLElement, list: HTMLElement, rows: number): void {
  if (!rows) return;
  const firstFrame = list.querySelector<HTMLElement>('.bp-gallery-widget__frame');
...
function setMeasuredHeight(root: HTMLElement, mode: GalleryHeightMode): void {
  if (!['smallest', 'largest'].includes(mode)) return;
  const images = Array.from(root.querySelectorAll<HTMLImageElement>('img'));
  const measure = () => {
    const heights = images
      .map(image => {
        if (!image.naturalWidth || !image.naturalHeight) return 0;
        const frame = image.closest<HTMLElement>('.bp-gallery-widget__frame');
...
function renderImage(
  item: MediaItem,
  defaults: { fit: string; focalX: number; focalY: number; objectPosition: string }
): HTMLElement {
  ...
  if (item.href) (frame as HTMLAnchorElement).href = item.href;
...
function appendCarouselControls(
  root: HTMLElement,
  list: HTMLElement,
  itemCount: number,
  options: {
  ...
  const items = Array.from(list.querySelectorAll<HTMLElement>('.bp-gallery-widget__item'));
  ...
  if (options.showDots && dotCount > 1) root.appendChild(dots);
...
export function render(el: HTMLElement | null, ctx: PublicWidgetContext = {}): void {
  ...
  if (mode === 'carousel') root.dataset.animation = animation;
