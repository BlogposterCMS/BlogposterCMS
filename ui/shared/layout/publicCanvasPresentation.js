...
export function boundedPercent(value, fallback, max = 100) {
    const num = typeof value === 'number' ? value : Number(value);
    return Number.isFinite(num) ? Math.min(max, Math.max(0, num)) : fallback;
}
export function publicPlacement(item) {
    return {
...
export function publicItemStyle(item) {
    ...
    if (Number.isFinite(Number(item.zIndex)))
        style['z-index'] = String(Number(item.zIndex));
    if (Number.isFinite(Number(item.opacity)))
        style.opacity = String(Number(item.opacity));
    const rotation = Number(item.rotationDeg);
    if (Number.isFinite(rotation) && rotation !== 0)
        style.transform = `rotate(${rotation}deg)`;
    return style;
}
export function publicCanvasStyle(layout) {
