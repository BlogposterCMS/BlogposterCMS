...
or bundle any site's content. Branch CI builds the non-deployable `build` stage
and checks native module loading as a non-root user. Tag release CI supplies
signed baseline artifacts, builds the final image, and verifies integrity plus
native modules with no network before publishing the release. Neither deploys it.
...
- The final image requires `.release-integrity` manifest, attestation bundle,
  and public trust roots generated by release CI. A plain source-only registry
  build cannot replace this signed release flow. Trust roots remain root-owned
  under `/app/.integrity`; the CMS never needs network access to refresh them.
