...
### Added
* Grid mode introduced for the admin dashboard. Widgets snap to unique cells and a resize arrow replaces the bounding box.
* Builder grids continue using bounding boxes and allow overlaps as before.
...
- Admin dashboard grid enables `pushOnOverlap` so widgets automatically move aside in grid mode.
