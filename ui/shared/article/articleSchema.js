...
export function safeArticleUrl(value, link = false) {
...
const Video = Node.create({
    ...
    parseHTML: () => [{ tag: 'video[src]', getAttrs: element => safeArticleUrl(element.getAttribute('src')) ? null : false }],
...
export function validateArticleDocument(value) {
    ...
    function walk(node, depth) {
        if (++count > 20000 || depth > 40 || !node || !allowed.has(node.type))
            throw new Error('ARTICLE_DOCUMENT_INVALID');
        if ((node.content != null && !Array.isArray(node.content)) || (node.marks != null && !Array.isArray(node.marks)))
            throw new Error('ARTICLE_DOCUMENT_INVALID');
        if (['image', 'video'].includes(node.type) && !safeArticleUrl(node.attrs?.src))
            throw new Error('ARTICLE_MEDIA_URL_INVALID');
        for (const mark of node.marks || []) {
            if (!marks.has(mark.type))
                throw new Error('ARTICLE_MARK_INVALID');
            if (mark.type === 'link' && !safeArticleUrl(mark.attrs?.href, true))
                throw new Error('ARTICLE_LINK_INVALID');
        }
    ...
    if (!value || value.type !== 'doc')
        throw new Error('ARTICLE_DOCUMENT_INVALID');
    if (JSON.stringify(value).length > 2000000)
    ...
    template.content.querySelectorAll('*').forEach(el => {
        for (const attr of [...el.attributes]) {
            if (/^on/i.test(attr.name) || ['style', 'srcset'].includes(attr.name) ||
                (['src', 'href', 'poster'].includes(attr.name) && !safeArticleUrl(attr.value, attr.name === 'href')))
