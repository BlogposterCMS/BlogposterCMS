...
function isEscapeHatch(select: HTMLSelectElement): boolean {
  return select.dataset.nativeSelect === 'true'
    ...
    || (select.multiple && select.dataset.enhance !== 'dropdown')
    || select.size > 1;
}
...
function createOptionButton(
  ...
  button.addEventListener('click', () => {
    ...
    if (select.multiple) state.optionList.querySelector<HTMLButtonElement>(`[data-option-index="${index}"]`)?.focus();
    else { state.close(); state.display.focus(); }
  });
...
function handleDisplayKeydown(event: KeyboardEvent, select: HTMLSelectElement, state: CustomSelectState): void {
  ...
  if (event.key === 'Escape') {
    if (state.wrapper.classList.contains('open')) event.stopPropagation();
    state.close();
  }
}
...
function handleListKeydown(event: KeyboardEvent, select: HTMLSelectElement, state: CustomSelectState): void {
  ...
  if (event.key === 'Enter' || event.key === ' ') {
    ...
    state.refresh();
    emitNativeChange(select);
    if (select.multiple) state.optionList.querySelector<HTMLButtonElement>(`[data-option-index="${optionIndex}"]`)?.focus();
    else { state.close(); state.display.focus(); }
    return;
  }
...
function enhanceSelect(select: HTMLSelectElement): void {
  ...
  if (select.multiple) optionList.setAttribute('aria-multiselectable', 'true');
  ...
  if (floating) optionList.setAttribute('popover', 'manual');
  const positionOptions = (): void => {
    if (!floating || !wrapper.classList.contains('open')) return;
    ...
    Object.assign(optionList.style, {
      position: 'fixed', margin: '0', right: 'auto', bottom: 'auto', minWidth: '0',
      width: `${Math.min(bounds.width, window.innerWidth - 24)}px`,
      maxHeight: `${Math.max(40, Math.min(280, useAbove ? above : below))}px`,
      left: `${Math.max(12, Math.min(bounds.left, window.innerWidth - bounds.width - 12))}px`,
      top: `${useAbove ? Math.max(12, bounds.top - optionList.offsetHeight - 8) : bounds.bottom + 8}px`
    });
  };
  ...
  const state: CustomSelectState = {
    ...
    open: () => {
      ...
      if (floating && !optionList.matches(':popover-open')) optionList.showPopover();
      positionOptions();
      if (floating) {
    ...
    close: () => {
      ...
      if (floating && optionList.matches(':popover-open')) optionList.hidePopover();
    ...
    refresh: () => {
      ...
      valueNode.textContent = select.multiple
        ? Array.from(select.selectedOptions).map(getOptionLabel).join(', ') || select.dataset.placeholder || 'Choose options'
    ...
    destroy: () => {
      state.close();
      select.removeEventListener('change', state.refresh);
  ...
  const observer = new MutationObserver(records => {
    records.forEach(record => {
      ...
      record.addedNodes.forEach(node => {
        ...
        if (node instanceof Element && (node.matches('dialog, [role="dialog"]') || node.querySelector('dialog, [role="dialog"]'))) closeAll();
        if (node instanceof HTMLSelectElement) {
          enhanceSelect(node);
          return;
        }
        if (node instanceof Element || node instanceof DocumentFragment) {
          // The parent observer already owns this subtree; avoid retaining a
          // second observer for every transient option/tooltip node.
...
/** Widgets own their scoped observer and dispose it when unmounted. */
export function destroyCustomSelects(root: HTMLElement | ShadowRoot): void {
  root.querySelectorAll<HTMLSelectElement>('select').forEach(select => enhancedSelects.get(select)?.destroy());
