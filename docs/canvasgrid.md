...
- Header viewport control lets you adjust the builder viewport width via a slider (default 1920px, up to 3840px) so the layout root and workspace scale for responsive previews.
...
- Zoom sizer bases its height on the larger of the viewport and grid so empty canvases don't collapse during zoom.
