...
export function articleCommandAvailable(editor: Editor, label: string): boolean {
...
export function mountArticleToolbarState(editor: Editor, toolbar: HTMLElement, busy: () => boolean) {
  const refresh = () => {
    for (const button of toolbar.querySelectorAll<HTMLButtonElement>('button[aria-label]')) button.disabled = busy() || !articleCommandAvailable(editor, button.getAttribute('aria-label') || '');
    const block = toolbar.querySelector<HTMLSelectElement>('select');
    if (block) {
      for (const option of block.options) option.disabled = busy() || !(option.value === 'p' ? editor.can().setParagraph() : editor.can().toggleHeading({ level: Number(option.value) as 1 | 2 | 3 }));
      block.disabled = busy() || [...block.options].every(option => option.disabled);
    }
  };
  editor.on('transaction', refresh); refresh();
  return { refresh, read: () => [...toolbar.querySelectorAll<HTMLButtonElement>('button[aria-label]')].map(button => ({ label: button.getAttribute('aria-label'), available: !button.disabled })) };
}
