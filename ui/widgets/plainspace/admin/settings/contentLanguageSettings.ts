...
export async function createContentLanguageSettings(emit: NonNullable<Window['meltdownEmit']>, jwt: string) {
  const values = await emitRuntimeAdmin<Record<string, unknown>>(emit, jwt, 'settings', 'public', { keys: [CONTENT_LANGUAGES_SETTING, 'DEFAULT_LANGUAGE'] });
  ...
  const primary = document.createElement('select'); primary.setAttribute('aria-label', 'Main content language');
  const fields: Record<string, HTMLInputElement | HTMLSelectElement> = { primaryLanguage: primary };
  ...
  const available = document.createElement('fieldset'); available.className = 'content-language-options';
  const legend = document.createElement('legend'); legend.textContent = 'Available content languages'; available.append(legend);
  for (const code of choices) {
    primary.add(new Option(contentLanguageLabel(code), code));
    const label = document.createElement('label'); label.className = 'form-choice';
    const checkbox = document.createElement('input'); checkbox.type = 'checkbox'; checkbox.checked = config.languages.includes(code); fields[code] = checkbox;
    label.append(checkbox, document.createTextNode(contentLanguageLabel(code))); available.append(label);
  }
  primary.value = config.primaryLanguage;
  const sync = () => { const selected = fields[primary.value] as HTMLInputElement; selected.checked = true; for (const field of Object.values(fields)) if (field instanceof HTMLInputElement) field.disabled = field === selected; };
  primary.addEventListener('change', sync); sync();
