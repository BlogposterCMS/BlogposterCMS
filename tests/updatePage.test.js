...
async function testUpdatePage() {
  ...
  assert.strictEqual(Object.prototype.hasOwnProperty.call(captured.data.params, 'title'), false);
  assert.strictEqual(Object.prototype.hasOwnProperty.call(captured.data.params, 'meta'), false);
  assert.strictEqual(Object.prototype.hasOwnProperty.call(captured.data.params, 'lane'), false);
  assert.strictEqual(Object.prototype.hasOwnProperty.call(captured.data.params, 'language'), false);

  captured = null;
  await new Promise((resolve, reject) => {
    emitter.emit(
      'updatePage',
      { jwt: 't', moduleName: 'pagesManager', moduleType: 'core', pageId: 3, parent_id: null },
      (err, res) => {
        if (err) reject(err); else resolve(res);
      }
    );
  ...
  assert.strictEqual(Object.prototype.hasOwnProperty.call(captured.data.params, 'parent_id'), true);
  assert.strictEqual(captured.data.params.parent_id, null);
}
