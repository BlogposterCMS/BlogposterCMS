...
async function handleGetDesignPlaceholder({ dbClient, params }) {
  ...
  if (dbType === "postgres") {
    const metaRes = await dbClient.query(
      `SELECT d.id, d.title, d.description, d.thumbnail, d.bg_color, d.bg_media_id, d.bg_media_url, d.created_at, d.updated_at, d.published_at, d.owner_id, d.version, d.is_draft, d.layout_id, l.layout_json FROM designer.designer_designs d LEFT JOIN designer.designer_layouts l ON d.layout_id = l.id WHERE d.id=$1 AND d.deleted_at IS NULL`,
      [designId]
    );
    ...
    return {
      design: { ...meta, layout, layout_json: layout },
      widgets: widgetsRes.rows,
      layout,
    };
  } else if (dbType === "sqlite") {
    const meta = await dbClient.get(
      `SELECT d.id, d.title, d.description, d.thumbnail, d.bg_color, d.bg_media_id, d.bg_media_url, d.created_at, d.updated_at, d.published_at, d.owner_id, d.version, d.is_draft, d.layout_id, l.layout_json FROM designer_designs d LEFT JOIN designer_layouts l ON d.layout_id = l.id WHERE d.id=? AND d.deleted_at IS NULL`,
      [designId]
    );
    if (!meta) return null;
    const layout = parseStoredLayout(meta.layout_json);
    ...
    let layout = null;
    if (design.layout_id) {
      const layouts = dbClient.collection("designer_layouts");
      const layoutId = ObjectId.isValid(design.layout_id)
        ? new ObjectId(design.layout_id)
        : design.layout_id;
      const layoutDoc = await layouts.findOne({ _id: layoutId });
      layout = parseStoredLayout(layoutDoc?.layout_json);
    }
    const widgets = await widgetsCol
      ...
      .toArray();
    return { design: { ...design, id: String(design._id), layout, layout_json: layout }, widgets, layout };
  } else {
