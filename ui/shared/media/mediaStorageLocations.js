import { listMediaCatalog, toFolderListing } from './mediaLibraryData.js';
/** Project catalog object keys into folders without creating another persistence owner. */
export function storageFolder(files, currentPath) {
    ...
    for (const file of files) {
        ...
        if (slash !== -1)
            ...
            details.push({ name: relative, size: file.size, modifiedAt: file.modifiedAt, url: file.url });
    }
...
export async function listMediaStorageLocations(request, emit, jwt) {
    const response = await request('/admin/api/media/storage', { credentials: 'same-origin' });
    if (!response.ok)
        throw new Error('MEDIA_EXPLORER_STORAGE_CONFIG_FAILED');
    const config = await response.json();
    if (config.error)
        throw new Error(config.error);
    const connections = Array.isArray(config.connections) ? config.connections : [];
    const locations = connections.filter(connection => connection.provider !== 'local').map(connection => ({
        ...
        list: async (path) => {
            ...
            if (!response.ok || listing.error)
                throw new Error(listing.error || 'MEDIA_EXPLORER_STORAGE_LIST_FAILED');
    ...
    for (const record of await listMediaCatalog(emit, jwt)) {
        ...
        if (provider === 'local' || rawKey.startsWith('public/'))
            continue;
        let url;
        try {
        ...
        if (connections.some(connection => connection.provider !== 'local' && (connection.connectionId === storage.connectionId ||
            (connection.publicBaseUrl && url.href.startsWith(`${connection.publicBaseUrl}/`)))))
            continue;
        const id = `external:${provider}:${storage.bucket || url.origin}`;
        if (!groups.has(id))
            groups.set(id, { label: `External files — ${storage.bucket || url.hostname}`, files: [] });
        const key = rawKey || String(record.fileName || record.file_name || url.pathname.split('/').pop() || '');
        if (!key || key.split('/').some(part => !part || part === '.' || part === '..' || /[\\\u0000]/.test(part)))
            continue;
    ...
    return [...locations, ...[...groups].map(([id, group]) => ({ id, label: group.label, list: async (path) => storageFolder(group.files, path) }))];
}
