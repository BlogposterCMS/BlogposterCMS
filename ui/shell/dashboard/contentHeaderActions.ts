...
export function initContentHeader(): void {
  ...
  const host = header.closest<HTMLElement>('#content-header') || header;
  ...
  if (mainContent && mainContent.parentElement && host !== mainContent && mainContent.contains(host)) mainContent.before(host);
  ...
  if (widgetsBtn) widgetsBtn.hidden = fixed;
  if (widgetsBtn) {
    widgetsBtn.onclick = () => {
      if (document.getElementById('content')?.dataset.dashboardLayout === 'fixed') return;
