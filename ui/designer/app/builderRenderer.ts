...
import {
  ...
  showBackgroundToolbar as showBgToolbar,
  hideBackgroundToolbar as hideBgToolbar
} from './editor/toolbar/backgroundToolbar.js';
import {
  initGrid,
  getCurrentLayoutForLayer as getWorkspaceLayoutForLayer,
...
} from './managers/layoutInteractionMode.js';
...
} from './widgets/styleSourceSync.js';
import {
  setDefaultWorkarea,
  ...
  linkContainerStyleSource as linkContainerStyleSourceNode,
  unlinkContainerStyleSource as unlinkContainerStyleSourceNode,
  duplicateContainer as duplicateContainerNode,
...
export async function initBuilder(sidebarEl, contentEl, pageId = null, startLayer = 1, layoutNameParam = null) {
  ...
  function getCurrentLayoutForLayer(targetGridEl, layer, currentCodeMap) {
    if (layoutGridRegistry) {
      return layoutGridRegistry.serializeLayer(
        getWorkspaceLayoutForLayer,
        layer,
        currentCodeMap
      );
    }
  ...
  const navigationInspector = createNavigationInspector(sceneInspector, {
    read: () => {
      ...
      if (!el || !['navigationMenu', 'breadcrumb'].includes(el.dataset.widgetId)) return null;
  ...
  const containerInteractionInspector = createContainerInteractionInspector(sceneInspector, {
    ...
    apply: interaction => {
      ...
      if (el && !state.activeWidgetEl) setContainerSettings(el, { interaction });
    }
  });
  ...
  function resetSidebarFlyoutPosition() {
    const flyout = sidebarEl.querySelector<HTMLElement>('[data-sidebar-flyout]');
    if (!flyout) return;
  ...
  function positionWidgetPresetPopover(anchorButton = null) {
    ...
    const anchor = anchorButton
      || sidebarEl.querySelector<HTMLElement>('[data-insert-group].active');
    if (!flyout || !anchor || !isSidebarFlyoutOpen()) return;
  ...
  function setInsertGroup(groupId, anchorButton = null) {
    ...
    positionWidgetPresetPopover(
      anchorButton || sidebarEl.querySelector(`[data-insert-group="${cssEscape(item.id)}"]`)
    );
    return item;
  }
  ...
  function setSidebarPanel(panelName = 'insert', options = {}) {
    ...
    if (activePanel !== 'insert') resetSidebarFlyoutPosition();
  ...
  async function initDesignBackgroundSettings() {
    ...
    if (!globalInput || !pageInput || !inheritInput || !layoutRoot) return;
    ...
    const persistedPageBackground = studioBackgroundColor(
      layoutRoot.dataset.layoutBackground || initialDesign.bg_color || '#ffffff',
      '#ffffff'
    );
  ...
  function syncInspectorSection(scene = getActiveScene()) {
    ...
    if (minWidth) minWidth.value = String(sectionEl?.dataset.layoutMinWidth || '0px');
    ...
    if (heightMode) heightMode.value = authoredHeight && authoredHeight !== 'auto' ? 'fixed' : 'auto';
    if (exactHeight) {
    ...
    if (contentHost) contentHost.checked = sectionEl?.dataset.dynamicHost === 'true';
    if (name) {
    ...
    if (groupTitle) groupTitle.textContent = nested ? 'Container' : 'Section';
    ...
    if (gap) gap.value = String(numberValue(sectionEl?.dataset?.layoutGap, 0));
    if (padding) padding.value = String(numberValue(sectionEl?.dataset?.layoutPadding, 0));
  ...
  function updateSectionFromInspector(target, persist = true) {
    const scene = getActiveScene();
    const sectionEl = inspectorLayoutContainer(layoutRoot, scene?.id);
    if (!scene || !sectionEl || !target) return;
    ...
    if (target.matches('.scene-container-min-width')) {
      ...
      if (key === 'borderWidth') {
        for (const side of ['Top', 'Right', 'Bottom', 'Left']) settings[`border${side}Width`] = value;
      }
      // A positive width should produce a visible border on an untouched box.
      if (key.endsWith('Width') && Number(target.value) > 0 && (!sectionEl.dataset.layoutBorderStyle || sectionEl.dataset.layoutBorderStyle === 'none')) settings.borderStyle = 'solid';
      ...
      if (nested) setContainerSettings(sectionEl, { background: target.value });
      else applySceneBackground(scene, target.value);
    } else if (target.matches('.scene-section-transparent')) {
      const background = target.checked ? 'transparent' : (
        sceneInspector.querySelector('.scene-section-background')?.value || '#ffffff'
      );
      if (nested) setContainerSettings(sectionEl, { background });
      else applySceneBackground(scene, background);
    }
  ...
  function updateSceneInspector(el = null, widgetDef = null) {
    ...
    if (sceneInspector.dataset.selectionKind !== kind) setInspectorMode('content');
    ...
    if (contentTab) contentTab.textContent = el ? 'Content' : 'Layout';
    ...
    if (el) {
      ...
      if (titleEl) titleEl.textContent = nested ? container.dataset.nodeId : activeScene.title;
      if (kickerEl) kickerEl.textContent = nested
  ...
  function ensureSceneSection(id, title, meta = {}) {
    ...
    if (existing) {
      ...
      if (meta.backgroundImageId) existing.backgroundImageId = String(meta.backgroundImageId);
      return;
    }
    ...
    if (meta.backgroundImageId) next.backgroundImageId = String(meta.backgroundImageId);
  ...
  function createSceneFromUi({ edit = true, insertionSource = 'storyboard' } = {}) {
    ...
    if (!revealInsertedSection(insertedSection, { source: insertionSource })) {
      warnDesignerContainerError(
        'DESIGNER_SECTION_INSERT_FEEDBACK_TARGET_MISSING',
        new Error('The inserted Section is missing from the canonical layout root.'),
        { sectionId: section.id, insertionSource }
      );
    }
    requestSceneChangePersist();
    return section;
  ...
  function updateSceneTitleReferences(sceneId, title) {
    ...
    if (scene) syncPageSection(layoutRoot, scene);
  ...
  function removeScene(sceneId) {
    ...
    if (!deletion.handled) return deletion;
    ...
    if (editingSceneId === deletion.removedSection.id) editingSceneId = null;
  ...
  function getSceneSectionsSnapshot() {
    return sceneSections
      ...
      .map(section => {
        ...
        if (backgroundImageUrl) snapshot.backgroundImageUrl = backgroundImageUrl;
    ...
    const renderSurface = (surface, depth = 0) => {
      orderedChildren(surface).forEach(widget => {
        ...
        const iconMarkup = isContainer
          ...
          : getWidgetIcon(
            widgetDef || { id: widget.dataset.widgetId || 'box', metadata: { icon: 'box' } },
            ICON_MAP
          );
        const metaMarkup = isContainer
          ? `<span class="scene-layer-behavior">${escapeHtml(
            widget.dataset.layoutMode === 'stack' || widget.dataset.layoutMode === 'row'
              ? 'Auto'
              : (widget.dataset.layoutMode || 'Free')
          )}</span>`
        ...
        if (isContainer) renderSurface(widget, depth + 1);
      });
    };
  ...
  sidebarEl.addEventListener('keydown', event => {
    if (event.key === 'Escape' && isSidebarFlyoutOpen()) {
      ...
      closeSidebarPanel();
      if (anchorId) {
        sidebarEl.querySelector<HTMLElement>(`[data-insert-group="${cssEscape(anchorId)}"]`)?.focus();
      }
      return;
    }
  ...
  gridViewportEl?.addEventListener('click', event => {
    ...
    if (deleteSectionButton) {
      ...
      if (owningSection?.dataset.sectionId) removeScene(owningSection.dataset.sectionId);
      return;
    }
    ...
    if (addSectionButton) {
      ...
      if (afterId && afterId !== activeSceneId) activeSceneId = afterId;
  ...
  function syncSectionWorkspaces({ scroll = false } = {}) {
    try {
      ...
      if (!layoutGridRegistry) {
        if (gridEl.parentElement !== workarea) workarea.appendChild(gridEl);
        return;
      }
      ...
      if (!activeRecord) {
        throw new Error(
          `DESIGNER_SECTION_GRID_ACTIVE_MISSING: no grid exists for Section "${activeSceneId}".`
        );
      }
  ...
  function syncSceneSectionsToLayout() {
    ...
    layoutRoot.querySelectorAll(':scope > .layout-section[data-section-id]').forEach(sectionEl => {
      if (!validIds.has(sectionEl.dataset.sectionId)) sectionEl.remove();
    });
  ...
  function linkContainerStyleSource(source, target) {
    try {
      ...
      if (!linked) {
        throw new Error(
          'DESIGNER_CONTAINER_STYLE_SOURCE_LINK_INVALID: source and target need distinct stable container ids.'
        );
      }
      handleContainerMutation();
    } catch (err) {
  ...
  async function duplicateContainer(el, { linked = false } = {}) {
    try {
      ...
      const widgetGroups = sourceSurfaces.map(surface => (
        directWidgetElements(surface).map(widgetDuplicateSnapshot)
      ));
      const clone = duplicateContainerNode(el, { linked, layoutRoot });
      if (!clone) {
        throw new Error(
          `DESIGNER_CONTAINER_DUPLICATE_FAILED: container "${el?.dataset?.nodeId || ''}" could not be copied.`
        );
      }
      ...
      for (let surfaceIndex = 0; surfaceIndex < widgetGroups.length; surfaceIndex += 1) {
        ...
        if (!targetRecord) {
          throw new Error(
            `DESIGNER_CONTAINER_DUPLICATE_GRID_MISSING: copied surface "${targetSurface?.dataset?.nodeId || ''}" has no grid record.`
          );
        }
  ...
  if (!initialSectionGrid) {
    throw new Error(
      'DESIGNER_SECTION_GRID_INITIALIZE_FAILED: the page has no canonical Section grid.'
    );
  }
  ...
  const {
    ...
    hide: hideActionBar,
    clearSelection: clearActionBarSelection
  ...
  gridViewportEl?.addEventListener('scroll', () => {
    ...
    if (now - actionBarSelectedAt < 350) return;
  ...
  bindEditingScope({ layoutRoot, onSelect: target => {
    if (target.classList.contains('canvas-item')) selectWidget(target);
    else {
      const section = target.closest('.layout-section[data-section-id]');
      if (section) activateSceneById(section.dataset.sectionId, false);
    }
  } });
  bindLayoutWidgetSelection({
    layoutRoot,
    ...
    onActivateLayer: async (layer, widget) => {
      ...
      if (current) selectWidget(current);
    },
  ...
  function normalizeSceneWidgetDimensions({ w, h, minW, minH }, targetGrid = grid, targetSurface = gridEl) {
    const canvasWidth = Math.max(
      1,
      Number(targetGrid?.options?.columns) || Number(targetSurface?.clientWidth) || 1040
    );
  ...
  function widgetDuplicateSnapshot(source) {
    ...
    return {
      ...
      datasets: Object.fromEntries(
        ['scrollStart', 'scrollEnd', 'effects', 'opacity', 'radius', 'responsivePlacement']
  ...
  async function duplicateSceneWidget(source, {
    linked = false,
    targetRecord = null,
    offset = 16,
    select = true,
    ...
    if (!snapshot.widgetDef) {
      throw new Error(
        `DESIGNER_WIDGET_DUPLICATE_DEFINITION_MISSING: no widget definition exists for "${source.dataset.widgetId || ''}".`
      );
    }
    ...
    if (snapshot.datasets.radius !== undefined) {
      ...
      if (content) content.style.borderRadius = `${snapshot.datasets.radius}px`;
    }
    if (linked) {
  ...
  async function createSceneWidget(widgetDef, {
    ...
    if (!widgetDef || !targetSurface || !targetGrid) return null;
    const dimensions = normalizeSceneWidgetDimensions(
      { w, h, minW, minH },
      targetGrid,
      targetSurface
    );
  ...
  async function updateContainerFromCommand(action, command = {}) {
    ...
    if (action === 'container.create') {
      ...
      if (!duplicate) return { handled: false, reason: 'container-duplicate-failed' };
  ...
  async function executeDesignerAgentCommand(command = {}) {
    ...
    if (action === 'navigation.configure') {
      const el = findCanvasItemByCommand(command);
      if (!el) throw new Error('DESIGNER_NAVIGATION_SELECTION_REQUIRED');
  ...
  function bindSectionGridEvents(record) {
    ...
    if (!workspace || !sectionGrid || workspace.dataset.editorEventsBound === 'true') return;
    workspace.dataset.editorEventsBound = 'true';

    sectionGrid.on('change', ({ el, contentOnly } = {}) => {
      if (applyingCompositeLayout) return;
      if (el) {
        selectWidget(el);
  ...
  layoutRoot.addEventListener('designerSectionBackgroundChanged', event => {
    ...
    const backgroundImageUrl = normalizeSceneBackgroundImageUrl(
      event.detail?.backgroundImageUrl
    );
    if (backgroundImageUrl) {
  ...
  function clearActiveWidgetSelection() {
    if (!clearActionBarSelection()) return false;
    removeStageBehaviorHuds();
    updateSceneInspector(null);
    renderSceneLayers();
    hideToolbar();
  ...
  const clearActiveWidgetFromExternalPointer = event => {
    ...
    if (!clearActiveWidgetSelection()) return;
  ...
  if (contentRoot && contentRoot !== layoutRoot) contentRoot.addEventListener('pointerdown', contentClickHandler);
  ...
  const captureBackgroundIntent = e => {
    ...
    if (e.target.closest('.scene-inspector')) return;
    ...
    if (!inContent) {
      ...
      if (hitWorkspace) inContent = true;
  ...
  document.addEventListener('click', e => {
    if (!layoutRoot?.querySelector('.layout-grid-surface')) return;
  ...
  const sectionWorkspaceForEvent = event => (
    event.target?.closest?.('.layout-grid-surface') || gridEl
  );
  ...
  function applyCompositeLayout(idx) {
    ...
    try {
      ...
      for (let i = 0; i <= idx; i++) {
        const { groups, unassigned } = layoutGridRegistry.partition(
          layoutLayers[i].layout,
          activeSceneId
        );
