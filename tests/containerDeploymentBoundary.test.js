...
test('release images require packaged trust roots and pass offline verification', () => {
  ...
  expect(read('Dockerfile')).toContain('FROM github-cli AS integrity-inputs');
  // Inherit the existing curl libraries and OS trust store without adding them to runtime.
  const inputStage = read('Dockerfile').split('FROM github-cli AS integrity-inputs')[1].split('FROM integrity-inputs AS verified-build')[0];
  expect(inputStage).toContain('COPY --from=build /app /app');
  expect(read('Dockerfile')).toContain('apt-get install --no-install-recommends -y ca-certificates curl');
  expect(read('.dockerignore')).toContain('!tools/fetch-build-integrity.js');
