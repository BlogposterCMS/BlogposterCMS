...
export async function createAnalyticsSettings(emit, jwt) {
    const defaults = { enabled: false, bannerEnabled: true, mode: 'opt-in', policyVersion: 1,
    ...
    for (const [key, label] of Object.entries({ enabled: 'Enable website analytics', bannerEnabled: 'Enable consent banner', analytics: 'Offer analytics category', recognition: 'Offer visitor, session and signed-in account linking', marketing: 'Offer marketing category', personalization: 'Offer personalization category', googleEnabled: 'Enable Google Analytics connector — UNTESTED' })) {
        ...
        fields[key] = input;
        root.append(createFormChoice(label, input));
    }
    for (const [key, label, choices] of [['mode', 'Consent mode', ['opt-in', 'opt-out']], ['position', 'Banner position', ['bottom', 'top']]]) {
        ...
        fields[key] = select;
        root.append(createFormField(label, select));
    }
    for (const [key, label, min, max] of [['policyVersion', 'Policy version', 1, 999999], ['cookieDays', 'Remember consent (days)', 1, 365], ['visitorDays', 'Visitor ID lifetime (days)', 1, 365], ['sessionMinutes', 'Session inactivity timeout (minutes)', 5, 240]]) {
        const input = document.createElement('input');
        input.type = 'number';
        ...
        fields[key] = input;
        root.append(createFormField(label, input));
    }
    for (const [key, label] of [['title', 'Banner title'], ['message', 'Banner explanation'], ['privacyUrl', 'Privacy policy path (for example /privacy)'], ['measurementId', 'Google Analytics measurement ID (G-…) — live connection untested']]) {
        const input = key === 'message' ? document.createElement('textarea') : document.createElement('input');
