import { applyJsonDelta, jsonDelta, validateJsonDelta, type JsonDelta } from './jsonDelta.js';

type RecordValue = Record<string, any>;
export type LocaleDesign = { layout: RecordValue | null; widgets: RecordValue[]; styles: RecordValue };
export type DesignLocalizations = { version: 1; primaryLanguage: string; variants: Record<string, JsonDelta> };
export const normalizeContentLocale = (value: unknown, fallback = 'en') => typeof value === 'string' && /^[a-z]{2,8}(?:-[a-z0-9]{1,8})*$/i.test(value) && value.length <= 35 ? value.toLowerCase() : fallback;
const parse = (value: unknown): RecordValue => { try { const result = typeof value === 'string' ? JSON.parse(value) : value; return result && typeof result === 'object' && !Array.isArray(result) ? result : {}; } catch { return {}; } };

export function readDesignLocalizations(layout: unknown, sourceLanguage = 'en'): DesignLocalizations {
  const data = parse(parse(layout).localizations);
  if (!Object.keys(data).length) return { version: 1, primaryLanguage: normalizeContentLocale(sourceLanguage), variants: {} };
  if (JSON.stringify(data).length > 2000000 || data.version !== 1 || Object.keys(parse(data.variants)).length > 60) throw new Error('DESIGN_LOCALE_DOCUMENT_INVALID');
  const variants: Record<string, JsonDelta> = {};
  for (const [locale, value] of Object.entries(parse(data.variants))) {
    if (normalizeContentLocale(locale, '') !== locale) throw new Error('DESIGN_LOCALE_CODE_INVALID');
    validateJsonDelta(value); variants[locale] = value;
  }
...
export function localeWidget(value: RecordValue): RecordValue {
  const code = parse(value.code), meta = parse(value.metadata ?? code.meta);
  const result: RecordValue = { id: String(value.instanceId ?? value.instance_id ?? value.id ?? ''), widgetId: String(value.widgetId ?? value.widget_id ?? ''),
  ...
  for (const key of ['workareaId', 'sceneId', 'sceneTitle', 'sceneBackground', 'behavior', 'effects', 'elementName', 'radius', 'responsivePlacement']) {
    const own = value[key] ?? meta[key]; if (own !== undefined) result[key] = own;
  }
  return result;
}

export function localeDesign(response: RecordValue): LocaleDesign {
  const design = response.design || response;
  const layout = { ...parse(response.layout ?? design.layout ?? design.layout_json) }; delete layout.localizations;
...
export function resolveDesignLocale(base: LocaleDesign, localizations: DesignLocalizations, language: unknown): LocaleDesign {
  ...
  let result = locale === localizations.primaryLanguage ? base : { ...base, widgets: base.widgets.map(widget => {
    ...
    delete meta.translations; delete settings.translations;
    if (meta.settings) meta.settings = settings;
    return { ...widget, code: { ...widget.code, meta: { ...meta, ...parse(translations[parent]), ...parse(translations[locale]) } } };
  }) };
  if (parent !== locale && parent !== localizations.primaryLanguage) result = applyJsonDelta(result, localizations.variants[parent]);
  return locale === localizations.primaryLanguage ? applyJsonDelta(base) : applyJsonDelta(result, localizations.variants[locale]);
}

export function saveDesignLocale(base: LocaleDesign, localizations: DesignLocalizations, language: string, edited: LocaleDesign) {
  const locale = normalizeContentLocale(language, localizations.primaryLanguage);
  if (locale === localizations.primaryLanguage) return { base: edited, localizations };
  ...
  if (change) variants[locale] = change; else delete variants[locale];
  return { base, localizations: { ...localizations, variants } };
}

export function projectLocalizedDesign(response: RecordValue, language: unknown): RecordValue {
