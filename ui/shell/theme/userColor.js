...
function isThemeModeStorageEvent(event) {
...
export function bindThemeModeStorageSync() {
    if (themeModeStorageSyncBound)
        return;
...
function updateThemeModeToggle(mode) {
...
export function bindThemeModeToggle() {
    applyThemeMode();
    const button = document.getElementById('theme-mode-toggle');
    if (!button || button.dataset.themeModeBound === 'true')
        return;
