...
import { adminBaseHref, adminSlugFromPath, deleteAdminPage, errorMessage, fetchAdminPageBySlug, isProtectedAdminWorkspace, normalizeAdminBase } from './contentHeaderActionsData.js';
function dispatchWidgetsToggle(open) {
    document.dispatchEvent(new CustomEvent('ui:widgets:toggle', { detail: { open } }));
}
let detachBreadcrumbResize = null;
function clearBreadcrumbResize() {
    detachBreadcrumbResize?.();
    detachBreadcrumbResize = null;
}
function renderBreadcrumb(breadcrumbEl) {
...
export function initContentHeader() {
    ...
    if (window.adminGrid && typeof window.adminGrid.on === 'function') {
        ...
        grid.on('staticchange', (isStatic) => {
            ...
            if (editIcon)
                editIcon.src = editing ? '/assets/icons/save.svg' : '/assets/icons/square-pen.svg';
...
async function handleDeleteCurrentAdminPage() {
    try {
        ...
        if (!(await bpDialog.confirm(`Delete admin page "${page.title}" (${page.slug})?`)))
            return;
