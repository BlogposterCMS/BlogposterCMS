...
import { acceptsMedia, isMediaImage, mediaDate, mediaEntries, mediaSize, mediaType, publicMediaUrl, visibleMediaEntries } from './mediaExplorerEntries.js';
...
function commandButton(label, iconName, visibleLabel = false) {
...
function fileIcon(entry) {
    if (entry.kind === 'folder')
        return 'folder';
    return { Image: 'file-image', Video: 'file-video-camera', Audio: 'file-music' }[mediaType(entry)] || 'file-text';
}
...
export function createMediaExplorerSurface(options = {}) {
    ...
    if (options.onPublishDownload)
        commands.append(publishBtn);
    ...
    if (mode === 'manage')
        commands.append(shareBtn);
    if (enableMutations)
        commands.append(renameBtn, deleteBtn);
    ...
    for (const [key, label] of [['name', 'Name'], ['type', 'Type'], ['modified', 'Modified'], ['size', 'Size']]) {
        const button = document.createElement('button');
        button.type = 'button';
    ...
    let listingEntries = [];
    let currentView = mode === 'picker' ? 'grid' : 'list';
    let selectedPath = '';
    let sortKey = 'name';
    ...
    const listFolder = (path) => {
        if (!isRemote())
            return listMediaFolder(emit, jwt, path);
        const location = storageLocations.find(item => item.id === locationId);
        if (!location)
            return Promise.reject(new Error('MEDIA_EXPLORER_STORAGE_UNAVAILABLE'));
    ...
    async function perform(action, propagate = false) {
        if (busy || loadState !== 'ready') {
            if (propagate)
                throw new Error('MEDIA_EXPLORER_NOT_READY: Reload the folder before changing files.');
            return;
        }
        busy = true;
        root.inert = true;
        ...
        try {
            ...
            setStatus(`MEDIA_EXPLORER_ACTION_FAILED: ${errorMessage(error)}`, 'error');
            if (propagate)
                throw error;
        }
        finally {
    ...
    async function loadTree(path) {
        if (folderCache.get(path)?.loading)
            return;
    ...
    function renderFolders() {
        ...
        function row(path, name, depth) {
            ...
            toggle.onclick = () => {
                if (expanded.has(path))
                    ...
                    expanded.add(path);
                if (expanded.has(path) && !cached?.folders)
                    void loadTree(path);
                else
                    renderFolders();
            };
            ...
            if (expanded.has(path)) {
                ...
                if (cached?.loading) {
                    ...
                    retry.type = 'button';
                    retry.textContent = 'Retry folders';
                    retry.title = cached.error;
        ...
        if (!options.loadStorageLocations)
            ...
            for (const location of [{ id: 'local', label: 'Local server' }, ...storageLocations]) {
                ...
                if (location.id === locationId) {
                    ...
                    branch.append(row('', 'Files', 1));
                    item.append(branch);
                }
                tree.append(item);
            }
            const settings = document.createElement('a');
            settings.href = '/admin/settings/general?tab=storage';
            settings.className = 'media-command media-command--label';
            settings.style.textDecoration = 'none';
            settings.textContent = 'Connect storage';
            folders.append(settings);
            if (storageError) {
                const retry = commandButton('Retry storage connections', 'refresh-cw', true);
                retry.title = storageError;
                retry.onclick = () => { void reloadLocations(); };
                folders.append(retry);
            }
        }
        folders.append(tree);
    }
    async function reloadLocations() {
        if (!options.loadStorageLocations)
            return;
        try {
            storageLocations = await options.loadStorageLocations();
            storageError = '';
        }
        catch (error) {
            storageError = `MEDIA_EXPLORER_STORAGE_FAILED: ${errorMessage(error)}`;
        }
        renderFolders();
    }
    async function selectLocation(id) {
        if (busy)
            return;
        if (id !== 'local' && !storageLocations.some(location => location.id === id))
            throw new Error('MEDIA_EXPLORER_STORAGE_UNAVAILABLE');
        locationId = id;
        folderCache.clear();
        expanded.clear();
        expanded.add('');
    ...
    function renderDetails() {
        ...
        if (url && isMediaImage(entry)) {
            ...
            preview.append(icon(fileIcon(entry)));
        const name = document.createElement('h3');
        name.textContent = entry.name;
        const path = document.createElement('p');
        path.className = 'media-explorer__detail-path';
        path.textContent = `Library / ${entry.path}`;
        const info = document.createElement('dl');
        for (const [key, value] of [['Type', mediaType(entry)], ['Size', entry.kind === 'folder' ? '—' : mediaSize(entry.size)], ['Modified', mediaDate(entry.modifiedAt)]]) {
        ...
        if (mode === 'picker' && !acceptsMedia(entry, options.accept)) {
            ...
            hint.textContent = `Choose a file matching ${options.accept}.`;
            details.append(hint);
        }
    }
    function select(entry, focus = false) {
        selectedPath = entry?.path || '';
        itemsEl.querySelectorAll('.media-item').forEach(item => {
            const active = item.dataset.path === selectedPath;
            item.classList.toggle('selected', active);
            const button = item.querySelector('.media-item__main');
    ...
    function renderEntry(entry, index) {
        ...
        if (currentView === 'grid' && preview && isMediaImage(entry)) {
            ...
            mark.append(icon(fileIcon(entry)));
        const name = document.createElement('span');
        name.className = 'media-name';
        name.textContent = entry.name;
        const identity = document.createElement('span');
        identity.className = 'media-item__identity';
        identity.append(mark, name);
        button.append(identity);
        for (const [className, text] of [['type', mediaType(entry)], ['modified', mediaDate(entry.modifiedAt)], ['size', entry.kind === 'folder' ? '—' : mediaSize(entry.size)]]) {
            const cell = document.createElement('span');
            cell.className = `media-item__${className}`;
        ...
        button.onkeydown = event => {
            ...
            if (target >= 0) {
                event.preventDefault();
                select(visible[target], true);
            }
            if (event.key === 'Escape') {
                event.preventDefault();
                select();
            }
            if (event.key === 'F2' && enableMutations) {
                event.preventDefault();
                select(entry);
                renameBtn.click();
            }
            if (event.key === 'Delete' && enableMutations) {
                event.preventDefault();
                select(entry);
                deleteBtn.click();
            }
    ...
    function renderEntries() {
        ...
        if (loadState !== 'ready') {
            ...
            message.className = 'media-explorer__empty';
            ...
            if (loadState === 'error') {
                ...
                retry.onclick = () => { void load(currentPath); };
                itemsEl.append(retry);
            }
        ...
        const visible = visibleEntries();
        if (!visible.some(entry => entry.path === selectedPath))
            selectedPath = '';
        if (!visible.length) {
            ...
            visible.forEach((entry, index) => itemsEl.append(renderEntry(entry, index)));
    ...
    async function navigate(path, direction) {
        if (busy)
            return;
    ...
    async function renameEntry(entry, name) {
        if (isRemote())
            throw new Error('MEDIA_EXPLORER_STORAGE_MUTATION_UNSUPPORTED');
    ...
    backBtn.onclick = () => { if (historyIndex > 0)
        void navigate(history[historyIndex - 1], historyIndex - 1); };
    forwardBtn.onclick = () => { if (historyIndex + 1 < history.length)
        void navigate(history[historyIndex + 1], historyIndex + 1); };
    ...
    openBtn.onclick = () => { const entry = selectedEntry(); if (entry?.kind === 'folder')
        open(entry); };
    useBtn.onclick = () => { const entry = selectedEntry(); if (entry?.kind === 'file')
        void perform(() => choose(entry)); };
    shareBtn.onclick = () => {
        const entry = selectedEntry();
        if (entry?.kind !== 'file')
        ...
        void perform(async () => {
            const result = isRemote() ? { shareURL: publicMediaUrl(entry) } : await createMediaShareLink(emit, jwt, entry.path);
            if (!result.shareURL)
                throw new Error('MEDIA_EXPLORER_SHARE_FAILED: No share URL was returned.');
    ...
    deleteBtn.onclick = () => {
        ...
        void perform(async () => {
            ...
            if (!await dialogApi().confirm(message, { title: 'Delete item', confirmLabel: 'Delete', cancelLabel: 'Cancel' }))
                return;
    ...
    function agentEntry(params) {
        ...
        if (!entry)
            throw new Error('MEDIA_EXPLORER_ITEM_NOT_FOUND: Inspect the current folder first.');
        return entry;
    }
    registerWorkspaceAgent({ root, id: mode === 'picker' ? 'media-picker' : 'media', title: mode === 'picker' ? 'Choose media' : 'Files and media',
        read: () => ({ dirty: false, busy: busy || loadState === 'loading', error: loadError || (statusEl.dataset.kind === 'error' ? statusEl.textContent : null),
            ...
            ...(options.loadStorageLocations ? [{ action: 'media.openStorage', label: 'Open storage', params: [{ name: 'id', type: 'string', required: true }],
                    run: async (p) => { await selectLocation(agentString(p, 'id')); if (loadState === 'error')
                        throw new Error(loadError); } }] : []),
            { action: 'media.openFolder', label: 'Open folder', params: [{ name: 'path', type: 'string', required: true }],
                run: async (p) => {
                    const path = typeof p.path === 'string' ? p.path : '';
                    if (path && !folderCache.has(path) && !listingEntries.some(entry => entry.path === path && entry.kind === 'folder'))
                        throw new Error('MEDIA_EXPLORER_FOLDER_UNKNOWN');
                    await navigate(path);
                    if (loadState === 'error')
                        throw new Error(loadError);
                } },
            ...
            { action: 'media.search', label: 'Search current folder', params: [{ name: 'query', type: 'string', required: true }], run: p => {
                    ...
                    search.value = p.query;
                    renderEntries();
                } },
            { action: 'media.refresh', label: 'Reload current folder', run: async () => { await load(currentPath); if (loadState === 'error')
                    throw new Error(loadError); } },
            ...(enableMutations ? [
                { action: 'media.createFolder', label: 'Create folder', params: [{ name: 'name', type: 'string', required: true }], run: (p) => perform(() => createFolder(agentString(p, 'name')), true) },
                { action: 'media.rename', label: 'Rename item', params: [{ name: 'path', type: 'string', required: true }, { name: 'name', type: 'string', required: true }], run: (p) => perform(() => renameEntry(agentEntry(p), agentString(p, 'name')), true) },
                { action: 'media.delete', label: 'Delete item', params: [{ name: 'path', type: 'string', required: true }], confirm: true, run: (p) => perform(() => deleteEntry(agentEntry(p)), true) }
            ] : []),
            ...(mode === 'picker' ? [{ action: 'media.useSelected', label: 'Use selected file', confirm: true, run: async () => {
                        const entry = selectedEntry();
                        if (!entry || entry.kind !== 'file' || !acceptsMedia(entry, options.accept))
                            throw new Error('MEDIA_EXPLORER_SELECTION_INVALID');
