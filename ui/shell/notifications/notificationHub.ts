...
export default function initNotificationHub(): void {
  const logo = document.querySelector<HTMLElement>('.top-header .logo');
  if (!logo || logo.dataset.notificationBound) return;
  ...
  const status = hub.querySelector<HTMLElement>('.notification-hub__status');
  const refresh = hub.querySelector<HTMLButtonElement>('[data-notification-refresh]');
  ...
  async function loadNotifications(): Promise<void> {
    ...
    if (!loaded) setStatus('Loading notifications…');
    try {
      const data = await fetchRecentNotifications(window.meltdownEmit, window.ADMIN_TOKEN);
      if (!hub.isConnected || listeners.signal.aborted) return;
      ...
      if (nextItems !== lastItems) renderNotificationItems(notificationList, data);
      lastItems = nextItems;
      loaded = true;
  ...
  const poll = window.setInterval(() => {
    if (!logo.isConnected) { window.clearInterval(poll); listeners.abort(); return; }
