...
export async function mountWebsiteDesignSettings(options: {
  ...
  presets: HTMLElement;
  emit: NonNullable<Window['meltdownEmit']>;
  jwt: string;
  branding: () => { logoUrl: string; logoDarkUrl: string };
  editTheme: () => void;
}): Promise<() => void> {
  ...
  const colorHost = document.createElement('div'); colorHost.dataset.colorSchemeHost = '';
  const fontHost = document.createElement('div'); fontHost.dataset.fontPackagesHost = '';
  ...
  const kitContext = { sidebarEl: root, cleanupSitePresetsPanel: undefined as (() => void) | undefined, setSidebarPanel: options.editTheme };
  ...
  return () => {
    libraries(); preview(); kitContext.cleanupSitePresetsPanel?.();
