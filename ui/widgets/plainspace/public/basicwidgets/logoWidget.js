...
class SiteLogoElement extends HTMLElement {
    ...
    updateLogo = () => {
        ...
        if (img)
            img.setAttribute('src', (dark ? this.darkUrl : this.lightUrl) || this.lightUrl || this.darkUrl);
...
if (!customElements.get('bp-site-logo'))
    customElements.define('bp-site-logo', SiteLogoElement);
...
export async function render(el) {
    ...
    try {
        ...
        if (!response.ok)
            throw new Error('BP_WIDGET_LOGO_SETTINGS_FAILED');
        const body = await response.json();
        if (!body?.settings || typeof body.settings !== 'object' || Array.isArray(body.settings)) {
            throw new Error('BP_WIDGET_LOGO_SETTINGS_INVALID');
        }
        if (pendingRenders.get(el) !== request)
            return;
        ...
        const logo = document.createElement('bp-site-logo');
