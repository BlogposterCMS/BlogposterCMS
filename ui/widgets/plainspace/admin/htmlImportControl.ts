...
type ImportResult = {
  plan: { summary: { sections?: number; containers?: number; editableWidgets: number; decorations: number; capturedWidths: number[]; warnings: { code: string }[] } };
  result?: { id?: string | number; designId?: string | number };
};

/** The gallery starts imports through the existing Runtime Manager / Importer boundary. */
export function createHtmlImportControl(): HTMLElement {
  ...
  async function run(dryRun: boolean): Promise<ImportResult> {
    if (typeof window.meltdownEmit !== 'function') throw new Error('HTML_IMPORT_BRIDGE_MISSING');
    return emitRuntimeAdmin<ImportResult>(window.meltdownEmit, window.ADMIN_TOKEN, 'importers', 'run', {
  ...
  input.addEventListener('change', async () => {
    ...
    try {
      if (file.size > 8 * 1024 * 1024) throw new Error('HTML_IMPORT_LIMIT_EXCEEDED: Maximum file size is 8 MB.');
      ...
      if (version === requestVersion) status.textContent = `HTML_IMPORT_FAILED: ${error instanceof Error ? error.message : String(error)}`;
    }
  });
  apply.addEventListener('click', async () => {
    ...
    try {
      ...
      if (!id) throw new Error('HTML_IMPORT_SAVE_RESULT_INVALID');
