...
import { bindEditingScope, inspectorLayoutContainer, parentLayoutContainer } from './managers/editingScope.js';
import { layerTreeChildren } from './managers/layerTree.js';
...
import { mountInspectorControls } from './managers/inspectorControls.js';
...
import { buildLivePreviewFrameUrl, buildLivePreviewPayload, createLivePreviewController } from './renderer/livePreviewFrame.js';
...
import { createSitePreset, getSitePresetsSnapshot } from '/ui/shared/presets/sitePresets.js';
import { componentWidgetPreset, resolveKitComponent } from '/ui/shared/design-system/componentDefinitions.js';
import { getActiveColorScheme } from '/ui/shared/colors/colorLibrary.js';
import { getActiveFontPackage } from '/ui/shared/fonts/fontPackages.js';
...
export async function initBuilder(sidebarEl, contentEl, pageId = null, startLayer = 1, layoutNameParam = null) {
    ...
    if (!displayPorts.length)
        displayPorts.push(...DEFAULT_PORTS);
    ...
    function closeSidebarPanel(options = {}) {
        if (!options.preserveInsertGroup)
            collapseInsertGroup();
    ...
    function normalizeRange(startValue, endValue) {
        return normalizeSceneRange(startValue ?? DEFAULT_SCROLL_RANGE.start, endValue ?? DEFAULT_SCROLL_RANGE.end);
    }
    function getElementRange(el) {
        if (!el)
            return { ...DEFAULT_SCROLL_RANGE };
    ...
    function syncSceneTitleDom(scene) {
        ...
        document.querySelectorAll(`.scene-section-title-input[data-scene-id="${cssEscape(scene.id)}"]`).forEach(inputNode => {
            if (inputNode instanceof HTMLInputElement && inputNode.value !== scene.title)
                inputNode.value = scene.title;
        });
        const stageLabel = document.querySelector('.scene-stage-title');
        if (stageLabel)
            stageLabel.textContent = scene.title;
        if (gridEl) {
    ...
    function syncInspectorBehaviorPreview(behaviorValue = 'scroll', rangeValue = DEFAULT_SCROLL_RANGE, effectsValue = []) {
        ...
        if (title)
            title.textContent = behaviorDef.title;
        if (detail)
            detail.textContent = `${range.start}% - ${range.end}% / ${effectSummary}`;
        if (element)
            element.textContent = behaviorDef.title;
        if (effects) {
    ...
    function removeStageBehaviorHuds(exceptEl = null) {
        ...
        document.querySelectorAll('.scene-stage-hud').forEach(hud => {
            if (exceptId && hud.dataset.instanceId === exceptId)
                return;
            hud.remove();
        });
    }
    function renderStageBehaviorHud(el, behaviorDef, range, effects = null) {
        if (!el || el !== state.activeWidgetEl)
            return;
    ...
    function applyElementRangeFromUi(el, range, persist = true) {
        ...
        if (persist && state.designId && state.autosaveEnabled)
            scheduleAutosave();
        return nextRange;
    }
    function beginRangeHandleDrag(event, handleEl, targetEl) {
        ...
        const update = moveEvent => {
            ...
            const nextRange = rangeFromPointer(moveEvent.clientX, rect, handle, getElementRange(targetEl));
    ...
    function syncInspectorRange(range) {
        ...
        if (startEl)
            startEl.value = String(range.start);
        if (endEl)
            endEl.value = String(range.end);
        setRangeVars(visual, range);
        const viewport = document.getElementById('builderViewport');
        setRangeVars(viewport, range);
        syncInspectorBehaviorPreview(state.activeWidgetEl?.dataset?.behavior || 'scroll', range, state.activeWidgetEl ? getElementEffects(state.activeWidgetEl) : readEffectsFromInspector());
    }
    ...
    function getElementAppearance(el) {
        if (!el)
            return { opacity: 100, radius: 8, name: '' };
    ...
    function applyNativeButtonContent(el, data = {}, persist = true) {
        if (!el || !isNativeButtonElement(el))
            return;
    ...
    function syncInspectorAppearance(el, widgetDef = null) {
        ...
        if (typeInput)
            typeInput.value = el ? (widgetDef?.metadata?.label || el.dataset.widgetId || 'Element') : 'Section';
        if (opacityRange)
            opacityRange.value = String(appearance.opacity);
        if (opacityValue)
            opacityValue.value = String(appearance.opacity);
        if (radiusRange)
            radiusRange.value = String(appearance.radius);
        if (radiusValue)
            radiusValue.value = String(appearance.radius);
    }
    function syncInspectorButton(el) {
        ...
        if (group)
            group.hidden = !isButton;
        if (labelInput)
            labelInput.disabled = !isButton;
        if (hrefInput)
            hrefInput.disabled = !isButton;
        if (!isButton) {
            if (labelInput)
                labelInput.value = '';
            if (hrefInput)
    ...
    function syncInspectorSection(scene = getActiveScene()) {
        if (!sceneInspector || !scene)
            return;
        const sectionEl = inspectorLayoutContainer(layoutRoot, scene.id);
        const nested = sectionEl && !sectionEl.classList.contains('layout-section');
        const background = nested ? sectionEl.dataset.layoutBackground || 'transparent' : getSceneBackground(scene);
        ...
        const positionOptions = containerPositionAvailability(sectionEl, layoutRoot);
        sceneInspector.querySelectorAll('[data-container-position]').forEach(button => {
            const mode = button.dataset.containerPosition;
            const option = positionOptions[mode];
            button.setAttribute('aria-disabled', String(!option.available));
            button.setAttribute('aria-pressed', String((sectionEl?.dataset.layoutPosition || 'normal') === mode));
            button.title = option.reason;
            button.setAttribute('aria-description', option.reason);
            button.querySelector('[role="tooltip"]').textContent = option.reason;
        });
        if (heightMode)
            heightMode.value = authoredHeight && authoredHeight !== 'auto' ? 'fixed' : 'auto';
        if (exactHeight) {
            exactHeight.disabled = !authoredHeight || authoredHeight === 'auto';
            exactHeight.value = String(numberValue(authoredHeight, numberValue(sectionEl?.dataset.layoutMinHeight, 320)));
            exactHeight.title = exactHeight.disabled ? 'Choose Fixed height to enter a pixel value.' : 'Minimum height still sets the lower limit.';
        }
        ...
        for (const key of ['borderWidth', 'borderStyle', 'borderColor', 'borderRadius', 'borderTopWidth', 'borderRightWidth', 'borderBottomWidth', 'borderLeftWidth']) {
            const input = sceneInspector.querySelector(`[data-container-border="${key}"]`);
            if (!input)
                continue;
            const value = sectionEl?.dataset[`layout${key[0].toUpperCase()}${key.slice(1)}`];
            input.value = key.endsWith('Width') || key === 'borderRadius'
                ? String(numberValue(value ?? (key.endsWith('Width') ? sectionEl?.dataset.layoutBorderWidth : undefined), 0)) : value || (key === 'borderStyle' ? 'none' : '#000000');
        }
        if (contentHost)
            contentHost.checked = sectionEl?.dataset.dynamicHost === 'true';
        if (name) {
            name.value = nested ? sectionEl.dataset.nodeId : scene.title;
            name.disabled = Boolean(nested);
        }
        const groupTitle = sceneInspector.querySelector('.scene-section-settings h3');
        if (groupTitle)
            groupTitle.textContent = nested ? 'Container' : 'Section';
        ...
        if (columns)
            columns.closest('label').hidden = visibleMode !== 'grid';
        if (gap)
            gap.closest('label').hidden = visibleMode === 'free';
        if (align)
            align.closest('label').hidden = visibleMode === 'free';
        if (transparent)
    ...
    function updateSectionFromInspector(target, persist = true) {
        ...
        if (target.matches('.scene-container-height-mode, .scene-container-height-value')) {
        ...
        else if (target.matches('[data-container-border]')) {
            const key = target.dataset.containerBorder;
            const value = key.endsWith('Width') || key === 'borderRadius' ? `${Number(target.value) || 0}px` : target.value;
            const settings = { [key]: value };
            // Changing the overall width resets explicit sides; individual inputs
            // then override only their own edge through the same saved settings.
            if (key === 'borderWidth') {
                for (const side of ['Top', 'Right', 'Bottom', 'Left'])
                    settings[`border${side}Width`] = value;
            }
            // A positive width should produce a visible border on an untouched box.
            if (key.endsWith('Width') && Number(target.value) > 0 && (!sectionEl.dataset.layoutBorderStyle || sectionEl.dataset.layoutBorderStyle === 'none'))
                settings.borderStyle = 'solid';
            setContainerSettings(sectionEl, settings);
        }
        else if (target.matches('.scene-section-content-host')) {
            ...
            if (nested)
                setContainerSettings(sectionEl, { background: target.value });
            else
                applySceneBackground(scene, target.value);
        }
        else if (target.matches('.scene-section-transparent')) {
            const background = target.checked ? 'transparent' : (sceneInspector.querySelector('.scene-section-background')?.value || '#ffffff');
            if (nested)
                setContainerSettings(sectionEl, { background });
            else
                applySceneBackground(scene, background);
        }
    ...
    function syncInspectorEffects(effects) {
        ...
        sceneInspector.querySelectorAll('.scene-effect-item').forEach(item => {
            ...
            if (start)
                start.value = String(effect.start);
            if (end)
                end.value = String(effect.end);
            setRangeVars(range, effect);
            item.classList.toggle('active', Boolean(effect.enabled));
        });
        syncInspectorBehaviorPreview(state.activeWidgetEl?.dataset?.behavior || 'scroll', state.activeWidgetEl ? getElementRange(state.activeWidgetEl) : normalizeRange(sceneInspector.querySelector('.scene-range-start')?.value, sceneInspector.querySelector('.scene-range-end')?.value), normalized);
    }
    ...
    function applyEffectItemRange(item, range, enable = true) {
        ...
        if (startEl)
            startEl.value = String(nextRange.start);
        if (endEl)
            endEl.value = String(nextRange.end);
        if (enable && toggle)
            toggle.checked = true;
    ...
    function beginEffectRangeDrag(event, handleEl) {
        ...
        const update = (moveEvent, persist = false) => {
            const rect = trackEl.getBoundingClientRect();
            const nextRange = rangeFromPointer(moveEvent.clientX, rect, handle, getEffectItemRange(item));
    ...
    function setInspectorMode(mode, inspectorEl = sceneInspector) {
        ...
        inspectorTabs?.selectMode(activeInspectorMode);
    }
    function ensureSceneInspector() {
      ...
      <button type="button" class="scene-inspector-parent" data-select-container-parent>Select parent</button>
      ...
      <section class="scene-inspector-group scene-section-settings" data-inspector-panel="content">
        ...
        <div class="scene-container-position" role="group" aria-label="Container scroll position">
          ${['normal', 'sticky', 'fixed'].map(mode => `<button type="button" data-container-position="${mode}" aria-describedby="container-position-${mode}-reason">${mode === 'normal' ? 'Normal' : mode === 'sticky' ? 'Sticky' : 'Fixed'}<span id="container-position-${mode}-reason" role="tooltip"></span></button>`).join('')}
        </div>
        ...
        <h4>Border</h4>
        <div class="scene-field-grid">
          <label><span>Width (px)</span><input data-container-border="borderWidth" type="number" min="0" max="64" step="1" value="0" /></label>
          <label><span>Radius (px)</span><input data-container-border="borderRadius" type="number" min="0" max="512" step="1" value="0" /></label>
        </div>
        <div class="scene-field-grid">
          <label><span>Top (px)</span><input data-container-border="borderTopWidth" type="number" min="0" max="64" step="1" value="0" /></label>
          <label><span>Right (px)</span><input data-container-border="borderRightWidth" type="number" min="0" max="64" step="1" value="0" /></label>
          <label><span>Bottom (px)</span><input data-container-border="borderBottomWidth" type="number" min="0" max="64" step="1" value="0" /></label>
          <label><span>Left (px)</span><input data-container-border="borderLeftWidth" type="number" min="0" max="64" step="1" value="0" /></label>
        </div>
        <label class="scene-select-field"><span>Line style</span><select data-container-border="borderStyle"><option value="none">None</option><option value="solid">Solid</option><option value="dashed">Dashed</option><option value="dotted">Dotted</option><option value="double">Double</option></select></label>
        <label class="scene-select-field"><span>Border color</span><input data-container-border="borderColor" type="color" value="#000000" /></label>
      </section>
    ...
    `;
        inspector.addEventListener('click', event => {
            const positionButton = event.target.closest('[data-container-position]');
            if (positionButton) {
                const selected = inspectorLayoutContainer(layoutRoot, getActiveScene()?.id);
                const mode = positionButton.dataset.containerPosition;
                const option = containerPositionAvailability(selected, layoutRoot)[mode];
                if (!option?.available)
                    return;
                setContainerSettings(selected, { position: mode });
                syncInspectorSection();
                requestSceneChangePersist();
                return;
            }
            if (event.target.closest('[data-select-container-parent]')) {
                const selected = state.activeWidgetEl || inspectorLayoutContainer(layoutRoot, getActiveScene()?.id);
                const parent = parentLayoutContainer(selected, layoutRoot);
                if (parent)
                    selectWidget(parent);
                return;
            }
            ...
            if (state.activeWidgetEl) {
                ...
                applyBehaviorRange(state.activeWidgetEl, state.activeWidgetEl.dataset.scrollStart, state.activeWidgetEl.dataset.scrollEnd);
                syncInspectorBehaviorPreview(state.activeWidgetEl.dataset.behavior, getElementRange(state.activeWidgetEl), getElementEffects(state.activeWidgetEl));
                renderSceneLayers();
                if (state.designId && state.autosaveEnabled)
                    scheduleAutosave();
            }
        ...
        inspector.addEventListener('pointerdown', event => {
            ...
            if (!handle || !state.activeWidgetEl)
                return;
        ...
        inspector.addEventListener('change', event => {
            ...
            const sectionField = event.target.closest?.('.scene-container-height-mode, .scene-container-height-value, [data-container-border], .scene-section-content-host, .scene-section-direction, .scene-section-gap, .scene-section-padding, .scene-section-height, .scene-section-columns, .scene-section-align, .scene-section-background, .scene-section-transparent');
            if (sectionField) {
        ...
        inspectorTabs = mountInspectorControls(inspector, mode => {
            activeInspectorMode = mode;
            inspector.dataset.activeMode = mode;
        });
        setInspectorMode(activeInspectorMode, inspector);
        return inspector;
    }
    function updateSceneInspector(el = null, widgetDef = null) {
        const parentButton = sceneInspector?.querySelector('[data-select-container-parent]');
        if (parentButton) {
            const selected = el || inspectorLayoutContainer(layoutRoot, getActiveScene()?.id);
            const parent = parentLayoutContainer(selected, layoutRoot);
            parentButton.disabled = !parent;
            parentButton.textContent = parent ? `Select parent · ${parent.dataset.sectionTitle || parent.dataset.nodeId || 'Container'}` : 'Select parent';
        }
        if (!sceneInspector)
            return;
        const kind = el ? 'widget' : 'container';
        if (sceneInspector.dataset.selectionKind !== kind)
            setInspectorMode('content');
        sceneInspector.dataset.selectionKind = kind;
        ...
        sceneInspector.querySelectorAll('[data-inspector-panel="behavior"], [data-inspector-panel="style"]').forEach(panel => {
            const available = panel.hasAttribute('data-container-panel') ? !el : Boolean(el);
        ...
        if (el) {
            ...
            if (kickerEl)
                kickerEl.textContent = activeScene.title;
            if (xEl)
                xEl.value = `${Math.round(Number(el.dataset.xPercent || 50))} %`;
            if (yEl)
                yEl.value = `${Math.round(Number(el.dataset.yPercent || 40))} %`;
        }
        else {
            const container = inspectorLayoutContainer(layoutRoot, activeScene.id);
            const nested = container && !container.classList.contains('layout-section');
            if (titleEl)
                titleEl.textContent = nested ? container.dataset.nodeId : activeScene.title;
            if (kickerEl)
                kickerEl.textContent = nested
                    ? `${activeScene.title} / ${container.dataset.dynamicHost === 'true' ? 'Page content area' : 'Container'}`
                    : 'Section';
            if (xEl)
                xEl.value = '50 %';
            if (yEl)
    ...
    function arrangeSceneWidget(widget, direction = 'forward') {
        ...
        if (currentIndex < 0 || nextIndex < 0 || nextIndex >= stack.length)
            return false;
    ...
    function widgetIsInsideStageViewport(widget) {
        if (!widget?.getBoundingClientRect)
            return true;
    ...
    function scrollWidgetIntoStageViewport(widget) {
        if (!widget?.scrollIntoView || widgetIsInsideStageViewport(widget))
            return false;
    ...
    function selectWidgetFromLayer(widget, options = {}) {
        ...
        if (options.scroll !== false)
            scrollWidgetIntoStageViewport(widget);
        pulseElement(widget);
    }
    ...
    function finishSceneRename(input, commit = true) {
        if (!input || input.dataset.sceneId !== editingSceneId)
            return;
    ...
    function moveScene(sceneId, delta) {
        ...
        if (current < 0 || next < 0 || next >= sceneSections.length)
            return;
    ...
    function renderSceneLayers() {
        ...
        if (!layerTreeChildren(sectionSurface).length) {
        ...
        const orderedChildren = surface => layerTreeChildren(surface)
            .sort((left, right) => {
            const zIndexDiff = layerStackIndex(left) - layerStackIndex(right);
            if (zIndexDiff !== 0)
                return zIndexDiff;
        ...
        const renderSurface = (surface, depth = 0) => {
            orderedChildren(surface).forEach(widget => {
                ...
                const isContainer = widget.classList.contains('layout-container');
                const widgetDef = isContainer
                    ? null
                    : allWidgets.find(candidate => candidate.id === widget.dataset.widgetId);
                const label = isContainer
                    ? (widget.dataset.elementName || widget.dataset.nodeName || widget.dataset.nodeId || 'Container')
    ...
    document.addEventListener('click', event => {
        ...
        if (!isSidebarFlyoutOpen() || !(target instanceof Node) || sidebarEl.contains(target))
            return;
  ...
  `;
    ...
    gridViewportEl?.addEventListener('click', event => {
        ...
        if (handleSceneNavigationClick(event))
            return;
    ...
    const saveLayoutCtx = {
        getDesignSaveOptions: () => ({
            name: document.querySelector('#layoutNameInput')?.value?.trim() || layoutName,
    ...
    function setContainerSettings(el, settings) {
        try {
            if (settings.position && !containerPositionAvailability(el, layoutRoot)[settings.position]?.available) {
                throw new Error(`DESIGNER_CONTAINER_POSITION_UNAVAILABLE: ${containerPositionAvailability(el, layoutRoot)[settings.position]?.reason || 'Unsupported position.'}`);
            }
    ...
    function selectWidget(el) {
        if (el?.classList?.contains('layout-container') && el !== layoutRoot) {
            ...
            hideToolbar();
            updateSceneInspector(null);
            const selectedSurface = layoutGridRegistry?.forWorkspace(el);
            if (selectedSurface) {
                gridEl = selectedSurface.surface;
                grid = selectedSurface.grid;
            }
            renderSceneLayers();
            return;
        }
        layoutRoot.querySelectorAll('.layout-container--active').forEach(container => container.classList.remove('layout-container--active'));
        ...
        if (!el.dataset.sceneTitle) {
            ...
            if (activeScene?.title)
                el.dataset.sceneTitle = activeScene.title;
        }
    ...
    bindEditingScope({ layoutRoot, onSelect: target => {
            if (target.classList.contains('canvas-item'))
                selectWidget(target);
            else {
                const section = target.closest('.layout-section[data-section-id]');
                if (section)
    ...
    bindLayoutWidgetSelection({
        layoutRoot,
        getActiveLayer: () => activeLayer,
        onActivateLayer: async (layer, widget) => {
            const instanceId = widget.dataset.instanceId;
            await switchLayer(layer);
            const current = Array.from(layoutRoot.querySelectorAll('.canvas-item'))
    ...
    layoutRoot.addEventListener('pointerdown', event => {
        ...
        if (!widget)
            return;
        if (widget !== state.activeWidgetEl)
            selectWidget(widget);
    ...
    function dropSceneInsertPosition(event, width = 4, height = DEFAULT_ROWS) {
        if (!gridEl || !grid)
            return nextSceneInsertPosition(width, height);
    ...
    async function createSceneWidget(widgetDef, { x = 0, y = 0, w = 480, h = 240, minW = 240, minH = 120, code = null, behavior = 'scroll', label = '', elementName = '', targetRecord = null, layer = activeLayer, sceneId = activeSceneId, sceneTitle = '', sceneBackground = '', global = false, select = true, persist = true } = {}) {
        ...
        wrapper.dataset.workareaId = targetRecord?.surfaceId || targetSurface.dataset.workareaId || sceneId;
    ...
    function activateSceneById(sceneId, persist = false) {
        ...
        if (!scene)
            return { handled: false, reason: 'scene-not-found', sceneId: id };
    ...
    function stepSceneBy(delta) {
        ...
        if (!next)
            return { handled: false, reason: 'scene-edge', activeScene: sceneSummary() };
    ...
    function updateActiveElementRange(command = {}) {
        ...
        if (selectResult && !selectResult.handled)
            return selectResult;
        const el = state.activeWidgetEl;
        if (!el)
            return { handled: false, reason: 'no-active-element' };
        ...
        if (effectId) {
            nextEffects = nextEffects.map(effect => {
                if (effect.id !== effectId)
                    return effect;
                const range = normalizeRange(commandValue(command, 'start', effect.start), commandValue(command, 'end', effect.end));
                return {
                    ...effect,
    ...
    function updateSceneFromCommand(command = {}) {
        ...
        if (!scene)
            return { handled: false, reason: 'scene-not-found', sceneId };
    ...
    function findLayoutContainerByCommand(command = {}, key = 'id') {
        ...
        if (!raw)
            return getActiveWorkareaContainer() || containers[0] || null;
        return containers.find(container => (container.dataset.nodeId === raw || container.id === raw)) || null;
    }
    ...
    function updateTextFromCommand(command = {}) {
        ...
        if (!editable)
            return { handled: false, reason: 'selected-element-is-not-text-editable' };
        const content = commandValue(command, 'content', undefined);
        if (content !== undefined)
            editable.textContent = String(content);
        const styleValues = {
        ...
        Object.entries(styleValues).forEach(([property, value]) => {
            if (value !== undefined)
                editable.style[property] = String(value || '');
        });
    ...
    async function updateContainerFromCommand(action, command = {}) {
        ...
        if (action === 'container.create') {
            ...
            if (!source || !destination)
                return { handled: false, reason: 'container-source-or-target-not-found' };
    ...
    async function createCurrentSitePreset(command = {}) {
        ...
        if (!name)
            return { handled: false, reason: 'missing-site-preset-name' };
    ...
    async function saveDesignFromCommand(command = {}) {
        ...
        if (!name)
            return { handled: false, reason: 'missing-design-name' };
    ...
    async function startPublishFromCommand(command = {}) {
        if (!publishController)
            throw new Error('DESIGNER_AGENT_PUBLISH_PANEL_UNAVAILABLE: Wait until Publishing finishes loading.');
    ...
    async function executeDesignerAgentCommand(command = {}) {
        ...
        if (action === 'scene.move' || action === 'scene.reorder') {
            ...
            if (currentIndex < 0 || !delta)
                return { handled: false, reason: currentIndex < 0 ? 'scene-not-found' : 'scene-position-unchanged' };
        ...
        if (action === 'insert' || action === 'insert.element') {
            ...
            if (componentId) {
                ...
                try {
                    ...
                    return { handled: false, errorCode: String(error.message).split(':')[0], message: error.message };
                }
            }
        ...
        if (action === 'element.select')
            return selectElementByCommand(command);
        if (action === 'element.move' || action === 'element.resize' || action === 'element.geometry.set')
            return updateElementGeometry(command);
        if (action === 'element.responsiveRange.set')
            return updateElementResponsiveRange(command);
        if (action === 'element.zIndex.set' || action === 'element.arrange')
            return setElementStack(command);
        if (action === 'element.delete')
            return deleteElementFromCommand(command);
        if (action === 'element.duplicate')
            return duplicateElementFromCommand(command);
        if (action === 'text.update' || action === 'text.format.set')
            return updateTextFromCommand(command);
        ...
        if (action === 'range.set' || action === 'behavior.range.set')
            return updateActiveElementRange(command);
        if (action === 'effect.set' || action === 'effects.set')
        ...
        if (action === 'sitePresets.create')
            return createCurrentSitePreset(command);
        if (action === 'design.save')
            return saveDesignFromCommand(command);
        if (action === 'design.publish')
            return startPublishFromCommand(command);
        if (action === 'publishing.open') {
            if (!publishController)
                throw new Error('DESIGNER_AGENT_PUBLISH_PANEL_UNAVAILABLE');
    ...
    layoutRoot.addEventListener('designerSectionStructureRequested', event => {
        ...
        if (!section || section.parentElement !== layoutRoot)
            return;
        if (event.detail?.action === 'addContainer')
            placeContainer(section, 'inside');
        else if (event.detail?.action === 'contentHost')
            setDynamicHost(section);
        refreshBackgroundToolbars(layoutRoot);
    });
    ...
    const clearActiveWidgetFromExternalPointer = event => {
        ...
        event.target.closest('.canvas-item') ||
            ...
            // Viewport range controls edit the current selection; opening them
            // must not clear their target before the pointer reaches the header.
            event.target.closest('.builder-header, .layout-bar')) {
            return;
        }
    ...
    document.addEventListener('click', e => {
        if (!state.activeWidgetEl)
            return;
        if (e.target.closest('.canvas-item') === state.activeWidgetEl ||
            ...
            e.target.closest('.builder-header, .layout-bar')) {
            return;
        }
    ...
    const contentClickHandler = e => {
        ...
        if (inPreview) {
            backgroundLogger.debug('skip: preview-mode contentDown');
            return;
        }
    ...
    document.addEventListener('click', e => {
        ...
        if (insideBgToolbar || insidePicker || insideTextTb) {
            backgroundLogger.debug('global click inside UI, ignore', { insideBgToolbar: !!insideBgToolbar, insidePicker: !!insidePicker, insideTextTb: !!insideTextTb });
            return;
        }
        ...
        if (delta < 250) {
            backgroundLogger.debug('global click within suppress window', { delta });
            return;
        }
        const insideViewport = e.target.closest('#builderViewport');
        if (!insideViewport) {
            backgroundLogger.debug('global click outside viewport -> hide bgToolbar');
            hideBgToolbar();
        }
        else {
            backgroundLogger.debug('global click inside viewport, keep bgToolbar', { insideGrid: !!insideGrid });
        }
    });
    let initialLayout = [];
    let pageData = null;
    ...
    // Zoom belongs to the page owner even when another Section is active.
    const pageZoomGrid = layoutGridRegistry.zoomGrid();
    layoutBar = buildLayoutBar({ footer, grid: pageZoomGrid, gridEl: pageZoomGrid.el });
    if (HAS_LAYOUT_STRUCTURE) {
        if (activeLayer === 0) {
            await startLayoutMode(layoutCtx);
        }
    ...
    const headerController = createBuilderHeader({
        ...
        grid,
        getGrid: () => grid,
        setViewport: width => layoutGridRegistry.setResponsiveViewport(width),
        saveDesign,
        getCurrentLayoutForLayer,
        getActiveLayer: () => activeLayer,
        ensureCodeMap,
