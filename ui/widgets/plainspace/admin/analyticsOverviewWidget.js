...
export async function render(el) {
    ...
    try {
        ...
        title.textContent = 'Analytics · last 7 days';
        const link = document.createElement('a');
        link.href = '/admin/analytics';
        link.textContent = 'Open Analytics';
        ...
        if (data.health.lastError || data.health.dropped || data.health.truncated) {
            ...
            warning.textContent = 'ANALYTICS_INCOMPLETE: Open Analytics for collection status.';
            el.append(warning);
        }
    }
    catch (error) {
