...
export async function editUserPermissions(permissions, selected) {
    ...
    search.placeholder = 'Search rights…';
    search.setAttribute('aria-label', 'Search rights');
    ...
    [...permissions].sort((a, b) => permissionKey(a).localeCompare(permissionKey(b))).forEach(permission => {
        ...
        if (!section) {
            ...
            section.className = 'user-permissions-group';
        ...
        label.className = 'user-permissions-option';
        const input = document.createElement('input');
        input.type = 'checkbox';
        input.checked = draft.has(key);
        ...
        name.textContent = permission.description || key;
        text.append(name);
    ...
    body.append(search, count, list, empty);
    refresh();
