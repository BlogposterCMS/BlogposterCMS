import { normalizeContainerInteraction } from './containerInteractionModel.js';
...
function owner(root, id) {
    if (!id)
        return undefined;
    return [...root.querySelectorAll('[data-node-id],[data-instance-id]')]
...
function definition(settings) {
    ...
    return { version: 1, state: { opened: settings.presentation === 'normal', query: '' },
        ...
        actions: {
            activate: [{ type: 'set', target: 'query', value: { ref: 'event.value' } }, { type: 'set', target: 'opened', value: true }, ...(settings.source ? [{ type: 'request', target: 'articles' }] : [])],
...
export function mountContainerInteractions(root) {
    mounted.get(root)?.();
    const disposers = [];
    for (const container of root.querySelectorAll('[data-layout-interaction]')) {
        const parsed = normalizeContainerInteraction(JSON.parse(container.dataset.layoutInteraction));
        if (!parsed)
            continue;
        const settings = parsed;
        const marker = document.createComment('container interaction');
        container.before(marker);
        const listeners = new AbortController();
        const services = createPublicUiData();
        const output = document.createElement('div');
        output.dataset.containerData = 'true';
        if (settings.source)
            container.append(output);
        const renderer = createWidgetUi(output);
        let popover, dialog;
        let controller, closed = false, restoring = false;
        ...
        function close(notify = false) {
            ...
            restoring = false;
            if (notify && !closed)
                void controller?.dispatch({ action: 'close' });
        }
        function paint(tree) {
            if (settings.source)
                renderer.render(tree);
            const visible = tree.open === true && tree.hidden !== true;
            if (settings.presentation === 'normal') {
            ...
            if (settings.presentation === 'popover') {
                ...
                for (const [key, value] of Object.entries(publicStyles))
                    popover.panel.style.setProperty(key, value);
            }
            else if (!dialog) {
                ...
                if (settings.presentation === 'drawer')
                    dialog.style.cssText += ';margin-right:0;height:100vh;max-height:100vh;width:min(90vw,420px)';
                ...
                dialog.append(button, container);
                root.append(dialog);
                dialog.addEventListener('close', () => { if (dialog)
                    close(true); }, { signal: listeners.signal });
                dialog.showModal();
            }
        }
        ...
        const activate = (event) => {
            ...
            const field = event.composedPath().find(item => item instanceof HTMLInputElement || item instanceof HTMLTextAreaElement || (item instanceof HTMLElement && item.isContentEditable));
            const queryField = (field && (!settings.queryId || event.composedPath().includes(fieldOwner)) ? field : queryControl(fieldOwner));
