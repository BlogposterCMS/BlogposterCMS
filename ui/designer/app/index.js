...
const renderLoadError = ({ container, before, message, title = 'Unable to load content', variant = 'section', replace = false }) => {
    if (!container && !before) {
        return null;
    }
...
let originPublicKeyPromise;
const fetchOriginPublicKey = async () => {
    ...
    if (!originPublicKeyPromise) {
        originPublicKeyPromise = (async () => {
            try {
                // The Designer runs in a sandboxed app frame, so this public key must
                // be fetched as a public CORS resource rather than a same-origin read.
...
const verifyWithSubtle = async (publicKeyPem, payload, signature) => {
    ...
    try {
        const cryptoKey = await window.crypto.subtle.importKey('spki', keyData, {
            name: 'RSASSA-PKCS1-v1_5',
            hash: 'SHA-256'
        }, false, ['verify']);
        return window.crypto.subtle.verify('RSASSA-PKCS1-v1_5', cryptoKey, signature, payload);
    }
    catch (err) {
...
let tokenPayloadCache = null;
const decodeOriginToken = async () => {
...
const ensureOriginPolicy = async () => {
...
async function bootstrap() {
    ...
    enableAutoEdit();
    startDesignerAgentSurface();
    window.parent.postMessage({ type: 'designer-ready' }, parentPostMessageOrigin);
}
function maybeBootstrap() {
    if (bootState.originPolicyReady &&
        bootstrapWindow.CSRF_TOKEN &&
        typeof bootstrapWindow.ADMIN_TOKEN !== 'undefined' &&
        document.readyState !== 'loading') {
        bootstrap();
    }
}
function applyInitTokens(msg, eventOrigin) {
    if (msg.originToken && msg.originToken !== originTokenParam) {
        appLogger.warn('Rejecting init message with mismatched origin token');
        return false;
    }
    const respondingOrigin = normalizeOrigin(eventOrigin);
    bootstrapWindow.CSRF_TOKEN = msg.csrfToken;
    bootstrapWindow.ADMIN_TOKEN = msg.adminToken;
...
function maybeApplyCachedInitTokens() {
    const cached = bootstrapWindow.__BLOGPOSTER_APP_INIT_TOKENS__;
    if (!cached || cached.type !== 'init-tokens')
        return;
    if (Array.isArray(cached.allowedOrigins)) {
        addAllowedOrigins(cached.allowedOrigins);
    }
    else if (cached.allowedOrigin) {
        addAllowedOrigins([cached.allowedOrigin]);
    }
    applyInitTokens(cached, parentPostMessageOrigin);
}
window.addEventListener('message', async (event) => {
    ...
    if (msg.type === 'init-tokens') {
        applyInitTokens(msg, event.origin);
    }
