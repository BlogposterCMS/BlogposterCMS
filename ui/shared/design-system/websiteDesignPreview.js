...
export function mountWebsiteDesignPreview(host, branding = () => ({}), source) {
    const render = () => {
    ...
    `;
        ...
        const surface = (dark) => {
            ...
            if (fonts)
                FONT_PACKAGE_ROLES.forEach(role => Object.entries(fonts.roles[role]).forEach(([property, value]) => {
        ...
        if (logos.logoUrl !== undefined || logos.logoDarkUrl !== undefined)
            row('Logo', dark => {
                ...
                if (url) {
                    ...
                    box.textContent = 'No logo selected';
                return box;
            });
        ...
        if (fonts)
            FONT_PACKAGE_ROLES.forEach(role => row(role, dark => {
                const box = surface(dark);
        ...
        (components || []).forEach(component => row(component.name, dark => {
            const box = surface(dark);
            try {
                renderKitComponent(box, component, { theme: dark ? 'dark' : 'light', preview: true });
            }
            catch (error) {
                box.textContent = error instanceof Error ? error.message : 'UI_KIT_COMPONENT_INVALID';
            }
            return box;
        }, component.id));
