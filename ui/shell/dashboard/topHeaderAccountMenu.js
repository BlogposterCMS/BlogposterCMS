...
function getAccountMenuItems(panel) {
    return Array.from(panel.querySelectorAll('.account-menu__item'))
        .filter(item => !item.hasAttribute('disabled') && item.tabIndex !== -1);
}
function focusAccountMenuItem(panel, index) {
...
function focusRelativeAccountMenuItem(panel, direction) {
    const items = getAccountMenuItems(panel);
    if (!items.length)
        return;
...
function setAccountMenuOpen(open, options = {}) {
...
function bindAccountMenuDocumentBehavior() {
    ...
    if (elements.panel.dataset.bound !== 'true') {
        ...
        elements.panel.addEventListener('click', event => {
            ...
            if (!(target instanceof HTMLElement))
                return;
            const item = target.closest('.account-menu__item');
