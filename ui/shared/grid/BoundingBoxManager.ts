...
type EdgePosition = 'n' | 'e' | 's' | 'w' | 'ne' | 'se' | 'sw' | 'nw';
...
export class BoundingBoxManager extends EventTarget {
  ...
  readonly MIN_H = 32;
  box: HTMLDivElement;
  edges: Partial<Record<EdgePosition, HTMLDivElement>>;
  handles: Partial<Record<EdgePosition, HTMLDivElement>>;
  private scheduled = false;
  private readonly updateHandler: () => void;
  private readonly ro: ResizeObserver;
  private onLoad: (() => void) | null = null;

  constructor(canvas: HTMLElement, opts: BoundingBoxOptions = {}) {
    super();
    this.canvas = canvas;
    this.widget = null;
    ...
    ['n', 'e', 's', 'w', 'ne', 'se', 'sw', 'nw'].forEach(pos => {
      const edgePos = pos as EdgePosition;
    ...
    if (opts.handles !== false) {
      const positions: EdgePosition[] = ['nw', 'n', 'ne', 'e', 'se', 's', 'sw', 'w'];
  ...
  setWidget(widget: HTMLElement | null): void {
  ...
  setDisabled(flag: boolean): void {
    ...
    Object.values(this.edges).forEach(edge => {
      if (edge) edge.style.pointerEvents = flag ? 'none' : 'auto';
    });
    if (flag) this.hide();
  }
}
