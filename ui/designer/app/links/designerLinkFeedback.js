import { mountLinkFeedback } from '../../../shared/links/linkFeedback.js';
let controller;
let detach;
/** Scan editable native widget content, including open ShadowRoots, without entering isolated widget sandboxes. */
function collectDesignerLinks(root) {
    const links = [];
    const seen = new Set();
    for (const widget of root.querySelectorAll('.canvas-item')) {
        const owner = widget.dataset.instanceId || widget.dataset.nodeId || widget.id || 'element';
        const visit = (scope) => {
            if (scope instanceof HTMLElement && scope.shadowRoot)
            ...
            for (const element of scope.querySelectorAll('*')) {
                ...
                if (element.matches('a[href]'))
                    links.push({ id: `${owner}:link-${links.length}`, href: element.getAttribute('href') || '', anchor: element });
    ...
    const selected = root.querySelector('.canvas-item.selected, .canvas-item.is-selected, .canvas-item[aria-selected="true"]');
    for (const input of root.querySelectorAll('.scene-button-href, [data-gallery-item-field="href"], [data-navigation-field="homeHref"]')) {
        if (!input.disabled && input.getClientRects().length)
            links.unshift({ id: `${selected?.dataset.instanceId || 'selected'}:${input.id || input.className || 'link-field'}`, href: input.value, anchor: input });
    }
    return links;
}
export function startDesignerLinkFeedback(root) {
    detach?.();
    controller?.stop();
    ...
    const observer = new MutationObserver(records => {
        if (records.some(record => record.target instanceof Element && !record.target.closest('[data-bp-link-feedback]')
            && (record.type === 'attributes' || [...record.addedNodes].some(node => node instanceof Element && (node.matches('a[href], .canvas-item') || node.querySelector('a[href], .canvas-item'))))))
            changed();
    });
