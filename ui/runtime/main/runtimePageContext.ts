export type RuntimeLane = 'admin' | 'public';

export type RuntimePageContextInput = {
  pathname?: string;
...
export type RuntimePageContext = {
  lane: RuntimeLane;
  slug: string;
  debug: boolean;
};

type RuntimePageTitleSource = {
  title?: unknown;
};

function normalizeExplicitSlug(value: unknown): string | null {
  return typeof value === 'string' && value.length > 0 ? value : null;
}

function normalizeAdminBase(value: unknown): string {
...
function stripAdminBase(pathname: string, adminBase: string): string[] {
...
function stripAdminDetailIdentifier(routeParts: string[]): string[] {
...
function isPathInsideBase(pathname: string, base: string): boolean {
  return pathname === base || pathname.startsWith(`${base}/`);
}

export function resolveRuntimePageContext(
  input: RuntimePageContextInput = {}
): RuntimePageContext {
  ...
  const lane: RuntimeLane = isPathInsideBase(pathname, adminBase) || isPathInsideBase(pathname, '/admin')
    ? 'admin'
    : 'public';
  const hasNavigationPath = Object.prototype.hasOwnProperty.call(input, 'pathname');
  const explicitSlug = normalizeExplicitSlug(
    input.pageSlug ?? (hasNavigationPath ? null : window.PAGE_SLUG)
  );
...
export function applyRuntimePageTitle(
  page: RuntimePageTitleSource | null | undefined,
  lane: string
): void {
...
export function exposeRuntimeWidgetRegistry(widgets: unknown): void {
