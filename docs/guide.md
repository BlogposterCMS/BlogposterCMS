...
## Creating Workspaces and Subpages
  ...
  The icon chooser loads the complete list of available icons from `/assets/icons` on demand and displays them in a floating grid. After selecting an icon the grid closes automatically, and submitting the subpage form immediately creates an admin subpage for the current workspace.
