...
function normalizeBoxSpacing(value: unknown): string | undefined {
  if (typeof value !== 'string') return normalizeCssLength(value);
  const parts = value.trim().split(/\s+/);
  if (parts.length < 1 || parts.length > 4 || parts.some(part => !/^\d+(?:\.\d+)?(?:px|rem|em|vh|vw|%)$/i.test(part))) return undefined;
  return parts.join(' ');
}
...
export function normalizeLayoutContainerSettings(value: unknown): LayoutContainerSettings {
  ...
  for (const key of ['borderWidth', 'borderTopWidth', 'borderRightWidth', 'borderBottomWidth', 'borderLeftWidth', 'borderRadius'] as const) {
    const raw = source[key];
    const text = typeof raw === 'number' ? String(raw) : typeof raw === 'string' ? raw.trim() : '';
    if (/^\d+(?:\.\d+)?(?:px)?$/.test(text)) {
      const number = Number.parseFloat(text);
      if (number <= (key === 'borderRadius' ? 512 : 64)) settings[key] = `${number}px`;
    }
  }
  if (['none', 'solid', 'dashed', 'dotted', 'double'].includes(String(source.borderStyle))) {
    settings.borderStyle = source.borderStyle as LayoutContainerSettings['borderStyle'];
  }
